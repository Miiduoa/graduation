...
const DEMO_RECEIVED_REVIEWS: ReceivedReview[] = [
  {
    ...
    overallScore: 4,
    comment:
      '內容論述非常清楚，邏輯結構良好。建議加強創意見解的部分，提供更多自己的看法而不只是文獻回顧。',
  },
  {
    id: 'review_2',
    reviewerLabel: '同學 E',
    ...
    overallScore: 4.6,
    comment:
      '非常優秀的作品！資料蒐集完整，分析深入，表達清晰。唯一建議是可以添加更多圖表來視覺化呈現數據。',
  },
];
...
export function PeerReviewScreen(props: any) {
  ...
  const handleRatingChange = (criterionKey: string, rating: number) => {
    ...
    setReviewForm({
      ...reviewForm,
      criteria: reviewForm.criteria.map((c) => (c.key === criterionKey ? { ...c, rating } : c)),
    });
  };
      ...
      >
        <Screen noPadding>
          ...
          >
            ...
            <AnimatedCard title="提交內容" delay={100}>
              <Text
                style={{
                  fontSize: 14,
                  lineHeight: 20,
            ...
            {/* Comment Section */}
            <View style={{ marginTop: theme.space.lg }}>
              <AnimatedCard title="整體評論" subtitle="必填，至少20個字" delay={300}>
                <TextInput
                ...
                <Text
                  style={{
                    fontSize: 12,
                    color:
                      reviewForm.comment.length >= 20 ? theme.colors.success : theme.colors.muted,
                    marginTop: 8,
                  }}
                >
        ...
        {activeTab === 'assigned' ? (
          <View style={{ gap: theme.space.md, paddingBottom: theme.space.xl }}>
            ...
            {DEMO_SUBMISSIONS.map((submission, idx) => (
              <AnimatedCard key={submission.id} delay={100 + idx * 80}>
                <View style={{ gap: 12 }}>
                  <View
                    style={{
                      flexDirection: 'row',
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {submission.reviewStatus === 'completed' ? '已評審' : '待評審'}
                      </Text>
                    </View>
                  </View>
          ...
          <View style={{ gap: theme.space.md, paddingBottom: theme.space.xl }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 2 }}>
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Ionicons
                          ...
                          name={
                            star <=
                            Math.round(parseFloat(String(getAverageScore(DEMO_RECEIVED_REVIEWS))))
                              ? 'star'
                              : 'star-outline'
                          }
                          size={20}
            ...
            {DEMO_RECEIVED_REVIEWS.map((review, idx) => (
              <AnimatedCard key={review.id} delay={100 + idx * 80}>
                <View style={{ gap: 12 }}>
                  <View>
                    <Text
                      style={{
