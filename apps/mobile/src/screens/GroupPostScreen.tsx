...
export function GroupPostScreen(props: any) {
  ...
  const postAuthorName =
    postAuthor?.displayName || post.authorName || post.authorId.slice(0, 8) || '匿名';
            ...
            >
              ...
              >
                {postAuthor?.avatarUrl ? (
                  ...
                  >
                    <Ionicons
                      name={getKindIcon(post.kind) as any}
                      size={24}
                      color={getKindColor(post.kind)}
                    />
                  </View>
                )}
                <View style={{ flex: 1 }}>
                ...
                {(isMyPost || canManage) && (
                  <Pressable
                    onPress={() => {
                      const options = [];
                      if (isMyPost || canManage)
                        options.push({ text: '編輯', onPress: () => setIsEditingPost(true) });
                      if (canManage)
                        options.push({
                          text: post.pinned ? '取消置頂' : '置頂',
                          onPress: onTogglePin,
                        });
                      if (isMyPost || canManage)
                        options.push({
              ...
              <View style={{ gap: 12 }}>
                {comments.map((c, idx) => {
                  ...
                  const commentAuthorName =
                    commentAuthor?.displayName || c.authorName || c.authorId.slice(0, 8) || '用戶';
                      ...
                      >
                        ...
                        >
                          {commentAuthor?.avatarUrl ? (
                            <Avatar
                              ...
                              size={32}
                              imageUrl={commentAuthor.avatarUrl}
                            />
                          ) : (
                            <View
                              style={{
                        ...
                        {isEditing ? (
                          ...
                          <>
                            ...
                            {c.updatedAt && (
                              <Text
                                style={{ color: theme.colors.muted, fontSize: 10, marginTop: 4 }}
                              >
                                （已編輯）
                              </Text>
                            )}
