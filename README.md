...
- **重算「專案快照」表格中的計數**（與本 README **基準日 2026-05-15** 之表一致時，下列指令應得到：1000／121／14／67／5／24／26／4／5／12；日後若數字漂移，請以指令輸出為準並更新表格）：
...
| 面向              | 盤點結果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git tracked files | **`1000`** 個（**複核**：`git ls-files \| wc -l`；基準日 **2026-05-15**，含 **`studyPlanner`**、**`notificationPlanner`**、**`mistakeRepertoire`**、**`socraticCoach`**、**`feedbackDrafter`**、**`assistantReplySupervisor`**、**`PuWebView`**、**`_mlx_lora_runtime.yaml`**（MLX LoRA 執行期 rank）、**`train_timed.py`**、**`localDocRAG`**、**`tronClassWebUiGate`**（含 **`webViewShouldAllowRequestUrl`**／**`webBrowserOpenWithPuTronClassGate`**）、**`gradePredictor`**、**`gradePredictionFromScoreRows`**、TronClass demo、口試 checklist、[`UI_POLISH_2026_05_14`](docs/UI_POLISH_2026_05_14.md) 對照等）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 測試檔            | Mobile **`67`**（`apps/mobile/src/__tests__/**/*.test.{ts,tsx}`；**不含** `pnpm ai:train:long` 專用之 `apps/mobile/scripts/ai-training-long.test.ts`；代表性 Jest：**`studyPlanner`**、**`notificationPlanner`**、**`mistakeRepertoire`**、**`socraticCoach`**、**`gradePredictor`**、**`feedbackDrafter`**、**`assistantReplySupervisor`**、**`PuWebView`**（`*.test.tsx`）、**`tronClassWebUiGate`**、**`tronClassDataEnabled`**、**`inboxTaskFromNotification`**、**`attendanceEngine`**、**`gradePredictionFromScoreRows`** 等；實際數量以 **`find apps/mobile/src/__tests__ … | wc -l`** 為準）、Web **`5`**（**複核**：`git ls-files 'apps/web/**/*.test.ts' 'apps/web/**/*.test.tsx' \| wc -l`）、Backend Functions **`24`**（**複核**：`git ls-files 'backend/**/*.test.js' \| wc -l`；目錄：`backend/functions/**/*.test.js`，含 **`agent/runtime.safety.test.js`**、`agent/safety.test.js`、`agent/classifyIntent.test.js`）、Rules `1`（`backend/tests/security-rules.test.js`）                                                                                                                                                                                                                                               |
