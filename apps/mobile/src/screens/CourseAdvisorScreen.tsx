      ...
      >
        ...
        >
          {[
            ...
            {
              label: '缺口分類',
              value: `${gaps.filter((g) => g.remaining > 0).length}`,
...
function ConflictSection({ conflicts }: { conflicts: ScheduleConflict[] }) {
  return (
    ...
    >
      <View
        style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: theme.space.md }}
      >
...
function WorkloadSection({ workload }: { workload: WorkloadAnalysis }) {
  const balanceColor =
    workload.balanceScore >= 80
      ? theme.colors.success
      : workload.balanceScore >= 60
        ? theme.colors.accent
        : workload.balanceScore >= 40
          ? theme.colors.warning
          : theme.colors.danger;
  ...
  const allDays = [...(workload.heavyDays || []), ...(workload.lightDays || [])].sort(
    (a, b) => a.day - b.day,
  );
  ...
  return (
    <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      ...
      >
        ...
        >
          <Text style={{ color: balanceColor, fontSize: 28, fontWeight: '800' }}>
