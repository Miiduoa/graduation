...
export function CourseSchedulePanel(props: Record<string, unknown>) {
  ...
  const { items: campusPois } = useAsyncList<CampusPoi>(
  ...
  const handleNavigateToCourseLocation = useCallback(
    (course: CourseSlot) => {
                ...
                if (course && isStart) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        style={{ color: 'rgba(255,255,255,0.8)', fontSize: 9, marginTop: 2 }}
                        numberOfLines={1}
                      >
                        {course.location}
                      </Text>
      ...
      >
        ...
        {schedule.error && (
          <AnimatedCard title="錯誤" delay={250}>
            ...
            <Button text="重試" onPress={handleRefresh} style={{ marginTop: 12 }} />
          </AnimatedCard>
        )}
      </ScrollView>
    </Screen>
