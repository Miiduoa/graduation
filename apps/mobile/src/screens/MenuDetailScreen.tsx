...
import { Screen, Card, Pill, Button, LoadingState, ErrorState, AnimatedCard, StatusBadge, InfoRow, RatingStars, FeatureHighlight, Divider, Avatar, ProgressRing, SegmentedControl } from "../ui/components";
...
import { formatDateTime, isOpenNow, getTimeUntilClose, formatDuration, formatRelativeTime, toDate } from "../utils/format";
...
function generateMockStatus(): CafeteriaStatus {
  ...
  return {
    ...
    openTime: "07:00",
    closeTime: "20:00",
    waitTime: isLunchTime || isDinnerTime ? Math.floor(Math.random() * 15) + 5 : Math.floor(Math.random() * 5),
    queueLength: isLunchTime || isDinnerTime ? Math.floor(Math.random() * 20) + 10 : Math.floor(Math.random() * 5),
...
const REVIEW_TAGS = ["份量大", "CP值高", "好吃", "新鮮", "服務好", "環境乾淨", "排隊長", "偏鹹", "偏油", "份量少"];
...
export function MenuDetailScreen(props: any) {
  ...
  const { items: raw, error: loadError, reload } = useAsyncList<any>(() => ds.listMenus(school.id), [ds, school.id]);

  const item = useMemo(() => findById(raw, id), [raw, id]);

  // Fetch Firebase reviews
  const { items: firebaseReviews, loading: reviewsLoading, reload: reloadReviews } = useAsyncList<Review>(
    async () => {
      if (!id) return [];
      try {
  ...
  const otherMenus = useMemo(() => {
    if (!item) return [];
    return raw
      .filter((m: any) => m.id !== item.id && m.cafeteria === item.cafeteria)
      .slice(0, 4);
  }, [raw, item]);
  ...
  if (loadError) {
    return <ErrorState title="餐點" subtitle="讀取失敗" hint={loadError} actionText="重試" onAction={reload} />;
  }
  if (!item) {
    ...
    <Screen>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
        ...
        <AnimatedCard title={item.name ?? item.cafeteria ?? "餐點"} subtitle="">
          <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
            <View
              style={{
          ...
          {reviews.length > 0 && (
            <View style={{ 
              ...
              <View style={{ alignItems: "center" }}>
                <Text style={{ color: "#F59E0B", fontWeight: "900", fontSize: 32 }}>{avgRating.toFixed(1)}</Text>
                <RatingStars rating={avgRating} size={14} />
                <Text style={{ color: theme.colors.muted, fontSize: 11, marginTop: 4 }}>{reviews.length} 則</Text>
              </View>
                ...
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  return (
                    <View key={star} style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                      <Text style={{ color: theme.colors.muted, fontSize: 10, width: 10 }}>{star}</Text>
          ...
          {popularTags.length > 0 && (
            <View style={{ flexDirection: "row", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
              {popularTags.map((tag) => (
                <View key={tag} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: theme.colors.accentSoft, borderRadius: 12 }}>
                  <Text style={{ color: theme.colors.accent, fontSize: 12, fontWeight: "600" }}>{tag}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          <View style={{ gap: 8, marginBottom: 14 }}>
            ...
            <InfoRow icon="calendar-outline" label="供應日期" value={formatDateTime(item.availableOn)} />
            {status && status.isOpen && (
              <>
                ...
                />
                <InfoRow icon="people-outline" label="目前排隊" value={`約 ${status.queueLength} 人`} />
              </>
            )}
          </View>
            ...
            <View style={{ alignItems: "center", padding: 16 }}>
              <View
                style={{
                  ...
                  height: 80,
                  borderRadius: 40,
                  backgroundColor: status.isOpen ? `${theme.colors.success}20` : `${theme.colors.danger}20`,
                  alignItems: "center",
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12 }}>
              {[
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.text, fontSize: 12, marginTop: 4 }}>{n.label}</Text>
                </View>
              ))}
            </View>
        ...
        <AnimatedCard title="評價" subtitle={`共 ${reviews.length} 則評價`} delay={300}>
          ...
          {!showReviewForm ? (
              ...
              {myReview ? (
                <View style={{ padding: 12, backgroundColor: theme.colors.accentSoft, borderRadius: theme.radius.md }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    ...
                    <Text style={{ color: theme.colors.success, fontSize: 13 }}>你已評價此餐點</Text>
                  </View>
                </View>
              ) : (
                <Button
            ...
            <View style={{ gap: 14, marginBottom: 16, padding: 14, backgroundColor: theme.colors.surface2, borderRadius: theme.radius.md }}>
              ...
              {/* Tags */}
              <View>
                <Text style={{ color: theme.colors.muted, fontSize: 12, marginBottom: 8 }}>選擇標籤（最多5個）</Text>
              ...
              <View>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <Text style={{ color: theme.colors.muted, fontSize: 12 }}>上傳照片（最多3張）</Text>
                  <Pressable onPress={handlePickImages} style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                    <Ionicons name="camera" size={16} color={theme.colors.accent} />
                    <Text style={{ color: theme.colors.accent, fontSize: 12, fontWeight: "600" }}>新增</Text>
                  </Pressable>
                </View>
                    ...
                    {selectedImages.map((uri, idx) => (
                      <View key={idx} style={{ position: "relative" }}>
                        ...
                        <Pressable
                          ...
                          style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: 10, backgroundColor: theme.colors.danger, alignItems: "center", justifyContent: "center" }}
                        >
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                ...
                />
                <Button text="取消" onPress={() => { setShowReviewForm(false); setSelectedTags([]); setSelectedImages([]); setUserRating(0); setUserComment(""); }} />
              </View>
            </View>
          )}
          ...
          {reviews.length > 0 && (
            <View style={{ marginBottom: 14 }}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "flex-end", marginTop: 8 }}>
                <Pressable 
                  onPress={() => setSortBy(prev => prev === "recent" ? "helpful" : prev === "helpful" ? "rating" : "recent")}
                  style={{ flexDirection: "row", alignItems: "center", gap: 4 }}
                >
                  <Text style={{ color: theme.colors.muted, fontSize: 12 }}>
                    排序：{sortBy === "recent" ? "最新" : sortBy === "helpful" ? "最有幫助" : "評分最高"}
                  </Text>
          ...
          <View style={{ gap: 12 }}>
            {reviewsLoading && reviews.length === 0 && (
              <Text style={{ color: theme.colors.muted, textAlign: "center", paddingVertical: 20 }}>載入中...</Text>
            )}
                ...
                <AnimatedCard key={review.id} delay={idx * 20}>
                  <View
                    style={{
                      ...
                      backgroundColor: theme.colors.surface2,
                      borderWidth: 1,
                      borderColor: review.uid === auth.user?.uid ? theme.colors.accent : theme.colors.border,
                    }}
                  >
                    {/* Header */}
                          ...
                          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                            ...
                            {review.uid === auth.user?.uid && (
                              <View style={{ paddingHorizontal: 6, paddingVertical: 2, backgroundColor: theme.colors.accentSoft, borderRadius: 4 }}>
                                <Text style={{ color: theme.colors.accent, fontSize: 10, fontWeight: "600" }}>你</Text>
                              </View>
                            )}
                          </View>
            ...
            {reviews.length === 0 && !reviewsLoading && (
              <View style={{ alignItems: "center", paddingVertical: 30 }}>
                ...
                <Text style={{ color: theme.colors.muted, marginTop: 10 }}>還沒有評價，成為第一個評價的人！</Text>
              </View>
            )}
          </View>
        </AnimatedCard>
            ...
            <View style={{ gap: 10 }}>
              {otherMenus.map((menu) => (
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.accent, fontWeight: "700" }}>${menu.price ?? "-"}</Text>
                </Pressable>
              ))}
            </View>
          </AnimatedCard>
