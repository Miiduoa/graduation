...
export type ProactiveAIReportInput = {
  ...
  courses?: Course[];
  pendingAssignments?: Array<PendingGroupAssignment | {
    id: string;
    groupId?: string;
    groupName?: string;
    title: string;
...
function getCourseMeetingRows(courses: Course[] = []): CourseMeetingRow[] {
  return courses.flatMap((course) => {
    const rows = Array.isArray(course.schedule) && course.schedule.length > 0
...
function normalizeAssignmentDueAt(assignment: ProactiveAIReportInput["pendingAssignments"][number]): Date | null {
...
function createReport(params: Omit<ProactiveAIReport, "sourceHash">): ProactiveAIReport {
  return {
    ...params,
    sourceHash: shortHash(`${params.kind}:${params.title}:${params.body}:${JSON.stringify(params.data ?? {})}`),
  };
}
...
function buildDailyBrief(input: ProactiveAIReportInput, now: Date): ProactiveAIReport | null {
  ...
  const rows = getCourseMeetingRows(input.courses ?? [])
    ...
    .sort((left, right) => (timeToMinutes(left.startTime) ?? 9999) - (timeToMinutes(right.startTime) ?? 9999));
  ...
  const importantAnnouncements = (input.announcements ?? []).filter(isImportantAnnouncement).slice(0, 2);
  ...
  if (rows.length > 0) {
    ...
    rows.slice(0, 4).forEach((row, index) => {
      lines.push(`${index + 1}. ${row.course.name} ${formatMeetingTime(row)}${row.location ? `，${row.location}` : ""}`);
    });
  } else {
    lines.push("今天沒有載入課程。");
  }
    ...
    dueSoon.slice(0, 3).forEach((assignment, index) => {
      ...
      const dueText = assignment.isLate
        ...
        : dueAt
          ...
          : "截止時間未提供";
      lines.push(`${index + 1}. ${assignment.title}（${assignment.groupName ?? "課程"}）${dueText ? `：${dueText}` : ""}`);
    });
  }
  ...
  return createReport({
    ...
    actions: [
      { label: "查看課表", action: "navigate", params: { screen: "課程", nested: "CourseSchedule" } },
...
function buildClassSoonReports(input: ProactiveAIReportInput, now: Date): ProactiveAIReport[] {
  return getCourseMeetingRows(input.courses ?? [])
    ...
    .map(({ row, startAt, minutes }) => createReport({
      ...
      actions: [{ label: "打開課表", action: "navigate", params: { screen: "課程", nested: "CourseSchedule" } }],
    }));
}
    ...
    if (assignment.isLate) {
      return [createReport({
        ...
        actions: assignment.groupId
          ? [{ label: "查看作業", action: "navigate", params: { screen: "收件匣", nested: "AssignmentDetail", groupId: assignment.groupId, assignmentId: assignment.id } }]
          : [{ label: "查看收件匣", action: "navigate", params: { screen: "收件匣", nested: "Inbox" } }],
      })];
    }
...
function buildAnnouncementReports(input: ProactiveAIReportInput, now: Date): ProactiveAIReport[] {
  return (input.announcements ?? [])
    ...
    .slice(0, 5)
    .map((announcement) => createReport({
      id: `announcement-${announcement.id}`,
      kind: "announcement",
      priority: /緊急|停課|安全|停水|停電/.test(`${announcement.title} ${announcement.body ?? ""}`) ? "high" : "medium",
      title: "重要公告提醒",
      ...
      actions: [{ label: "查看公告", action: "navigate", params: { screen: "Today", nested: "公告詳情", id: announcement.id } }],
    }));
}
...
export function getProactiveAIReportsStorageKey(userId?: string | null, schoolId?: string | null): string {
  return getScopedStorageKey("ai-proactive-reports", { uid: userId ?? null, schoolId: schoolId ?? null });
}

function getProactiveAIStateStorageKey(userId?: string | null, schoolId?: string | null): string {
  return getScopedStorageKey("ai-proactive-state", { uid: userId ?? null, schoolId: schoolId ?? null });
}
...
function normalizeReport(value: unknown): ProactiveAIReport | null {
  ...
  return {
    ...
    data: raw.data,
    suggestions: Array.isArray(raw.suggestions) ? raw.suggestions.filter((item): item is string => typeof item === "string") : undefined,
...
export async function loadProactiveAIReports(params: {
  ...
  try {
    const raw = await AsyncStorage.getItem(getProactiveAIReportsStorageKey(params.userId, params.schoolId));
    if (!raw) return [];
...
async function saveProactiveAIReports(params: {
  userId?: string | null;
  schoolId?: string | null;
}, reports: ProactiveAIReport[]): Promise<void> {
...
async function loadProactiveAIState(params: {
  ...
  try {
    const raw = await AsyncStorage.getItem(getProactiveAIStateStorageKey(params.userId, params.schoolId));
    ...
    return {
      ...
      scheduledNotificationIds: Array.isArray(parsed.scheduledNotificationIds) ? parsed.scheduledNotificationIds.filter((id): id is string => typeof id === "string") : [],
    };
  } catch {
...
async function saveProactiveAIState(params: {
  userId?: string | null;
  schoolId?: string | null;
}, state: ProactiveAIState): Promise<void> {
  await AsyncStorage.setItem(getProactiveAIStateStorageKey(params.userId, params.schoolId), JSON.stringify(state));
}

function mergeReports(existing: ProactiveAIReport[], generated: ProactiveAIReport[], now: Date): ProactiveAIReport[] {
  const nowTime = now.getTime();
  const byId = new Map<string, ProactiveAIReport>();

  existing.forEach((report) => {
    const expiresAt = report.expiresAt ? new Date(report.expiresAt).getTime() : Number.POSITIVE_INFINITY;
...
export async function markProactiveAIReportsSeen(params: {
  ...
  const updated = reports.map((report) => reportIds.has(report.id) ? { ...report, seenInChat: true } : report);
...
function weeklyReminderTrigger(dayOfWeek: number, startTime: string, leadMinutes: number): Notifications.NotificationTriggerInput | null {
...
function buildScheduledNotificationPlans(input: ProactiveAIReportInput): NotificationPlan[] {
  ...
  getCourseMeetingRows(input.courses ?? []).slice(0, 40).forEach((row) => {
...
async function syncProactiveAINotificationSchedules(input: ProactiveAIReportInput, state: ProactiveAIState): Promise<ProactiveAIState> {
  ...
  await Promise.all(state.scheduledNotificationIds.map((id) => cancelNotification(id).catch(() => void 0)));
...
export async function syncProactiveAIReports(
  ...
  if (options.notify !== false && newReports.length > 0) {
    ...
    if (permission.granted) {
      ...
      if (notifiedIds.size > 0) {
        reports = reports.map((report) => notifiedIds.has(report.id)
          ? { ...report, notifiedAt: now.toISOString() }
          : report);
      }
    }
  }
  ...
  await Promise.all([
    saveProactiveAIReports(params, reports),
    saveProactiveAIState(params, state),
  ]);
