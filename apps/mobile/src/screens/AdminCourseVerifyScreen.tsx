...
export function AdminCourseVerifyScreen() {
  ...
  const stats = useMemo(() => {
    ...
    const pending = items.filter(
      (g) => !g.verification?.status || g.verification.status === 'unverified',
    ).length;
    const verified = items.filter(
      (g) =>
        g.verification?.status === 'verified_teacher' || g.verification?.status === 'verified_org',
    ).length;
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filterStatus === 'pending') {
      ...
      list = list.filter(
        (g) =>
          g.verification?.status === 'verified_teacher' ||
          g.verification?.status === 'verified_org',
      );
  ...
  const rejectCourse = async (g: CourseGroup) => {
    ...
    Alert.alert('拒絕認證', `確定要拒絕「${g.name}」的認證申請嗎？`, [
      { text: '取消', style: 'cancel' },
      {
        text: '確認拒絕',
        style: 'destructive',
  ...
  const handleBatchVerify = async () => {
    ...
    Alert.alert('批量認證', `確定要認證 ${selectedForBatch.length} 個課程嗎？`, [
      { text: '取消', style: 'cancel' },
      {
        text: '確認認證',
        onPress: async () => {
    ...
    <Screen>
      {loading ? (
        ...
        >
          ...
          <AnimatedCard
            title="課程認證管理"
            subtitle={`${school.name} · ${auth.user?.email ?? ''}`}
          >
            {/* Stats */}
          ...
          {/* Course List */}
          <AnimatedCard
            title={`${getStatusLabel(filterStatus === 'all' ? undefined : filterStatus === 'pending' ? 'unverified' : filterStatus === 'verified' ? 'verified_teacher' : 'rejected')}課程`}
            subtitle={`共 ${filteredItems.length} 個`}
            delay={100}
          >
            <View style={{ gap: 12 }}>
                    ...
                    >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          >
                            ...
                            >
                              <Text
                                style={{
                                  color: getStatusColor(g.verification?.status),
                                  fontSize: 11,
                                  fontWeight: '600',
                                }}
                              >
                          ...
                          {g.description && (
                            <Text
                              style={{ color: theme.colors.muted, fontSize: 13, marginTop: 4 }}
                              numberOfLines={2}
                            >
                              {g.description}
                            </Text>
                      ...
                      <View style={{ marginTop: 12, gap: 6 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                          ...
                          <Text
                            style={{ color: theme.colors.text, fontWeight: '600', fontSize: 12 }}
                          >
                            {g.joinCode}
                          </Text>
                        </View>
                      ...
                      {g.verification?.verifiedAt && (
                        ...
                        >
                          <Text style={{ color: theme.colors.muted, fontSize: 11 }}>
                            {g.verification.status === 'rejected' ? '拒絕' : '認證'}於{' '}
                            {formatRelativeTime(toDate(g.verification.verifiedAt))}
                            {g.verification.verifiedByEmail
                              ? ` · ${g.verification.verifiedByEmail}`
                              : ''}
                          </Text>
                      ...
                      {!showBatchMode && isPending && (
                        <View
                          style={{ marginTop: 14, flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}
                        >
                          <Button
