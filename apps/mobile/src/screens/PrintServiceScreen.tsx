...
import {
  ...
  SearchBar, ProgressRing, EmptyState,
} from "../ui/components";
...
import {
  PRINT_SERVICE_INFO, PRICING_RULES, calculateCost,
  type PaperSize, type ColorMode, type PrintMode,
  PRINT_STATIONS, type PrintStation, type MachineStatus, type MachineType,
  getStationStatusLabel, getStationStatusColor,
  getMachineTypeLabel, getMachineTypeIcon,
  recommendStation, type PrintRecommendation,
  ECO_ACTIONS, ECO_LEVELS, getEcoLevel, type EcoAction, type EcoLevel,
  PRINT_QUOTAS,
  PRINT_SCENARIOS, type PrintScenario,
  simulateStationStatus,
  PRINT_FAQS, type PrintFAQ,
  ROLE_PRINT_ACCESS,
...
export function PrintServiceScreen(props: any) {
  ...
  const [stationFilter, setStationFilter] = useState<"all" | "printer" | "copier" | "multifunction">("all");
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setStations(updated);
      if (auth.user?.uid) {
        const jobsData = await ds.listPrintJobs(auth.user.uid, undefined, school?.id).catch(() => []);
        setJobs(jobsData);
      }
    } catch (e) {
        ...
        { text: "取消", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("已送出 ✅", `列印工作已送至 ${selectedStation.name}\n${duplex ? "🌿 獲得 2 環保積分！" : ""}`);
            } catch (err: any) {
  ...
  const renderHome = () => (
    <View style={{ gap: 14, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
        ...
        <View style={{ gap: 12 }}>
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            <View style={{
              ...
              <Text style={{ color: theme.colors.accent, fontSize: 22, fontWeight: "800" }}>{cardBalance}</Text>
              <Text style={{ color: theme.colors.accent, fontSize: 11, marginTop: 2 }}>剩餘點數</Text>
            </View>
            <View style={{
              flex: 1, padding: 12, borderRadius: theme.radius.md,
              backgroundColor: theme.colors.successSoft, alignItems: "center",
            }}>
              <Text style={{ color: theme.colors.success, fontSize: 22, fontWeight: "800" }}>{ecoPoints}</Text>
              <Text style={{ color: theme.colors.success, fontSize: 11, marginTop: 2 }}>環保積分</Text>
            </View>
            <View style={{
              flex: 1, padding: 12, borderRadius: theme.radius.md,
              backgroundColor: theme.colors.surface2, alignItems: "center",
            }}>
              <Text style={{ color: theme.colors.text, fontSize: 22, fontWeight: "800" }}>{totalPrintedPages}</Text>
              <Text style={{ color: theme.colors.muted, fontSize: 11, marginTop: 2 }}>已印頁數</Text>
            </View>
          </View>
        </View>
      </AnimatedCard>
      ...
      <AnimatedCard title="我要做什麼？" delay={80}>
          ...
          {PRINT_SCENARIOS.map((sc) => (
            <Pressable
              ...
              style={{
                ...
                backgroundColor: selectedScenario?.id === sc.id ? theme.colors.accentSoft : theme.colors.surface2,
        ...
        {selectedScenario?.savingTip && (
          <View style={{
            ...
            padding: 10, borderRadius: theme.radius.md, backgroundColor: theme.colors.successSoft,
          }}>
        ...
        {selectedFile ? (
          <View style={{ gap: 10 }}>
            <View style={{
              ...
              padding: 14, borderRadius: theme.radius.md, backgroundColor: theme.colors.surface2,
            }}>
              <View style={{
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 2 }}>
                  {(selectedFile.size / 1024).toFixed(1)} KB · 預估 {Math.max(1, Math.ceil(selectedFile.size / 50000))} 頁
                </Text>
              </View>
          ...
          <Pressable onPress={handleSelectFile} style={{
            ...
            <Text style={{ color: theme.colors.text, fontWeight: "700", marginTop: 10, fontSize: 15 }}>
              點擊上傳檔案
            </Text>
            <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 4 }}>
              PDF、Word、圖片
      ...
      <AnimatedCard title="列印設定" delay={160}>
        <View style={{ gap: 14 }}>
              ...
              {(["A4", "B4", "A3"] as PaperSize[]).map((size) => (
                <Pressable
                  ...
                  style={{
                    ...
                    backgroundColor: paperSize === size ? theme.colors.accentSoft : theme.colors.surface2,
          ...
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <ToggleSwitch value={colorMode === "color"} onToggle={() => setColorMode(colorMode === "color" ? "bw" : "color")} />
          </View>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
              ...
              <Text style={{ color: theme.colors.text, fontWeight: "800", fontSize: 16, minWidth: 28, textAlign: "center" }}>
                {copies}
              </Text>
              <Pressable
            ...
            {recommendations.slice(0, 3).map((rec, idx) => (
              <Pressable
                ...
                style={{
                  ...
                  backgroundColor: selectedStation?.id === rec.station.id
                    ? theme.colors.accentSoft : theme.colors.surface2,
                  borderWidth: selectedStation?.id === rec.station.id ? 1.5 : 0,
                  borderColor: selectedStation?.id === rec.station.id ? theme.colors.accent : "transparent",
                }}
              >
                {/* 排名 */}
                <View style={{
                ...
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: "700" }}>{rec.station.name}</Text>
                    <View style={{
            ...
            return (
              <View style={{ gap: 10 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  ...
                  <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{est} 頁 × {copies} 份</Text>
                </View>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  <Text style={{ color: theme.colors.muted }}>單價</Text>
                  <Text style={{ color: theme.colors.text, fontWeight: "600" }}>
                    {PRICING_RULES.find((r) => r.paperSize === paperSize && r.colorMode === colorMode)?.pointsPerPage ?? 1} 點/張
                  </Text>
                </View>
                {duplex && (
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                ...
                <View style={{
                  height: 1, backgroundColor: theme.colors.border, marginVertical: 4,
                }} />
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "700", fontSize: 15 }}>總計</Text>
      ...
      {/* 收費速查 */}
      <AnimatedCard title="收費標準" subtitle={`影印卡 $${PRINT_SERVICE_INFO.cardPrice} / ${PRINT_SERVICE_INFO.cardPoints} 點`} delay={280}>
        <View style={{ gap: 8 }}>
          {PRICING_RULES.map((rule) => (
            <View key={rule.description} style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Text style={{ color: theme.colors.muted }}>{rule.description}</Text>
              <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{rule.pointsPerPage} 點/張</Text>
            </View>
          ))}
          <View style={{
    ...
    <View style={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
        ...
        {([
          ...
          <Pressable
            ...
            style={{
              ...
              backgroundColor: stationFilter === f.key ? theme.colors.accentSoft : theme.colors.surface2,
      ...
      <View style={{
        ...
        <StatChip
          ...
          count={stations.filter((s) => ["offline", "error", "outOfPaper", "outOfToner", "maintenance"].includes(s.status)).length}
          label="異常"
        />
      </View>
          ...
          >
            <View style={{ gap: 10 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                ...
                <View style={{ alignItems: "flex-end", gap: 4 }}>
                  <Pill
                    ...
                    kind={station.status === "online" ? "success" : station.status === "busy" ? "warning" : "danger"}
                    size="sm"
                  />
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                {station.colorModes.includes("color") && <Pill text="彩色" kind="accent" size="sm" icon="color-palette" />}
                {station.paperSizes.includes("A3") && <Pill text="A3" size="sm" />}
                {station.supportsCloud && <Pill text="雲端" kind="accent" size="sm" icon="cloud-outline" />}
                {station.supportsScan && <Pill text="掃描" size="sm" icon="scan-outline" />}
                {station.capabilities.includes("duplex") && <Pill text="雙面" size="sm" icon="copy-outline" />}
                {station.capabilities.includes("staple") && <Pill text="裝訂" size="sm" icon="attach-outline" />}
              </View>
  ...
  const renderHistory = () => (
    <View style={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
      ...
      <AnimatedCard>
        <View style={{ flexDirection: "row", gap: 12 }}>
          <View style={{
            ...
            <Text style={{ color: theme.colors.accent, fontSize: 28, fontWeight: "800" }}>{cardBalance}</Text>
            <Text style={{ color: theme.colors.accent, fontSize: 12, marginTop: 4 }}>目前點數</Text>
            <View style={{
              marginTop: 8, paddingHorizontal: 10, paddingVertical: 4,
              borderRadius: theme.radius.full, backgroundColor: `${theme.colors.accent}20`,
            }}>
          ...
          <View style={{
            ...
            <Text style={{ color: theme.colors.text, fontSize: 28, fontWeight: "800" }}>{totalPrintedPages}</Text>
            <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 4 }}>本學期已印</Text>
            <View style={{
              marginTop: 8, paddingHorizontal: 10, paddingVertical: 4,
      ...
      <AnimatedCard title="免費列印額度" subtitle="依身份別" delay={60}>
        <View style={{ gap: 8 }}>
          {PRINT_QUOTAS.map((q) => (
            <View key={q.role} style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={{ color: theme.colors.muted, fontSize: 13 }}>{q.description.split("每學期")[0]}</Text>
              <Text style={{ color: q.semesterFreePages > 0 ? theme.colors.accent : theme.colors.muted, fontWeight: "700" }}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                  ...
                  <View style={{ alignItems: "flex-end" }}>
                    <Pill
                      ...
                      kind={job.status === "completed" ? "success" : job.status === "failed" ? "danger" : "default"}
                      size="sm"
                    />
                    <Text style={{ color: theme.colors.accent, fontWeight: "700", fontSize: 13, marginTop: 4 }}>
                      ${job.cost}
                    </Text>
                  </View>
                </View>
    ...
    <View style={{ gap: 14, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
        ...
        <View style={{ alignItems: "center", gap: 12, paddingVertical: 8 }}>
          ...
          {nextLevel && (
            <View style={{ width: "100%", gap: 6, marginTop: 4 }}>
              ...
              <View style={{
                height: 8, borderRadius: 4, backgroundColor: theme.colors.surface2, overflow: "hidden",
              }}>
                <View style={{
      ...
      <AnimatedCard title="你的環保成就" delay={180}>
        <View style={{
          ...
          backgroundColor: theme.colors.successSoft, alignItems: "center", gap: 8,
        }}>
              ...
              {PRINT_FAQS.map((faq, idx) => (
                ...
                >
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    ...
                    />
                    <Text style={{
                      color: theme.colors.text, fontWeight: "700", flex: 1, fontSize: 14,
                    }}>
                      {faq.question}
                    </Text>
                    <Ionicons
                  ...
                  {expandedFAQ === idx && (
                    <Text style={{
                      color: theme.colors.muted, fontSize: 13, marginTop: 10,
                      lineHeight: 20, paddingLeft: 30,
                    }}>
                      {faq.answer}
                    </Text>
                  )}
          ...
          <View style={{
            ...
            <ScrollView style={{ padding: 20 }}>
              <View style={{ gap: 16, paddingBottom: 20 }}>
                {/* 基本資訊 */}
                <View style={{ gap: 10 }}>
                  <InfoRow icon="location-outline" label="位置" value={`${selectedStation.building} ${selectedStation.location}`} />
                  <InfoRow icon="print-outline" label="機型" value={selectedStation.model} />
                  <InfoRow icon="albums-outline" label="類型" value={getMachineTypeLabel(selectedStation.type)} />
                  <InfoRow
                    icon="ellipse"
                    label="狀態"
                ...
                {/* 耗材 */}
                <View style={{ gap: 8 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "700", marginBottom: 4 }}>耗材狀態</Text>
                  <SupplyBar label="碳粉" value={selectedStation.tonerLevel} icon="water-outline" />
                  <SupplyBar label="紙張" value={selectedStation.paperLevel} icon="document-outline" />
                </View>

                {/* 支援功能 */}
                <View style={{ gap: 8 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "700", marginBottom: 4 }}>支援功能</Text>
                  <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                    {selectedStation.paperSizes.map((s) => <Pill key={s} text={s} size="sm" />)}
                    {selectedStation.colorModes.includes("color") && <Pill text="彩色" kind="accent" size="sm" />}
                    {selectedStation.supportsCloud && <Pill text="雲端列印" kind="accent" size="sm" icon="cloud-outline" />}
                    {selectedStation.supportsScan && <Pill text="掃描" size="sm" icon="scan-outline" />}
                    {selectedStation.capabilities.includes("duplex") && <Pill text="雙面" size="sm" />}
                    {selectedStation.capabilities.includes("staple") && <Pill text="裝訂" size="sm" />}
                    {selectedStation.capabilities.includes("scan_to_email") && <Pill text="掃描至 Email" size="sm" />}
                    {selectedStation.capabilities.includes("scan_to_usb") && <Pill text="掃描至 USB" size="sm" />}
                  </View>
                </View>
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  <Button
                    text="選擇此機器"
                    kind="primary"
                    disabled={selectedStation.status === "offline" || selectedStation.status === "maintenance"}
                    onPress={() => {
...
function getJobStatusLabel(status: PrintJob["status"]): string {
  const m: Record<string, string> = {
    pending: "等待中", printing: "列印中", completed: "已完成", failed: "失敗", cancelled: "已取消",
  };
...
function getJobStatusColor(status: PrintJob["status"]): string {
  const m: Record<string, string> = {
    pending: theme.colors.muted, printing: "#F59E0B",
    completed: theme.colors.success, failed: theme.colors.danger, cancelled: theme.colors.muted,
  };
