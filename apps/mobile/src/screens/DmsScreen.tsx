...
export function DmsScreen(props: any) {
  ...
  const { items, loading, error, reload } = useAsyncList<Conversation>(async () => {
      ...
      <Screen>
        <AnimatedCard title="私訊" subtitle="與其他用戶的對話">
          <View style={{ alignItems: 'center', padding: 24 }}>
            ...
            <Text
              style={{ color: theme.colors.text, fontWeight: '700', fontSize: 18, marginBottom: 8 }}
            >
              尚未登入
            </Text>
            ...
            <View style={{ marginTop: 10, gap: 10 }}>
              {rows.map((c) => {
                ...
                return (
                  ...
                  >
                    <Card
                      title={`${peerId.slice(0, 8)}…`}
                      subtitle={c.lastMessageText ?? '(尚無訊息)'}
                    >
                      <Button
                        text="開啟"
                        kind="primary"
