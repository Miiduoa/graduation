...
function navAction(label: string, params: Record<string, unknown>, sensitivity: AssistantActionProposal["sensitivity"] = "low"): AssistantActionProposal {
...
function executed(message: string, recordId?: string, actions?: AssistantActionProposal[]): AIActionExecutionResult {
...
function timeSlotRange(slot: unknown): { start: string; end: string; label: string } {
  ...
  if (match) return { start: match[1].replace("：", ":"), end: match[2].replace("：", ":"), label: value };
...
function chooseMenu(params: Record<string, any>, menus: MenuItem[] = [], userMessage = ""): MenuItem | undefined {
...
function chooseCafeteria(
  ...
  return cafeterias.find((cafeteria) => {
    ...
    return name === requested || name.includes(requested) || requested.includes(name) || normalizeText(cafeteria.id) === requested;
  });
}

function findCourse(params: Record<string, any>, courses: CourseLike[] = []): CourseLike | undefined {
...
function chooseSeat(params: Record<string, any>, seats: LibrarySeat[]): LibrarySeat | undefined {
  ...
  if (explicitSeat) return seats.find((seat) => seat.id === explicitSeat || seat.seatNumber === explicitSeat);

  const floor = asString(params.floor);
  const type = asString(params.type);
  return seats.find((seat) => {
    if (seat.status !== "available") return false;
    if (floor && seat.floor !== floor && !normalizeText(seat.name).includes(normalizeText(floor))) return false;
...
function choosePrinter(params: Record<string, any>, printers: Printer[]): Printer | undefined {
  ...
  if (explicit) {
    const byId = printers.find((printer) => printer.id === explicit || normalizeText(printer.name).includes(normalizeText(explicit)));
    if (byId) return byId;
  }
  return printers.find((printer) => printer.status === "online") ?? printers[0];
}

async function createActionDraft(ctx: AIActionExecutorContext, reason?: string): Promise<AIActionExecutionResult> {
...
async function executeOrderMeal(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  if (!cafeteria) return blocked("缺少可驗證的餐廳資料，不能建立正式訂單。", defaultActionsForTool(ctx.tool));
  if (cafeteria.orderingEnabled !== true || cafeteria.pilotStatus === "inactive" || (cafeteria.activeOperatorCount ?? 0) <= 0) {
    return blocked("這間餐廳尚未開通 APP 接單或沒有店員在線，不能假裝已下單。", defaultActionsForTool(ctx.tool));
  }
  if (!menu || typeof menu.price !== "number") {
    return blocked("找不到可下單的正式品項或價格，請從餐廳頁選擇可下單餐點。", defaultActionsForTool(ctx.tool));
  }
...
async function executeHealthBooking(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  return executed(`已預約健康服務。\n預約編號：${appointment.id}\n日期：${appointment.date}\n時段：${appointment.timeSlot}`, appointment.id, defaultActionsForTool(ctx.tool));
}

async function executeSeatReservation(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  const { params, userId, schoolId, dataSource } = ctx;
  const authBlock = requireSignedIn(userId);
  if (authBlock) return authBlock;
  const schoolBlock = requireSchool(schoolId);
  ...
  if (!seat) return blocked("目前找不到可預約的圖書館座位，沒有建立預約。", defaultActionsForTool(ctx.tool));

  const date = parseDateValue(params.date);
  const slot = timeSlotRange(params.time_slot ?? params.timeSlot);
  const reservation = await dataSource.reserveSeat(seat.id, userId!, date, slot.start, slot.end, schoolId!);
  return executed(`已預約圖書館座位。\n座位：${seat.name ?? seat.seatNumber}\n日期：${date}\n時段：${slot.label}`, reservation.id, defaultActionsForTool(ctx.tool));
}

async function executeRepairRequest(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  return executed(`維修單已提交。\n工單編號：${repair.id}\n房號：${repair.room}\n狀態：${repair.status}`, repair.id, defaultActionsForTool(ctx.tool));
}

async function executeLostFoundPost(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  const { params, userId, schoolId, dataSource } = ctx;
  const authBlock = requireSignedIn(userId);
  if (authBlock) return authBlock;
  const schoolBlock = requireSchool(schoolId);
  ...
  const item = await dataSource.createLostFoundItem({
    ...
    description: asString(params.features) || asString(params.description) || "AI 代理建立的遺失物公告",
  ...
  return executed(`遺失公告已發布。\n公告編號：${item.id}\n物品：${item.title}\n地點：${item.location}`, item.id, defaultActionsForTool(ctx.tool));
}
...
async function executePrintJob(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  if (!printer) return blocked("目前找不到可用印表機，沒有建立列印工作。", defaultActionsForTool(ctx.tool));
  ...
  return executed(`列印工作已建立。\n工作編號：${job.id}\n印表機：${printer.name}\n檔案：${fileName}`, job.id, defaultActionsForTool(ctx.tool));
}
...
async function executeReminder(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  return executed(`已建立提醒。\n提醒編號：${created.id}\n內容：${created.title}\n時間：${created.startAt ?? created.startDate}`, created.id, defaultActionsForTool(ctx.tool));
}
...
async function executeSendMessage(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  if (!conversationId && !recipientId) return createActionDraft(ctx, "無法解析收件人，不能直接送出訊息。");
...
async function executeAssignmentPublish(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  ...
  return executed(`作業已發布。\n作業編號：${assignment.id}\n課程：${course?.name ?? groupId}\n標題：${assignment.title}`, assignment.id, defaultActionsForTool(ctx.tool));
}

export async function executeAgentToolAction(ctx: AIActionExecutorContext): Promise<AIActionExecutionResult> {
  const { tool, role } = ctx;
