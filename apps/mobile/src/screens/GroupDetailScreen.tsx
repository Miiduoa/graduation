...
export function GroupDetailScreen(props: any) {
  ...
  const { items: lastCommentRows, reload: reloadComments } = useAsyncList<{
    postId: string;
    ...
    for (let i = 0; i < posts.length; i += batchSize) {
      ...
      const promises = batch.map(async (p) => {
        try {
          ...
          const c = cs.empty
            ? null
            : ({ id: cs.docs[0].id, postId: p.id, ...(cs.docs[0].data() as any) } as Comment);
  ...
  const onArchiveToKnowledgeBase = async (postId: string, postTitle: string, postBody: string) => {
    if (!groupId || !auth.user) return;
    Alert.alert('歸入知識庫', `將「${postTitle}」歸入此課程的知識庫，讓未來的同學更容易找到？`, [
    ...
    <Screen>
      ...
      >
        <Card title="群組資訊" subtitle={`${group?.type ?? 'group'}｜${group?.name ?? ''}`}>
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            {group?.type === 'course' ? (
              <Pill
                text={
                  (group?.verification?.status ?? 'unverified') === 'verified_teacher'
                    ? '老師認證'
                    : '未驗證'
                }
                kind={
            ...
            {group?.type === 'course' ? (
              <Pill text={group?.isPublished ? '已發布' : '未發布'} />
            ) : null}
          </View>
          ...
          <View style={{ marginTop: 10, flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
            <Button
              text="成員"
              kind="primary"
              onPress={() => nav?.navigate?.('GroupMembers', { groupId })}
            />
            {group?.type === 'course' ? (
              <Button
                text="作業 / 成績"
        ...
        >
          <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
            <Button
              text={composeKind === 'question' ? '✓ 問題' : '問題'}
              onPress={() => setComposeKind('question')}
            />
            <Button
              text={composeKind === 'post' ? '✓ 貼文' : '貼文'}
              onPress={() => setComposeKind('post')}
            />
            {canCreateAnnouncement ? (
              <Button
          ...
          <Card title="公告 / Q&A" subtitle="最新 50 則">
            ...
            <View style={{ marginTop: 10, flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
              <Pressable
                ...
                style={({ pressed }) => ({
                  ...
                  borderWidth: 1,
                  borderColor:
                    selectedTopicFilter === null ? theme.colors.accent : theme.colors.border,
                  opacity: pressed ? 0.7 : 1,
                })}
              >
                <Text
              ...
              {(['課程討論', '作業問題', '考試準備', '資源分享', '其他'] as Topic[]).map((t) => (
                <Pressable
                  ...
                  style={({ pressed }) => ({
                    ...
                    borderWidth: 1,
                    borderColor:
                      selectedTopicFilter === t ? theme.colors.accent : theme.colors.border,
                    opacity: pressed ? 0.7 : 1,
                  })}
                >
                  <Text
            ...
            <View style={{ marginTop: 10, gap: 10 }}>
              {visiblePosts.map((p) => {
                ...
                return (
                  ...
                  >
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        >
                          ...
                          <Pill
                            ...
                            kind={
                              isQuestion && p.solved ? 'success' : isQuestion ? 'accent' : 'default'
                            }
                            size="sm"
                          />
                          {p.topic && <Pill text={p.topic} kind="default" size="sm" />}
                    ...
                    <Text style={{ color: theme.colors.text, lineHeight: 20 }} numberOfLines={3}>
                      {p.body}
                    </Text>
                    ...
                    <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {isOwnerOrTeacher && isQuestion && !p.solved && (
                        ...
                        >
                          <Text
                            style={{ color: theme.colors.success, fontSize: 13, fontWeight: '700' }}
                          >
                            標記最佳解答
                          </Text>
                        </Pressable>
              ...
              {visiblePosts.length === 0 ? (
                <Text style={{ color: theme.colors.muted }}>目前沒有貼文。</Text>
              ) : null}
            </View>
          </Card>
        )}
