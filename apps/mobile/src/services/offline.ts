    ...
    if (wasOffline && isNowOnline && !isProcessingQueue) {
      ...
      setTimeout(() => {
        if (
          ...
          currentStatus.isInternetReachable !== false &&
          !isProcessingQueue
        ) {
  ...
  for (const action of sortedQueue) {
    ...
    try {
      ...
      if (action.retryCount < RETRY_CONFIG.maxRetries) {
        ...
        console.log(
          `[offline] Will retry action ${action.id} in ${delay}ms (attempt ${action.retryCount})`,
        );
 ...
 */
async function verifyActionUser(
  action: QueuedAction,
  ...
  try {
    ...
    if (currentUser.uid !== action.userId) {
      console.warn(
        '[offline] Action belongs to different user:',
        action.userId,
        'current:',
        currentUser.uid,
      );
...
async function processQueuedAction(
  action: QueuedAction,
): Promise<{ success: boolean; conflict: boolean; conflictInfo?: ConflictInfo }> {
    ...
    case 'update': {
      ...
      if (existingUpdateDoc.exists()) {
        ...
        const serverUpdatedAt =
          serverData.updatedAt?.toMillis?.() ?? serverData._offlineUpdatedAt ?? 0;
        ...
        if (serverUpdatedAt > effectiveAction.timestamp) {
          ...
          if (conflictFields.length > 0) {
            console.warn(
              '[offline] Server version is newer with conflicting fields:',
              conflictFields,
            );
            const conflictInfo: ConflictInfo = {
              action: effectiveAction,
  ...
  console.log(
    '[offline] Successfully processed:',
    effectiveAction.type,
    effectiveAction.collection,
    effectiveAction.documentId,
  );
  return { success: true, conflict: false };
}
...
export async function getOfflineDataSize(): Promise<{ count: number; approximateBytes: number }> {
  ...
  const offlineKeys = keys.filter(
    (k) => k.startsWith(OFFLINE_DATA_PREFIX) || k === OFFLINE_QUEUE_KEY,
  );
...
export const OFFLINE_SYNC_CONFIG: Record<
  string,
  { priority: OfflinePriority; maxAgeHours: number }
> = {
...
export async function syncImportantData(
  schoolId: string,
): Promise<{ synced: string[]; failed: string[] }> {
  const isOnline = await checkConnectivity();
  if (!isOnline) {
