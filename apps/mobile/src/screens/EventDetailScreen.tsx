...
export function EventDetailScreen(props: any) {
  ...
  } = useAsyncList<Registration>(async () => {
    if (!id) return [];
    return loadEventRegistrations(id, school.id);
  }, [id, school.id]);

  // Fetch reviews for ended events
  const { items: reviews, reload: reloadReviews } = useAsyncList<EventReview>(async () => {
    if (!id) return [];
  ...
  const myRegistration = useMemo(() => {
    ...
    return (
      registrations.find(
        (r) => r.uid === auth.user?.uid && (r.status === 'registered' || r.status === 'waitlist'),
      ) ?? null
    );
  ...
  const onRegister = async (joinWaitlist = false) => {
    ...
    if (isFull && !joinWaitlist) {
      Alert.alert('名額已滿', '是否要加入候補名單？有名額釋出時會通知你。', [
  ...
  const handleAddReminder = () => {
    Alert.alert('設定活動提醒', '選擇提醒時間', [
      {
        text: '活動前 1 小時',
        onPress: () => Alert.alert('已設定', '將在活動開始前 1 小時提醒你'),
      },
    ...
    <Screen>
      ...
      >
        ...
        <AnimatedCard title={item.title} subtitle="">
                ...
                <StatusBadge
                  ...
                  label={
                    myRegistration.status === 'waitlist'
                      ? `候補第 ${myRegistration.waitlistPosition ?? '?'}`
                      : '已報名'
                  }
                />
              )}
          ...
          <Text
            style={{ color: theme.colors.text, lineHeight: 22, fontSize: 15, marginBottom: 16 }}
          >
            {item.description}
          </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              ...
              >
                ...
                <Text style={{ color: theme.colors.muted, fontSize: 11 }}>
                  {item.capacity ? `/ ${item.capacity}` : ''} 已報名
                </Text>
              </View>
              {waitlistCount > 0 && (
                <View
          ...
          {item.capacity && (
            <View style={{ marginBottom: 16 }}>
              ...
              >
                ...
                <Text
                  style={{
                    color: isFull ? theme.colors.danger : theme.colors.accent,
                    fontSize: 12,
                    fontWeight: '700',
                  }}
                >
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: '700' }}>
                    {myRegistration.status === 'waitlist'
                      ? `你在候補名單第 ${myRegistration.waitlistPosition ?? '?'} 位`
                      : '你已成功報名此活動'}
                  </Text>
            ...
            {myRegistration ? (
              <>
                {eventStatus !== 'ended' &&
                  ...
                  !myRegistration.checkedIn && (
                    <Button text="顯示簽到 QR Code" kind="primary" onPress={onShowQRCode} />
                  )}
                <Button
        ...
        {registeredCount > 0 && (
          <AnimatedCard
            title="報名名單"
            subtitle={`已報名 ${registeredCount} 人${waitlistCount > 0 ? ` · 候補 ${waitlistCount} 人` : ''}`}
            delay={300}
          >
            <View style={{ gap: 8 }}>
              {registrations
                ...
                .map((r, idx) => {
                  ...
                  const displayName =
                    profile?.displayName || r.displayName || `${r.uid.slice(0, 8)}…`;
                  return (
                    <View
                      key={r.uid}
                      style={{
                      ...
                      <View style={{ position: 'relative' }}>
                        {profile?.avatarUrl || r.avatarUrl ? (
                          <Avatar
                            name={displayName}
                            size={36}
                            imageUrl={profile?.avatarUrl || r.avatarUrl}
                          />
                        ) : (
                          <View
                            style={{
        ...
        {eventStatus === 'ended' && (
          <AnimatedCard
            ...
            subtitle={
              reviews.length > 0 ? `平均評分 ${averageRating.toFixed(1)} 分` : '分享你的參與心得'
            }
            delay={450}
          >
            {/* Rating Summary */}
            ...
            {auth.user && myRegistration && !myReview && (
              ...
              >
                <Text style={{ color: theme.colors.text, fontWeight: '600', marginBottom: 10 }}>
                  分享你的體驗
                </Text>
                <View
                  style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <Text style={{ color: theme.colors.muted, fontSize: 11 }}>
                          {formatRelativeTime(toDate(r.createdAt))}
                        </Text>
                      </View>
                      {r.comment && (
