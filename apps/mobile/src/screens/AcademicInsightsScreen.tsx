...
const riskColor = (level: RiskLevel) => {
  switch (level) {
    ...
    case "watch": return theme.colors.warning;
...
const riskLabel = (level: RiskLevel) => {
  switch (level) {
    case "safe": return "低風險";
    case "watch": return "中風險";
    case "warning": return "高風險";
    case "critical": return "警戒";
  }
};
...
const recTypeColor = (t: string) => {
  switch (t) {
    ...
    case "strategy": return theme.colors.accent;
    case "balance": return theme.colors.success;
    case "opportunity": return theme.colors.social ?? theme.colors.accent;
...
const recTypeLabel = (t: string) => {
  switch (t) {
    ...
    case "balance": return "平衡";
    case "opportunity": return "機會";
    default: return t;
  }
};
...
const perfIcon = (p: "above" | "at" | "below") => {
  switch (p) {
    case "above": return { name: "arrow-up", color: theme.colors.success };
    case "at": return { name: "remove", color: theme.colors.textSecondary };
    case "below": return { name: "arrow-down", color: theme.colors.danger };
  }
};
...
export function AcademicInsightsScreen() {
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: theme.colors.bg, justifyContent: "center", alignItems: "center" }}>
        ...
        <Text style={{ color: theme.colors.textSecondary, marginTop: theme.space.md }}>分析學業數據中...</Text>
      </View>
    );
  }
  ...
  const { gpaPrediction, courseDifficulty, riskAssessment, recommendations, semesterSummaries, academicProfile } = data;
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      >
        ...
        <View style={{ paddingHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
          <Text style={{ color: theme.colors.text, fontSize: 28, fontWeight: "800" }}>學業分析</Text>
        ...
        <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.md }}>
            <Text style={{ color: theme.colors.text, fontSize: 18, fontWeight: "700" }}>課程難度分析</Text>
            <Pressable
        ...
        {/* Semester History */}
        <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
          <Text style={{ color: theme.colors.text, fontSize: 18, fontWeight: "700", marginBottom: theme.space.md }}>學期歷史</Text>
        ...
        {/* Recommendations */}
        <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
          <Text style={{ color: theme.colors.text, fontSize: 18, fontWeight: "700", marginBottom: theme.space.md }}>學習建議</Text>
...
function GpaTrendChart({ prediction }: { prediction: GpaPrediction }) {
  ...
  const dirIcon = direction === "improving" ? "trending-up" : direction === "declining" ? "trending-down" : "remove";
  const dirColor = direction === "improving" ? theme.colors.success : direction === "declining" ? theme.colors.danger : theme.colors.warning;
  const dirLabel = direction === "improving" ? "上升趨勢" : direction === "declining" ? "下降趨勢" : "持平";
    ...
    <View style={{
      ...
      <View style={{
        ...
        <Text style={{ color: theme.colors.text, fontSize: 13, flex: 1 }}>
          預測下學期 GPA: <Text style={{ fontWeight: "700", color: theme.colors.accent }}>{predictedNext.toFixed(2)}</Text>
        </Text>
      </View>

      <Text style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: theme.space.sm, lineHeight: 18 }}>
        {analysis}
      </Text>
    </View>
  );
...
function RiskGauge({ assessment, animValue }: { assessment: RiskAssessment; animValue: Animated.Value }) {
  ...
  return (
    <View style={{
      ...
      <Text style={{ color: theme.colors.text, fontSize: 18, fontWeight: "700", marginBottom: theme.space.md }}>
        學業風險評估
      </Text>
      ...
      <Text style={{ color: theme.colors.text, fontSize: 15, fontWeight: "600", marginBottom: theme.space.sm }}>
        風險因子
      </Text>
      {factors.map((f, i) => (
        <View key={i} style={{ marginBottom: theme.space.md }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
            <Text style={{ color: theme.colors.text, fontSize: 13, fontWeight: "600" }}>{f.name}</Text>
          ...
          <Text style={{ color: theme.colors.textSecondary, fontSize: 11, marginTop: 2 }}>{f.detail}</Text>
        </View>
      ))}
...
function ProfileCard({ profile }: { profile: AcademicProfile }) {
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-around", marginBottom: theme.space.lg }}>
        {[
          ...
          <View key={s.label} style={{ alignItems: "center" }}>
            ...
            <Text style={{ color: theme.colors.text, fontSize: 20, fontWeight: "800", marginTop: 4 }}>{s.value}</Text>
            <Text style={{ color: theme.colors.textSecondary, fontSize: 11 }}>{s.label}</Text>
          </View>
        ))}
      </View>

      <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: "600", marginBottom: 6 }}>擅長領域</Text>
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: theme.space.md }}>
        {profile.strongCategories.length > 0 ? profile.strongCategories.map((c) => (
          <View key={c} style={{
            ...
            <Text style={{ color: theme.colors.success, fontSize: 12, fontWeight: "600" }}>{c}</Text>
          </View>
        )) : (
      ...
      <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: "600", marginBottom: 6 }}>需加強領域</Text>
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: theme.space.md }}>
        {profile.weakCategories.length > 0 ? profile.weakCategories.map((c) => (
          <View key={c} style={{
            ...
            <Text style={{ color: theme.colors.danger, fontSize: 12, fontWeight: "600" }}>{c}</Text>
          </View>
        )) : (
      ...
      <View style={{
        ...
        <Text style={{ color: theme.colors.text, fontSize: 13 }}>
          偏好難度: <Text style={{ fontWeight: "700", color: theme.colors.accent }}>{profile.preferredDifficulty}</Text>
        </Text>
      </View>
    </View>
  );
    ...
    <View style={{
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View style={{ flexDirection: "row", gap: 12 }}>
          ...
          <Text style={{ color: theme.colors.textSecondary, fontSize: 12 }}>
            預期 <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{course.expectedScore.toFixed(1)}</Text>
          </Text>
        </View>
...
function SemesterCard({
  ...
  const gpaColor =
    summary.gpa >= 3.7 ? theme.colors.success :
    summary.gpa >= 3.0 ? theme.colors.accent :
    summary.gpa >= 2.0 ? theme.colors.warning :
    theme.colors.danger;
    ...
    >
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          <Text style={{ color: theme.colors.text, fontSize: 15, fontWeight: "600" }}>{summary.semester}</Text>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <View style={{
            ...
            <Text style={{ color: gpaColor, fontSize: 14, fontWeight: "700" }}>GPA {summary.gpa.toFixed(2)}</Text>
          </View>
        ...
        <View style={{ marginTop: theme.space.md }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
            <Text style={{ color: theme.colors.textSecondary, fontSize: 13 }}>
              平均: <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{summary.avgScore.toFixed(1)}</Text>
            </Text>
            <Text style={{ color: theme.colors.textSecondary, fontSize: 13 }}>
              課程: <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{summary.courseCount}</Text>
            </Text>
          </View>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
            <Text style={{ color: theme.colors.textSecondary, fontSize: 13 }}>
              學分: <Text style={{ color: theme.colors.text, fontWeight: "600" }}>{summary.totalCredits}</Text>
            </Text>
            {summary.rankPercentile != null && (
              <Text style={{ color: theme.colors.textSecondary, fontSize: 13 }}>
                前 <Text style={{ color: theme.colors.accent, fontWeight: "600" }}>{summary.rankPercentile}%</Text>
              </Text>
            )}
          </View>
          <View style={{
  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: "700", flex: 1 }}>{rec.title}</Text>
        <View style={{
          backgroundColor: color + "20",
          borderRadius: theme.radius.sm,
          paddingHorizontal: 6,
      ...
      <Text style={{ color: theme.colors.textSecondary, fontSize: 13, lineHeight: 19 }}>{rec.description}</Text>
      {rec.relatedCourse && (
