...
import {
  // 健康中心
  HEALTH_CENTER, COUNSELING_CENTER,
  // 科別
  DEPARTMENTS, getDeptInfo, type HealthDepartment, type DepartmentInfo,
  // AI 症狀自評
  SYMPTOM_OPTIONS, triageSymptom, type SymptomOption, type TriageResult, type SymptomSeverity,
  // Mood Tracker
  MOOD_EMOJIS, MOOD_FACTORS, analyzeMoodTrend,
  type MoodLevel, type MoodEntry, type MoodTrend, type MoodFactor,
  // 健康護照
  AVAILABLE_VACCINES, type HealthPassport, type VaccinationRecord,
  // AED
  AED_LOCATIONS, EMERGENCY_NUMBERS, type AEDLocation,
  // 運動處方
  generateExercisePrescription, type FitnessProfile, type ExercisePrescription,
  // 角色
  ROLE_HEALTH_CONFIG, type HealthRole,
  // 互動
  HEALTH_ROLE_INTERACTIONS,
  // 同儕互助
  PEER_TOPICS, type PeerSupportPost, type PeerSupportTopic,
  // 季節預警
  getActiveAlerts, type SeasonalAlert,
  // 推薦
  getSmartHealthSuggestions, type HealthSuggestion,
  // 通知
  HEALTH_NOTIFICATION_TYPES,
  // 模擬
  simulateHealthStats, simulateMyHealth, simulatePeerPosts,
...
export function HealthScreen(props: any) {
  ...
  const loadData = useCallback(() => {
    ...
    const profile: FitnessProfile = {
      userId: "current-user", bmi: health.passport.bmi ?? 22, restingHR: 72,
      level: "beginner", weeklyExerciseMinutes: 60,
      goals: ["紓壓", "改善睡眠"], limitations: [],
    };
  ...
  const renderHome = () => (
    <>
            ...
            {suggestions.map((s, i) => (
              <Pressable
                ...
                style={{
                  ...
                  padding: 10, borderRadius: theme.radius.md, backgroundColor: `${s.color}10`,
                }}
              >
        ...
        <AnimatedCard title="健康預警" delay={50}>
          <View style={{ gap: 8 }}>
            {alerts.slice(0, 3).map((alert) => {
              const bgColor = alert.severity === "danger" ? "#EF444415" : alert.severity === "warning" ? "#F59E0B12" : `${alert.color}10`;
              return (
                <View key={alert.id} style={{
                  flexDirection: "row", alignItems: "center", gap: 10,
                  padding: 10, borderRadius: theme.radius.md, backgroundColor: bgColor,
                }}>
                  <Ionicons name={alert.icon as any} size={18} color={alert.color} />
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{alert.title}</Text>
      ...
      <AnimatedCard title="快速操作" delay={100}>
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
          {[
            ...
            { key: "passport", label: "健康護照", icon: "shield-checkmark-outline", color: "#059669" },
        ...
        <AnimatedCard title="本週心情" subtitle={`平均 ${moodTrend.weekAvg}/5`} delay={150}>
          <View style={{ gap: 8 }}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-around" }}>
              {myHealth.moodEntries.slice(0, 7).reverse().map((entry, i) => {
            ...
            <View style={{
              ...
              backgroundColor: moodTrend.alertLevel === "normal" ? "#10B98110" :
                moodTrend.alertLevel === "attention" ? "#F59E0B10" : "#EF444410",
            }}>
              <Ionicons
                name={moodTrend.trend === "improving" ? "trending-up" : moodTrend.trend === "declining" ? "trending-down" : "remove-outline"}
                size={18}
                color={moodTrend.trend === "improving" ? "#10B981" : moodTrend.trend === "declining" ? "#EF4444" : "#F59E0B"}
              />
              <Text style={{ color: theme.colors.text, fontSize: 12, flex: 1 }}>{moodTrend.suggestion}</Text>
            </View>
          </View>
        </AnimatedCard>
      )}
            ...
            return (
              <View key={apt.id} style={{
                ...
                <View style={{
                  ...
                  backgroundColor: `${deptInfo.color}15`, alignItems: "center", justifyContent: "center",
                }}>
                  <Ionicons name={deptInfo.icon as any} size={20} color={deptInfo.color} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{deptInfo.label}</Text>
                  <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{apt.date} {apt.time} · {apt.doctor}</Text>
                </View>
      ...
      <AnimatedCard title="服務時間" delay={250}>
        <View style={{ gap: 6 }}>
          {[
            { name: HEALTH_CENTER.name, hours: HEALTH_CENTER.hours[0].time, icon: "medical-outline", color: "#3B82F6" },
            { name: COUNSELING_CENTER.name, hours: COUNSELING_CENTER.hours[0].time, icon: "heart-outline", color: "#7C3AED" },
          ].map((svc) => (
            <View key={svc.name} style={{
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{svc.name}</Text>
      ...
      <AnimatedCard title="AI 症狀自評" subtitle="幫助你快速分流掛號">
        {!selectedSymptom ? (
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
              {SYMPTOM_OPTIONS.map((s) => (
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.text, fontSize: 12, fontWeight: "600" }}>{s.label}</Text>
                </Pressable>
              ))}
            </View>
          </View>
          ...
          <View style={{ gap: 10 }}>
            ...
            <View style={{
              flexDirection: "row", alignItems: "center", gap: 8,
              padding: 10, borderRadius: theme.radius.md, backgroundColor: theme.colors.accentSoft,
            }}>
              <Ionicons name={selectedSymptom.icon as any} size={18} color={theme.colors.accent} />
              <Text style={{ color: theme.colors.accent, fontWeight: "700", fontSize: 14 }}>{selectedSymptom.label}</Text>
              <Pressable onPress={() => { setSelectedSymptom(null); setTriageResult(null); }} style={{ marginLeft: "auto" }}>
            ...
            {triageResult && (
              <View style={{ gap: 8 }}>
                <View style={{
                  ...
                  backgroundColor: triageResult.severity === "emergency" ? "#EF444420" :
                    triageResult.severity === "severe" ? "#F59E0B20" :
                    triageResult.severity === "moderate" ? "#3B82F620" : "#10B98115",
                  borderWidth: 1,
                  borderColor: triageResult.severity === "emergency" ? "#EF444450" :
                    triageResult.severity === "severe" ? "#F59E0B50" :
                    triageResult.severity === "moderate" ? "#3B82F650" : "#10B98130",
                }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                    <Ionicons
                      name={triageResult.shouldCallEmergency ? "alert-circle" : "checkmark-circle"}
                      size={22}
                      color={triageResult.severity === "emergency" ? "#EF4444" :
                        triageResult.severity === "severe" ? "#F59E0B" : "#10B981"}
                    />
                ...
                {triageResult.selfCareAdvice && (
                  <View style={{ gap: 4 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 12 }}>居家照護建議：</Text>
                ...
                {triageResult.shouldCallEmergency ? (
                  ...
                  <Button text="前往掛號" kind="primary" onPress={() => Alert.alert("掛號", `將為您掛號 ${getDeptInfo(triageResult.recommendedDepartment).label}`)} />
                )}
              </View>
            )}
          </View>
          ...
          {DEPARTMENTS.filter(d => d.id !== "crisis").map((dept) => (
            ...
            >
              <View style={{
                ...
                backgroundColor: `${dept.color}15`, alignItems: "center", justifyContent: "center",
              }}>
                <Ionicons name={dept.icon as any} size={16} color={dept.color} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{dept.label}</Text>
                <Text style={{ color: theme.colors.muted, fontSize: 11 }} numberOfLines={1}>{dept.description}</Text>
              </View>
              {dept.requiresAppointment && (
                <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: "#F59E0B18" }}>
  ...
  const renderMental = () => (
    <>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-around" }}>
            {([1, 2, 3, 4, 5] as MoodLevel[]).map((level) => {
              ...
              return (
                <Pressable
                  key={level}
                  onPress={() => Alert.alert("記錄心情", `今天心情：${mood.label}\n\n（實際版本會展開選擇困擾因素）`)}
                  style={({ pressed }) => ({
        ...
        <AnimatedCard title="心情趨勢" subtitle="近 7 天" delay={50}>
          <View style={{ gap: 8 }}>
            <View style={{ flexDirection: "row", gap: 12 }}>
              <View style={{ flex: 1, padding: 10, borderRadius: 8, backgroundColor: theme.colors.surface2, alignItems: "center" }}>
                <Text style={{ color: theme.colors.text, fontWeight: "800", fontSize: 20 }}>{moodTrend.weekAvg}</Text>
                <Text style={{ color: theme.colors.muted, fontSize: 10 }}>本週平均</Text>
              </View>
              <View style={{ flex: 1, padding: 10, borderRadius: 8, backgroundColor: theme.colors.surface2, alignItems: "center" }}>
                <Text style={{ color: theme.colors.text, fontWeight: "800", fontSize: 20 }}>{moodTrend.monthAvg}</Text>
                <Text style={{ color: theme.colors.muted, fontSize: 10 }}>本月平均</Text>
              </View>
              <View style={{ flex: 1, padding: 10, borderRadius: 8, backgroundColor: theme.colors.surface2, alignItems: "center" }}>
                <Ionicons
                  name={moodTrend.trend === "improving" ? "trending-up" : moodTrend.trend === "declining" ? "trending-down" : "remove"}
                  size={22}
                  color={moodTrend.trend === "improving" ? "#10B981" : moodTrend.trend === "declining" ? "#EF4444" : "#F59E0B"}
                />
            ...
            <View style={{
              flexDirection: "row", alignItems: "center", gap: 8,
              padding: 10, borderRadius: 8, backgroundColor: `${MOOD_FACTORS.find(f => f.id === moodTrend.dominantFactor)?.id ? "#7C3AED" : "#3B82F6"}10`,
            }}>
              <Ionicons name="analytics-outline" size={16} color="#7C3AED" />
              <Text style={{ color: theme.colors.text, fontSize: 12, flex: 1 }}>
                主要影響因素：{MOOD_FACTORS.find(f => f.id === moodTrend.dominantFactor)?.label ?? ""}
              </Text>
            </View>
          </View>
        </AnimatedCard>
        ...
        <View style={{ gap: 8 }}>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
            {COUNSELING_CENTER.services.map((svc) => (
              <View key={svc} style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999, backgroundColor: "#7C3AED12" }}>
                <Text style={{ color: "#7C3AED", fontSize: 11, fontWeight: "600" }}>{svc}</Text>
              </View>
            ))}
          </View>
          <Button text="預約諮商" kind="primary" onPress={() => Alert.alert("預約諮商", "將為您開啟線上預約系統")} />
        </View>
      </AnimatedCard>
      ...
      <AnimatedCard title="匿名互助圈" subtitle="你不孤單" delay={150}>
        <View style={{ gap: 8 }}>
            ...
            return (
              <View key={post.id} style={{
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                  <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: `${topicInfo?.color ?? "#999"}15` }}>
                    <Text style={{ color: topicInfo?.color ?? "#999", fontSize: 9, fontWeight: "600" }}>{topicInfo?.label}</Text>
                  </View>
                  <Text style={{ color: theme.colors.muted, fontSize: 10 }}>{post.authorAlias}</Text>
                </View>
                <Text style={{ color: theme.colors.text, fontSize: 12, marginTop: 4, lineHeight: 18 }} numberOfLines={2}>
                  {post.content}
                </Text>
                <View style={{ flexDirection: "row", gap: 12, marginTop: 6 }}>
                  {post.reactions.map((r) => (
                    <View key={r.type} style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
                      <Text style={{ fontSize: 12 }}>{r.type === "hug" ? "🤗" : r.type === "same" ? "💭" : "💪"}</Text>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 3, marginLeft: "auto" }}>
                    ...
                    <Text style={{ color: theme.colors.muted, fontSize: 10 }}>{post.replyCount}</Text>
                  </View>
                </View>
              </View>
            );
          ...
          <Pressable style={{ alignItems: "center", padding: 8 }}>
            <Text style={{ color: theme.colors.accent, fontSize: 12, fontWeight: "600" }}>查看更多 / 發布心情 →</Text>
          </Pressable>
        </View>
      </AnimatedCard>
      ...
      <AnimatedCard title="24h 求助資源" delay={200}>
        <View style={{ gap: 6 }}>
          {EMERGENCY_NUMBERS.filter(n => ["安心專線", "生命線", "張老師專線"].includes(n.name)).map((num) => (
            <Pressable
              key={num.name}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{num.name}</Text>
                <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{num.available}</Text>
              </View>
              <Text style={{ color: theme.colors.accent, fontWeight: "700", fontSize: 14 }}>{num.phone}</Text>
            </Pressable>
          ))}
        </View>
      </AnimatedCard>
      ...
      <>
        ...
        <AnimatedCard title="我的健康護照" subtitle="一碼通行">
          <View style={{ gap: 8 }}>
            <View style={{ flexDirection: "row", gap: 8 }}>
              {[
                { label: "血型", value: passport.bloodType ?? "未登錄", icon: "water", color: "#EF4444" },
                { label: "BMI", value: passport.bmi?.toString() ?? "--", icon: "body", color: "#3B82F6" },
                { label: "視力(左)", value: passport.visionLeft?.toString() ?? "--", icon: "eye", color: "#8B5CF6" },
                { label: "視力(右)", value: passport.visionRight?.toString() ?? "--", icon: "eye", color: "#8B5CF6" },
              ].map((info) => (
                <View key={info.label} style={{
                  flex: 1, padding: 10, borderRadius: 8, backgroundColor: theme.colors.surface2, alignItems: "center",
                }}>
                  <Ionicons name={info.icon as any} size={16} color={info.color} />
                  <Text style={{ color: theme.colors.text, fontWeight: "700", fontSize: 14, marginTop: 2 }}>{info.value}</Text>
            ...
            {passport.allergies && passport.allergies.length > 0 && (
              <View style={{ padding: 10, borderRadius: 8, backgroundColor: "#EF444412", borderWidth: 1, borderColor: "#EF444430" }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                ...
                {passport.allergies.map((a, i) => (
                  <Text key={i} style={{ color: theme.colors.text, fontSize: 12, marginTop: 4 }}>
                    {a.name}（{a.severity === "severe" ? "嚴重" : a.severity === "moderate" ? "中度" : "輕微"}）
                  </Text>
                ))}
              </View>
            )}
            ...
            {passport.emergencyContact && (
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8, padding: 10, borderRadius: 8, backgroundColor: theme.colors.surface2 }}>
                <Ionicons name="call-outline" size={16} color={theme.colors.accent} />
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 12 }}>
                    緊急聯絡人：{passport.emergencyContact.name}（{passport.emergencyContact.relation}）
                  </Text>
                  <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{passport.emergencyContact.phone}</Text>
                </View>
              </View>
            )}
          </View>
        ...
        <AnimatedCard title="疫苗接種紀錄" delay={50}>
          {passport.vaccinations && passport.vaccinations.length > 0 ? (
            <View style={{ gap: 6 }}>
              {passport.vaccinations.map((vac) => (
                <View key={vac.id} style={{
                  ...
                  <View style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "#05966918", alignItems: "center", justifyContent: "center" }}>
                    <Ionicons name="shield-checkmark" size={16} color="#059669" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 12 }}>{vac.vaccine}</Text>
                    <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{vac.date} · 第 {vac.dose}/{vac.totalDoses} 劑</Text>
                  </View>
                  {vac.certificate && (
                    <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: "#10B98118" }}>
                      <Text style={{ color: "#10B981", fontSize: 9, fontWeight: "600" }}>已認證</Text>
                    </View>
                  )}
                </View>
              ))}
            ...
            <EmptyState title="無接種紀錄" subtitle="可到衛保組登錄歷史接種" icon="shield-outline" />
          )}
        </AnimatedCard>
          ...
          <View style={{ gap: 6 }}>
            {AVAILABLE_VACCINES.slice(0, 4).map((vac) => (
              <View key={vac.id} style={{
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 12 }}>{vac.name}</Text>
        ...
        <AnimatedCard title="健康檢查" delay={150}>
          <View style={{
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: theme.colors.muted, fontSize: 11 }}>建議每學年進行一次全面健檢</Text>
            </View>
          </View>
        </AnimatedCard>
      </>
  ...
  const renderExercise = () => (
    <>
          ...
          <View style={{ gap: 8 }}>
            {exerciseRx.map((rx) => (
              <View key={rx.id} style={{
                padding: 12, borderRadius: theme.radius.lg, backgroundColor: theme.colors.surface2,
              }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                  <View style={{
                    width: 38, height: 38, borderRadius: 12,
                    backgroundColor: `${rx.color}15`, alignItems: "center", justifyContent: "center",
                  }}>
                    <Ionicons name={rx.icon as any} size={18} color={rx.color} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: "700", fontSize: 14 }}>{rx.name}</Text>
                    <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{rx.description}</Text>
                  </View>
                </View>
                <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                  {[
                    ...
                    <View key={i} style={{
                      ...
                      paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4, backgroundColor: theme.colors.bg,
                    }}>
      ...
      <AnimatedCard title="校園運動設施" delay={50}>
        <View style={{ gap: 6 }}>
          {[
            { name: "體育館重訓室", hours: "07:00-21:00", icon: "barbell-outline", color: "#F59E0B" },
            ...
            { name: "游泳池", hours: "週一三五 12:00-20:00", icon: "water-outline", color: "#0D9488" },
            ...
            <View key={facility.name} style={{
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 13 }}>{facility.name}</Text>
      ...
      <AnimatedCard title="運動小知識" delay={100}>
        <View style={{ gap: 6 }}>
          {[
            ...
            <View key={i} style={{ flexDirection: "row", gap: 8, padding: 8, borderRadius: 6, backgroundColor: theme.colors.surface2 }}>
  ...
  const renderEmergency = () => (
    <>
          ...
          {EMERGENCY_NUMBERS.map((num) => (
            ...
            >
              <View style={{
                ...
                backgroundColor: `${num.color}15`, alignItems: "center", justifyContent: "center",
              }}>
                <Ionicons name={num.icon as any} size={18} color={num.color} />
              </View>
              <View style={{ flex: 1 }}>
      ...
      {/* AED 位置 */}
      <AnimatedCard title="AED 自動體外電擊器" subtitle={`校內 ${AED_LOCATIONS.length} 台`} delay={50}>
        <View style={{ gap: 6 }}>
          {AED_LOCATIONS.map((aed) => (
            <View key={aed.id} style={{
              flexDirection: "row", alignItems: "center", gap: 10,
              padding: 10, borderRadius: theme.radius.md, backgroundColor: theme.colors.surface2,
            }}>
              <View style={{
                ...
                <Ionicons name="pulse" size={16} color={aed.status === "available" ? "#10B981" : "#EF4444"} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontWeight: "600", fontSize: 12 }}>{aed.name}</Text>
      ...
      <AnimatedCard title="急救步驟" subtitle="黃金 4 分鐘" delay={100}>
        <View style={{ gap: 6 }}>
          {[
            ...
            { step: "4", text: "胸外按壓 30 下（5-6cm 深）", icon: "hand-left-outline", color: "#DC2626" },
            { step: "5", text: "人工呼吸 2 次（30:2 循環）", icon: "pulse-outline", color: "#7C3AED" },
    ...
    <Screen>
      ...
      >
        ...
        <SegmentedControl
          options={TABS}
          selected={tab}
          onChange={(k) => setTab(k as HealthTab)}
        />
