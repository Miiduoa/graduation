...
function generateMockCrowdInfo(category: string): CrowdInfo {
  ...
  const peakHours =
    category === '餐廳' ? ['11:30-12:30', '17:30-18:30'] : ['10:00-12:00', '14:00-16:00'];
...
function deriveCrowdInfoFromReports(reports: PoiCrowdReport[]): CrowdInfo | null {
  ...
  const recentWindow = withDate.filter(
    (report) => now - report.createdDate.getTime() <= 2 * 60 * 60 * 1000,
  );
  const effectiveReports = recentWindow.length > 0 ? recentWindow : withDate.slice(0, 12);
  const averageScore =
    effectiveReports.reduce((sum, report) => sum + CROWD_LEVEL_SCORE[report.level], 0) /
    effectiveReports.length;
  ...
  const olderSlice = effectiveReports.slice(
    Math.min(3, effectiveReports.length),
    Math.min(6, effectiveReports.length),
  );
  const newestAverage =
    newestSlice.reduce((sum, report) => sum + CROWD_LEVEL_SCORE[report.level], 0) /
    newestSlice.length;
  const olderAverage =
    olderSlice.length > 0
      ? olderSlice.reduce((sum, report) => sum + CROWD_LEVEL_SCORE[report.level], 0) /
        olderSlice.length
      : newestAverage;
...
export function PoiDetailScreen(props: any) {
  ...
  const {
    ...
    error: loadError,
    reload,
  } = useAsyncList<any>(() => ds.listPois(school.id), [ds, school.id]);
  ...
  const accessibilityInfo: AccessibilityInfo = useMemo(
    () => ({
      hasElevator:
        item?.category === '圖書館' || item?.category === '行政大樓' || Math.random() > 0.3,
  ...
  return (
    <Screen>
      ...
      >
            ...
            {Object.entries(operatingHours).map(([day, hours]) => {
              ...
              return (
                ...
                >
                  ...
                  <Text
                    style={{ color: hours === '休息' ? theme.colors.danger : theme.colors.muted }}
                  >
                    {hours}
                  </Text>
                </View>
            ...
            {[
              {
                ...
                icon: 'arrow-up',
                label: '電梯',
                available: accessibilityInfo.hasElevator,
              },
              {
                key: 'hasRamp',
                icon: 'trending-up',
                label: '無障礙坡道',
                available: accessibilityInfo.hasRamp,
              },
              {
                key: 'hasAccessibleRestroom',
                icon: 'body',
                label: '無障礙廁所',
                available: accessibilityInfo.hasAccessibleRestroom,
              },
              {
                key: 'hasAccessibleParking',
                icon: 'car',
                label: '無障礙停車位',
                available: accessibilityInfo.hasAccessibleParking,
              },
              {
                key: 'hasBrailleSigns',
                icon: 'eye-off',
                label: '點字標示',
                available: accessibilityInfo.hasBrailleSigns,
              },
            ].map((item) => (
          ...
          >
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 12 }}>
              <View style={{ alignItems: 'center' }}>
                <Ionicons
                  name={
                    crowdInfo.trend === 'increasing'
                      ? 'trending-up'
                      : crowdInfo.trend === 'decreasing'
                        ? 'trending-down'
                        : 'remove'
                  }
                  size={24}
                ...
                >
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={{ color: theme.colors.accent, fontWeight: '700' }}>
                      {facility.distance < 100
                        ? `${Math.round(facility.distance)}m`
                        : `${(facility.distance / 1000).toFixed(1)}km`}
                    </Text>
                    <Pressable
                      onPress={() => openInMaps(facility.lat, facility.lng, facility.name)}
                    >
        ...
        {/* Reviews Section */}
        <AnimatedCard
          title={`評價 (${reviews.length})`}
          subtitle="幫助其他同學了解這個地點"
          delay={400}
        >
          {/* Write Review */}
          {!showReviewForm ? (
        ...
        <AnimatedCard title="回報問題" subtitle="幫助我們改善資訊" delay={450}>
          {!showReportForm ? (
            ...
            <View style={{ gap: 12 }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {(['closed', 'wrong_info', 'accessibility', 'safety', 'other'] as const).map(
                  (type) => (
                    <Pressable
                      key={type}
                      ...
                      style={{
                        ...
                        borderRadius: theme.radius.sm,
                        backgroundColor:
                          reportType === type ? theme.colors.accent : theme.colors.surface2,
                        borderWidth: 1,
                        borderColor:
                          reportType === type ? theme.colors.accent : theme.colors.border,
                      }}
                    >
                      <Text
                        style={{
                          color: reportType === type ? '#fff' : theme.colors.text,
                          fontSize: 12,
                        }}
                      >
