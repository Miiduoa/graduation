...
export function GlobalSearchScreen(props: any) {
      ...
      try {
        ...
        await Promise.all(
          groupIds.slice(0, 10).map(async (gid) => {
            ...
            postsSnap.docs.forEach((d) => posts.push({ id: d.id, groupId: gid, ...d.data() }));
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    if (category === 'all' || category === 'groups') {
      groupPosts.forEach((p) => {
        const text =
          `${p.title ?? ''} ${p.body ?? ''} ${p.authorName ?? ''} ${p.authorId ?? ''}`.toLowerCase();
        if (text.includes(q)) {
          allResults.push({
            id: p.id,
            type: 'post',
  ...
  const showHistory =
    isFocused && !queryText.trim() && (historyItems.length > 0 || popularItems.length > 0);
          ...
          >
            {historyItems.length > 0 && (
              <AnimatedCard title="搜尋紀錄" subtitle="">
                ...
                >
                  <Text
                    style={{ color: theme.colors.textSecondary, fontSize: 12, fontWeight: '500' }}
                  >
                    最近搜尋
                  </Text>
                  <Pressable
