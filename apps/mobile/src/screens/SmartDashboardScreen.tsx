...
function GreetingHeader({
  ...
  if (hour < 6) { greeting = "夜深了"; greetingIcon = "moon-outline"; }
  else if (hour < 12) { greeting = "早安"; greetingIcon = "sunny-outline"; }
  else if (hour < 18) { greeting = "午安"; greetingIcon = "partly-sunny-outline"; }
  else { greeting = "晚安"; greetingIcon = "moon-outline"; }
  ...
  return (
    <View style={{ paddingHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      ...
      <Text style={{ color: theme.colors.text, fontSize: 28, fontWeight: "800", marginTop: theme.space.xs, letterSpacing: -0.5 }}>
        {displayName}
      </Text>
      {gamification && (
        <View style={{ flexDirection: "row", alignItems: "center", gap: theme.space.md, marginTop: theme.space.sm }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: theme.colors.achievementSoft, paddingHorizontal: 10, paddingVertical: 4, borderRadius: theme.radius.full }}>
            <Ionicons name={gamification.levelInfo.icon as any} size={14} color={gamification.levelInfo.color} />
          ...
          {streak > 0 && (
            <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: theme.colors.streakSoft, paddingHorizontal: 10, paddingVertical: 4, borderRadius: theme.radius.full }}>
...
function XPProgressBar({ gamification }: { gamification: GamificationState }) {
  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: theme.space.sm }}>
      ...
      <View style={{ height: 8, backgroundColor: theme.colors.surface2, borderRadius: theme.radius.full, overflow: "hidden" }}>
        <Animated.View
          style={{
            height: "100%",
...
/** GPA 趨勢迷你圖 */
function GpaTrendChart({ trends, prediction }: {
  trends: GpaTrend[];
  ...
  const trendColor =
    prediction.trend === "improving" ? theme.colors.success :
    prediction.trend === "declining" ? theme.colors.danger :
    theme.colors.accent;
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.md }}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          <View style={{
            ...
            backgroundColor: prediction.trend === "improving" ? theme.colors.successSoft :
              prediction.trend === "declining" ? theme.colors.dangerSoft : theme.colors.infoSoft,
            paddingHorizontal: 8, paddingVertical: 3, borderRadius: theme.radius.full,
          }}>
            <Ionicons
              name={prediction.trend === "improving" ? "trending-up" : prediction.trend === "declining" ? "trending-down" : "remove-outline" as any}
              size={14}
              color={trendColor}
            />
            <Text style={{ color: trendColor, fontSize: 11, fontWeight: "700" }}>
              {prediction.trend === "improving" ? "上升中" : prediction.trend === "declining" ? "下降中" : "穩定"}
            </Text>
          </View>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: theme.space.xs }}>
        {trends.length <= 6 ? trends.map((t) => (
          ...
          <>
            ...
            <Text style={{ color: theme.colors.muted, fontSize: 10 }}>{trends[trends.length - 1]?.semester}</Text>
          </>
        )}
      </View>
    </LinearGradient>
...
/** 學業風險卡 */
function RiskAssessmentCard({ level, score, factors }: {
  level: RiskLevel;
  score: number;
  ...
  const config: Record<RiskLevel, { color: string; bg: string; label: string; icon: string }> = {
    safe: { color: theme.colors.success, bg: theme.colors.successSoft, label: "狀態良好", icon: "shield-checkmark" },
    watch: { color: theme.colors.calm, bg: theme.colors.calmSoft, label: "注意觀察", icon: "eye-outline" },
    warning: { color: theme.colors.warning, bg: theme.colors.warningSoft, label: "需要改善", icon: "warning-outline" },
    critical: { color: theme.colors.danger, bg: theme.colors.dangerSoft, label: "嚴重警告", icon: "alert-circle" },
  };
      ...
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        <View>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 4 }}>
            <View style={{ backgroundColor: c.bg, paddingHorizontal: 10, paddingVertical: 4, borderRadius: theme.radius.full, flexDirection: "row", alignItems: "center", gap: 4 }}>
        ...
        <View style={{ width: 56, height: 56, justifyContent: "center", alignItems: "center" }}>
          <View style={{
            ...
            borderWidth: 4, borderColor: theme.colors.surface2,
      ...
      {factors.length > 0 && (
        <View style={{ marginTop: theme.space.md }}>
          {factors.slice(0, 3).map((f, i) => (
            <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: i > 0 ? 6 : 0 }}>
              <Ionicons name={f.icon as any} size={14} color={theme.colors.textSecondary} />
              <Text style={{ color: theme.colors.textSecondary, fontSize: 12, flex: 1 }} numberOfLines={1}>
                {f.description}
              </Text>
              <View style={{ width: 40, height: 4, backgroundColor: theme.colors.surface2, borderRadius: 2, overflow: "hidden" }}>
                <View style={{ width: `${f.severity * 100}%`, height: "100%", backgroundColor: f.severity > 0.6 ? theme.colors.danger : f.severity > 0.3 ? theme.colors.warning : theme.colors.success, borderRadius: 2 }} />
              </View>
            </View>
          ))}
        </View>
...
/** 智慧建議卡片 */
function RecommendationCards({ recommendations, onPress }: {
  ...
  return (
    <View style={{ marginBottom: theme.space.lg }}>
      <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "700", paddingHorizontal: theme.space.lg, marginBottom: theme.space.md }}>
        智慧建議
      </Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: theme.space.lg, gap: theme.space.sm }}>
          ...
          return (
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: config.bg, justifyContent: "center", alignItems: "center" }}>
              ...
              <Text style={{ color: theme.colors.textSecondary, fontSize: 12, lineHeight: 18 }} numberOfLines={3}>
                {rec.description}
              </Text>
...
/** 校園脈動迷你卡 */
function CampusPulseMini({ pulse, onReportPress }: {
    ...
    <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.md }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "700" }}>校園脈動</Text>
        ...
        <Pressable onPress={onReportPress} style={({ pressed }) => ({
          flexDirection: "row", alignItems: "center", gap: 4,
          backgroundColor: theme.colors.accentSoft, paddingHorizontal: 10, paddingVertical: 5,
          borderRadius: theme.radius.full, opacity: pressed ? 0.7 : 1,
        })}>
        ...
        {topLocations.map((loc) => (
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <Text style={{ color: theme.colors.text, fontSize: 12, fontWeight: "600", flex: 1 }} numberOfLines={1}>
                {loc.name}
              </Text>
            </View>
...
function GraduationProgressCard({ optimization }: { optimization: ScheduleOptimization | null }) {
  ...
  return (
    <View style={{
      ...
      <View style={{ height: 10, backgroundColor: theme.colors.surface2, borderRadius: theme.radius.full, overflow: "hidden" }}>
        <View style={{
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: theme.space.sm }}>
        {gaps.filter((g) => g.remaining > 0).slice(0, 4).map((gap) => {
          const urgencyColor =
            gap.urgency === "critical" ? theme.colors.danger :
            gap.urgency === "high" ? theme.colors.warning :
            gap.urgency === "medium" ? theme.colors.calm :
            theme.colors.success;
          return (
            <View key={gap.category} style={{
...
/** 成就速覽 */
function AchievementPreview({ gamification, onPress }: {
        ...
        {recentUnlocked.map((ach) => (
          <View key={ach.id} style={{ alignItems: "center", gap: 4 }}>
            <View style={{
              ...
              backgroundColor: theme.colors.surface2,
              justifyContent: "center", alignItems: "center",
              borderWidth: 2, borderColor: rarityColor[ach.rarity] ?? theme.colors.muted,
            }}>
              <Ionicons name={ach.icon as any} size={20} color={rarityColor[ach.rarity] ?? theme.colors.muted} />
            </View>
            <Text style={{ color: theme.colors.textSecondary, fontSize: 9, textAlign: "center" }} numberOfLines={1}>
              {ach.title}
            </Text>
          </View>
        ))}
      ...
      {challenges.map((ch) => {
        ...
        return (
          <View key={ch.id} style={{
            ...
            padding: theme.space.sm, marginBottom: 6,
            borderWidth: 1, borderColor: theme.colors.border,
          }}>
            <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: theme.colors.accentSoft, justifyContent: "center", alignItems: "center" }}>
              <Ionicons name={ch.icon as any} size={18} color={theme.colors.accent} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ color: theme.colors.text, fontSize: 13, fontWeight: "600" }}>{ch.title}</Text>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 3 }}>
                <View style={{ flex: 1, height: 4, backgroundColor: theme.colors.surface2, borderRadius: 2, overflow: "hidden" }}>
                  <View style={{ width: `${Math.min(progress * 100, 100)}%`, height: "100%", backgroundColor: ch.completed ? theme.colors.success : theme.colors.accent, borderRadius: 2 }} />
                </View>
                <Text style={{ color: theme.colors.muted, fontSize: 10 }}>{ch.current}/{ch.target}</Text>
              </View>
            </View>
            <Text style={{ color: theme.colors.achievement, fontSize: 11, fontWeight: "700" }}>+{ch.xpReward} XP</Text>
          </View>
        );
      })}
    </View>
  ...
  return (
    <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: theme.space.md }}>
        <View>
          <Text style={{ color: theme.colors.text, fontSize: 17, fontWeight: "800" }}>行動中樞</Text>
          <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 2 }}>
            Today 只保留最該做的下一步
          </Text>
        </View>
        {props.risk ? (
          <View style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: theme.radius.full, backgroundColor: `${urgencyColor(props.risk.level === "critical" ? "critical" : props.risk.level === "warning" ? "high" : "medium")}18` }}>
            <Text style={{ color: urgencyColor(props.risk.level === "critical" ? "critical" : props.risk.level === "warning" ? "high" : "medium"), fontSize: 11, fontWeight: "800" }}>
              風險 {props.risk.score}
            </Text>
          </View>
        ) : null}
      ...
      {primary ? (
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "flex-start", gap: theme.space.md }}>
            <LinearGradient
              ...
              style={{ width: 42, height: 42, borderRadius: 16, alignItems: "center", justifyContent: "center" }}
            >
              <Ionicons name={primary.requiresConfirmation ? "shield-checkmark-outline" : "flash-outline"} size={22} color={color} />
            </LinearGradient>
            <View style={{ flex: 1 }}>
              <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "800" }}>{primary.title}</Text>
              <Text style={{ color: theme.colors.textSecondary, fontSize: 13, lineHeight: 20, marginTop: 4 }}>
                {primary.description}
              </Text>
        ...
        >
          <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "800" }}>今天沒有高壓待辦</Text>
          <Text style={{ color: theme.colors.textSecondary, fontSize: 13, lineHeight: 20, marginTop: 4 }}>
      ...
      {secondary.length > 0 ? (
        <View style={{ marginTop: theme.space.sm, gap: 8 }}>
          {secondary.map((action) => (
            ...
            >
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: urgencyColor(action.urgency) }} />
              <View style={{ flex: 1 }}>
      ...
      >
        <LinearGradient
          ...
          style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, borderRadius: theme.radius.md }}
        />
        <LinearGradient
...
function CloudPulseStrip(props: {
  ...
  return (
    <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: theme.space.sm }}>
        <View>
          <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "800" }}>校園脈動資料層</Text>
          <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 2 }}>匿名匯總，不儲存個人軌跡</Text>
        </View>
        <Pressable onPress={props.onReportPress} style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: theme.radius.full, backgroundColor: theme.colors.accentSoft }}>
        ...
        {sorted.map((aggregate) => {
          ...
          return (
            ...
            >
              ...
              <Text style={{ color: theme.colors.muted, fontSize: 10, marginTop: 2 }} numberOfLines={1}>
                {aggregate.bestTimeToVisit ?? "持續收集中"} · {Math.round(aggregate.confidence * 100)}%
              </Text>
            </View>
          );
        })}
...
function averagePulseConfidence(aggregates: PulseAggregate[]) {
  ...
  return Math.round(
    (aggregates.reduce((sum, aggregate) => sum + aggregate.confidence, 0) / aggregates.length) * 100
  );
}
    ...
    >
      {AGENT_MODES.map((mode) => {
        ...
        return (
          <Pressable
            key={mode.key}
            onPress={() => { props.onSelect(mode.key); props.onStart(mode.key); }}
            style={({ pressed }) => ({
              flexDirection: "row",
              alignItems: "center",
              gap: 6,
          ...
          >
            ...
            <Text style={{ color: active ? mode.color : theme.colors.textSecondary, fontSize: 13, fontWeight: "700" }}>
              {mode.label}
            </Text>
          </Pressable>
        );
...
function AgentOSHero(props: {
  ...
  if (hour < 6) { greeting = "夜深了"; greetIcon = "moon-outline"; }
  else if (hour < 12) { greeting = "早安"; greetIcon = "sunny-outline"; }
  else if (hour < 18) { greeting = "午安"; greetIcon = "partly-sunny-outline"; }
  else { greeting = "晚安"; greetIcon = "moon-outline"; }
    ...
    <View style={{ paddingHorizontal: theme.space.lg, marginBottom: theme.space.md }}>
      ...
      {props.gamification && (
        <View style={{ flexDirection: "row", alignItems: "center", gap: 10, marginTop: 10 }}>
          <LinearGradient
            colors={[`${props.gamification.levelInfo.color}20`, `${props.gamification.levelInfo.color}08`] as [string, string]}
            style={{ flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20 }}
          >
            <Ionicons name={props.gamification.levelInfo.icon as any} size={13} color={props.gamification.levelInfo.color} />
            <Text style={{ color: props.gamification.levelInfo.color, fontSize: 12, fontWeight: "700" }}>
      ...
      >
        ...
        >
          {/* Decorative circle */}
          <View style={{ position: "absolute", right: -20, top: -20, width: 100, height: 100, borderRadius: 50, backgroundColor: `${heroColor}12` }} />

          <Text style={{ color: "#FFFFFF", fontSize: 20, fontWeight: "800", lineHeight: 28 }} numberOfLines={2}>
            {primary ? primary.title : "校園代理已就緒"}
          </Text>
          <Text style={{ color: "#A8B7CC", fontSize: 13, lineHeight: 20, marginTop: 6 }} numberOfLines={2}>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}>
            <LinearGradient
              ...
              style={{ flex: 1, height: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 8 }}
            >
            ...
            <Pressable
              onPress={props.onOpenAI}
              style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: "#0E1B2D", borderWidth: 1, borderColor: "#253A58", alignItems: "center", justifyContent: "center" }}
            >
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
        ...
        <Text style={{ color: theme.colors.text, fontSize: 11, fontWeight: "900" }} numberOfLines={1}>
          {props.title}
        </Text>
      </View>
...
function CampusTwinPanel(props: {
  ...
  const courseRefs = props.actions.flatMap((action) => action.evidenceRefs).filter((ref) => ref.type === "course").length;
  ...
  return (
    <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <View>
          <Text style={{ color: theme.colors.text, fontSize: 17, fontWeight: "900" }}>Campus Twin</Text>
          <Text style={{ color: theme.colors.muted, fontSize: 12, marginTop: 2 }}>
            課程、地點、風險與動作即時關聯
          </Text>
        </View>
        ...
        >
          ...
          />
          <Text style={{ color: theme.colors.success, fontSize: 12, fontWeight: "900" }}>Live Pulse</Text>
        </Pressable>
      </View>

      <LinearGradient
        colors={isDarkTwin
          ? [`${theme.colors.accent}12`, `${theme.colors.surface}`, theme.colors.surface] as [string, string, string]
          : [theme.colors.surface, theme.colors.surface, theme.colors.surface] as [string, string, string]
        }
        style={{
          height: 268,
          borderRadius: theme.radius.lg,
      ...
      >
        <View style={{ position: "absolute", left: centerX + 31, top: 102, width: 38, height: 38, borderRadius: 19, backgroundColor: `${theme.colors.accent}15` }} />
        <GraphLine x1={72} y1={64} x2={centerX + 50} y2={122} color={`${theme.colors.accent}55`} />
        <GraphLine x1={rightX + 50} y1={66} x2={centerX + 50} y2={122} color={`${theme.colors.success}55`} />
        <GraphLine x1={78} y1={198} x2={centerX + 50} y2={122} color={`${theme.colors.warning}55`} />
        <GraphLine x1={rightX + 48} y1={202} x2={centerX + 50} y2={122} color={`${theme.colors.social}55`} />

        <TwinNode x={14} y={26} icon="school-outline" title="課程" value={`${courseRefs || actionCount} 個關聯`} color={theme.colors.info} />
        <TwinNode x={rightX} y={28} icon="location-outline" title="地點" value={topPulse?.locationName ?? "待同步"} color={theme.colors.success} />
        <TwinNode x={centerX} y={100} icon="sparkles-outline" title="AI Agent" value={`${actionCount} 個下一步`} color={theme.colors.accent} />
        <TwinNode x={16} y={174} icon="analytics-outline" title="風險" value={props.risk ? `${props.risk.score} 分` : "計算中"} color={theme.colors.warning} />
        <TwinNode x={rightX} y={176} icon="lock-closed-outline" title="確認閘" value={`${props.actions.filter((a) => a.requiresConfirmation).length} 個動作`} color={theme.colors.social} />
        ...
        >
          ...
          <Text style={{ flex: 1, color: theme.colors.textSecondary, fontSize: 11, lineHeight: 16 }}>
  ...
  const missions = [
    topAction
      ? {
          ...
          icon: topAction.requiresConfirmation ? "shield-checkmark-outline" as const : "flash-outline" as const,
    ...
    },
    {
      ...
      detail: props.risk ? `${props.risk.recommendedActions.length} 個可執行建議` : "讀取作業、出席與成績趨勢",
...
function DataMoatPanel(props: {
  ...
  const evidenceCount = new Set(props.actions.flatMap((action) => action.evidenceRefs.map((ref) => `${ref.type}:${ref.id}`))).size;
  const rows = [
    { label: "授權校務資料", value: Math.min(100, 42 + evidenceCount * 9), color: theme.colors.info },
    ...
    { label: "風險推理", value: props.risk ? Math.min(96, 52 + props.risk.signals.length * 14) : 36, color: theme.colors.social },
  ];
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "900" }}>資料閉環狀態</Text>
      </View>
      <View style={{ gap: 12 }}>
        {rows.map((row) => (
          <View key={row.label}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
              <Text style={{ color: theme.colors.textSecondary, fontSize: 12, fontWeight: "700" }}>{row.label}</Text>
              <Text style={{ color: row.color, fontSize: 12, fontWeight: "900" }}>{row.value}%</Text>
            </View>
...
export function SmartDashboardScreen(props: any) {
  ...
  const loadData = useCallback(async () => {
    try {
      const [insightsData, pulseData, gamData, optData, actionData, riskData, aggregateData] = await Promise.allSettled([
        getFullAcademicInsights(),
        getCampusPulseSnapshot(),
        getGamificationState(displayName, auth.profile?.department ?? ""),
        getScheduleOptimization(),
        auth.user ? ds.listNextBestActions(auth.user.uid, school.id) : Promise.resolve([] as NextBestAction[]),
        auth.user ? ds.listRiskSnapshots(auth.user.uid, school.id) : Promise.resolve([] as StudentRiskSnapshot[]),
  ...
  const openAgentAction = useCallback((action: NextBestAction) => {
  ...
  const startAgentMode = useCallback((modeKey: AgentModeKey) => {
      ...
      >
        ...
        {hasTronClass && tcAttendance.length > 0 && (
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center", gap: theme.space.sm, marginBottom: theme.space.md }}>
              ...
              <Text style={{ color: theme.colors.text, fontSize: 15, fontWeight: "700", flex: 1 }}>出席概況</Text>
              ...
              <View style={{ alignItems: "center" }}>
                <Text style={{ color: theme.colors.success, fontSize: 22, fontWeight: "800" }}>
                  {tcAttendance.length > 0
                    ? Math.round(tcAttendance.reduce((sum, a) => sum + a.rate, 0) / tcAttendance.length)
                    : 0}%
                </Text>
            ...
            {tcAttendance.filter(a => a.rate < 70).length > 0 && (
              <View style={{
                ...
                backgroundColor: theme.colors.danger + "15", borderRadius: theme.radius.md,
        ...
        <View style={{ marginHorizontal: theme.space.lg, marginBottom: theme.space.lg }}>
          ...
          <View style={{ flexDirection: "row", gap: theme.space.sm, flexWrap: "wrap" }}>
            {[
              { icon: "qr-code-outline", label: "智慧點名", nav: () => navigateToCourseScreen(nav, auth.profile?.role, "Attendance") },
              { icon: "chatbubbles-outline", label: "AI 助理", nav: () => nav?.navigate?.("Today", { screen: "AIChat" }) },
              { icon: "school-outline", label: "選課顧問", nav: () => navigateToCourseScreen(nav, auth.profile?.role, "AICourseAdvisor") },
              { icon: "chatbubble-ellipses-outline", label: "校園社群", nav: () => nav?.navigate?.("CampusSocialScreen") },
              { icon: "calendar-outline", label: "智慧行事曆", nav: () => nav?.navigate?.("SmartCalendarScreen") },
              { icon: "trophy-outline", label: "成就", nav: () => nav?.navigate?.("我的", { screen: "Achievements" }) },
            ].map((item) => (
              <Pressable
                key={item.label}
                onPress={item.nav}
              ...
              >
                ...
                <Text style={{ color: theme.colors.textSecondary, fontSize: 10 }}>{item.label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
