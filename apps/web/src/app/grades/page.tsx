...
const DEFAULT_SEMESTERS = ["113-2", "113-1", "112-2", "112-1"];
...
export default function GradesPage(props: {
  ...
  const semGpa = useMemo(
    () =>
      +(
        grades.reduce((s, g) => s + g.gpa * g.credits, 0) /
  ...
  const maxGpa = Math.max(...GPA_HISTORY.map((h) => h.gpa));

  return (
    <SiteShell
      title="成績"
      subtitle="學期成績查詢與 GPA 統計"
    ...
    >
      <div className="pageStack">
          ...
          >
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {[
                ...
                { label: "最高分", val: Math.max(...grades.map((g) => g.score)) },
        ...
        <div className="sectionCard">
          ...
          >
            {selectedSemester} 學期成績 · {grades.length} 門課程
          </div>
              ...
              <div key={row.label}>
                ...
                <div className="progressTrack">
                  <div
                    ...
                    style={
                      {
                        "--progress-width": `${(row.count / grades.length) * 100}%`,
          ...
          <button
            ...
            onClick={() => {
              const csv = ["科目代碼,課程名稱,學分,成績,分數,GPA,任課教師"]
                .concat(
                  grades.map(
                    (g) => `${g.code},${g.name},${g.credits},${g.grade},${g.score},${g.gpa},${g.instructor}`
                  )
                )
                .join("\n");
