  ...
  const exportData = async () => {
    ...
    try {
      ...
      for (const category of selectedCategories) {
        setProgress(`正在匯出：${category.name}...`);

        switch (category.id) {
          case "profile":
            const profileDoc = await getDoc(doc(db, "users", uid));
            if (profileDoc.exists()) {
              exportedData.profile = profileDoc.data();
            }
            break;

          case "favorites":
            exportedData.favorites = {
              announcement: [],
              event: [],
              poi: [],
              menu: [],
          ...
          case "groups":
            const groupsSnap = await getDocs(
              query(collection(db, "users", uid, "groups"), limit(100))
            );
            exportedData.groups = groupsSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));

            const postsSnap = await getDocs(
              query(collectionGroup(db, "posts"), where("authorId", "==", uid), limit(200))
            );
            exportedData.posts = postsSnap.docs.map((d) => ({
              id: d.id,
          ...
          case "assignments":
            ...
            exportedData.submissions = submissionsSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
            break;
          ...
          case "registrations":
            ...
            exportedData.registrations = regsSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
            break;
          ...
          case "messages":
            ...
            for (const convDoc of conversationsSnap.docs) {
              const messagesSnap = await getDocs(
                query(collection(db, "conversations", convDoc.id, "messages"), limit(100))
              );
              conversations.push({
                id: convDoc.id,
                ...convDoc.data(),
          ...
          case "notifications":
            ...
            if (prefsDoc.exists()) {
              exportedData.notificationPreferences = prefsDoc.data();
            }

            const tokensSnap = await getDocs(
              collection(db, "users", uid, "pushTokens")
            );
            exportedData.pushTokens = tokensSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
            break;
          ...
          case "lostfound":
            const lostFoundSnap = await getDocs(
              query(
                collection(db, "schools", school.id, "lostFound"),
                where("userId", "==", uid),
                limit(100)
              )
            );
            exportedData.lostFound = lostFoundSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
            break;
          ...
          <View style={{ gap: 10 }}>
            ...
            <Pressable
              ...
              style={{
                ...
                borderWidth: 2,
                borderColor:
                  exportFormat === "json" ? theme.colors.accent : theme.colors.border,
              }}
            >
              <Ionicons
