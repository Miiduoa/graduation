...
| Demo 資料流／最終版 SOP | [`docs/DEMO_DATA_FLOW_MAP.md`](docs/DEMO_DATA_FLOW_MAP.md)、[`docs/DEMO_FINAL_AND_DEV_SOP_2026_05_15.md`](docs/DEMO_FINAL_AND_DEV_SOP_2026_05_15.md)、[`docs/USER_JOURNEY_REALITY_2026_05_15.md`](docs/USER_JOURNEY_REALITY_2026_05_15.md)；示範診斷清單：[`DEMO_診斷清單.md`](DEMO_診斷清單.md) |
...
| Web 離線身分切換（`localStorage`） | **`DemoRole`**／**`DEMO_USERS`**：[`demoRole.ts`](apps/web/src/lib/demoRole.ts)、[`DemoRolePill.tsx`](apps/web/src/components/DemoRolePill.tsx)（[`SiteShell.tsx`](apps/web/src/components/SiteShell.tsx)）；擴充 [`demoData.ts`](apps/web/src/lib/demoData.ts) **`DemoUser`**（`department`／`affiliation`／多 uid） |
| 巴士路線資料庫（靜態 TS）／離線路網 mockup | [`campusBusRoutes.ts`](apps/mobile/src/data/campusBusRoutes.ts)（`RouteCategory`、`polyline`、`weekdayDepartures`）；[`demo_預覽/campus_bus_map_v2_mockup.html`](demo_預覽/campus_bus_map_v2_mockup.html) |
...
- **重算「專案快照」表格中的計數**（與本 README **基準日 2026-05-16（含 `campusBusRoutes`、`OnBusModeScreen`、Web **`demoRole`／`DemoRolePill`**、中控台敘事主線、`demo_預覽`、pptx）** 之表一致時，下列指令應得到：**1063**／**137**／**15**／**75**／5／24／**26**／4／5／12；日後若數字漂移，請以指令輸出為準並更新表格，`git ls-files | wc -l` 請在 **已 commit** 後複核）：
...
- [目前最重要的事實](#目前最重要的事實)
- [專案快照](#專案快照)
- [本期程式與 QA 對照摘要（2026-05-15–16）](#readme-qa-20260516)
...
下列數字為 **2026-05-15 主線對齊、2026-05-16 複核** 對目前 repo 的實際盤點（見上方「複核指令」備註）。後續功能增減時，請以程式碼與當下指令輸出為準。

| 面向              | 盤點結果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git tracked files | **`1063`** 個（**複核**：`git ls-files \| wc -l`；**2026-05-16 本批**含 **`campusBusRoutes.ts`**、**[`OnBusModeScreen.tsx`](apps/mobile/src/screens/OnBusModeScreen.tsx)**、**[`AIAgentObservatoryScreen.tsx`](apps/mobile/src/screens/AIAgentObservatoryScreen.tsx)**、**[`dynamicQuietHours.ts`](apps/mobile/src/services/dynamicQuietHours.ts)**＋[**`dynamicQuietHours.test.ts`**](apps/mobile/src/__tests__/dynamicQuietHours.test.ts)、`TodayCockpitScreen` **`getStudentLifeQuickFacts`**、`TeacherCockpitScreen` **辦公室／Office Hour**、Web **`demoRole`／膠囊／登入網格／教師課程 TA 視角**、擴充 [**`demoData.ts`](apps/web/src/lib/demoData.ts)、`aiOrchestrator.test.ts` flake 修復、`demo_預覽`／`口試簡報.pptx`；並延續 **2026-05-15** `LoginLanding`、各 cockpit、AI‑Core、`roleEventBus`、PuWebView、TC demo 資產等）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Mobile UI         | **`137`** 個 `apps/mobile/**/*Screen.tsx`、`15` 個 `*Stack.tsx`（複核：`find apps/mobile/src/screens -name '*Screen.tsx' \| wc -l`，Stack 請用 `find … '*Stack.tsx'`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 測試檔            | Mobile **`75`**（`apps/mobile/src/__tests__/**/*.test.{ts,tsx}`；**不含** `pnpm ai:train:long` 專用之 `apps/mobile/scripts/ai-training-long.test.ts`；代表性 Jest：既有 **LMS／閘道** 類測試外，本批含 **`dynamicQuietHours`**、`aiOrchestrator`（含 vendor 時段測試穩定化）、既有 **`aiContextBuilder`**／**`aiDataInventory`** 等）；Web **`5`**、Backend Functions **`24`**、Rules **`1`**（複核指令同前）                                                                                                                                                                                                                                               |
...
<h3 id="readme-qa-20260516">本期程式與 QA 對照摘要（2026-05-15–16）</h3>

下列條目不取代完整 diff，而是用「接手的人可以立刻對上檔案與責任」的方式描述近期主線工程重點。

**2026-05-16（中控台敘事 × 交通／校車資料 × Web demo 身分 × AI 天文台 × Vendor Jest）：**
- **`apps/mobile/src/data/campusBusRoutes.ts`**：結構化 **校園／市區公車／接駁** 路線；每線含 **`stops[]`**、`polyline`、平日 **`weekdayDepartures`**、**`CrowdLevel`**（壅擠／客滿標註）等。消費端含 **[`OnBusModeScreen.tsx`](apps/mobile/src/screens/OnBusModeScreen.tsx)**；其餘請與 **`BusScheduleScreen`**、**[`MapScreen.tsx`](apps/mobile/src/screens/MapScreen.tsx)**／[`MapStack.tsx`](apps/mobile/src/screens/MapStack.tsx)、AI 「搭車建議」語境之實際 `import` 對照。**`campusBusRoutes.ts` 檔頭**仍對未來 **GoogleMaps 風 POI 圖層**留白說明。
- **Web：** [`demoRole.ts`](apps/web/src/lib/demoRole.ts)（**`demoRole`** → **`localStorage`**）、[`DemoRolePill.tsx`](apps/web/src/components/DemoRolePill.tsx)（[`SiteShell.tsx`](apps/web/src/components/SiteShell.tsx)）、擴充 [`demoData.ts`](apps/web/src/lib/demoData.ts) **`DemoUser`／`DEMO_USERS`**；[`login/page.tsx`](apps/web/src/app/login/page.tsx) 示範模式為 **八角色網格**（**`writeDemoRole`** → **`router.push(entryHref)`**）；[`teacher/course/[courseId]/page.tsx`](apps/web/src/app/teacher/course/[courseId]/page.tsx) 以 **`useDemoRole`**／**`getCapabilities`** 標示 **TA 視角**並依 **`capabilities`** 收斂高風險操作。Jest：**[`dynamicQuietHours.test.ts`](apps/mobile/src/__tests__/dynamicQuietHours.test.ts)** × [`dynamicQuietHours.ts`](apps/mobile/src/services/dynamicQuietHours.ts)。
...
- **備訂：** **`apps/mobile/src/services/dynamicQuietHours.ts`** — **`isInDynamicQuietHours`** 等純函式（夜間靜音、課表中、用餐尖峰、自訂窗），預備與 **`evaluateGuardrails`／推播護欄**接軌（目前請以程式 `import` 為準）。
