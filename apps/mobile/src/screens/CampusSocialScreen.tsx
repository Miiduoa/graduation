...
const ALL_CATEGORIES: (PostCategory | "all")[] = [
  "all", "course_discussion", "confession", "vent", "marketplace",
  "food_review", "question", "poll", "lost_found", "club_recruit",
];
...
export function CampusSocialScreen() {
  ...
  const handleBookmark = useCallback(async (postId: string) => {
    ...
    setPosts((prev) => prev.map((p) =>
      p.id === postId ? { ...p, bookmarked: result } : p
    ));
  }, []);
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: theme.colors.bg, justifyContent: "center", alignItems: "center" }}>
        ...
        <Text style={{ color: theme.colors.textSecondary, marginTop: theme.space.md }}>載入社群動態...</Text>
      </View>
    );
  }
      ...
      >
        ...
        <View style={{ paddingHorizontal: theme.space.lg, marginBottom: theme.space.md }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={{ color: theme.colors.text, fontSize: 28, fontWeight: "800" }}>校園社群</Text>
            <Pressable
        ...
        <ScrollView
          ...
          contentContainerStyle={{ paddingHorizontal: theme.space.lg, gap: 6, marginBottom: theme.space.md }}
        >
        ...
        {/* Sort Tabs */}
        <View style={{ flexDirection: "row", paddingHorizontal: theme.space.lg, gap: theme.space.md, marginBottom: theme.space.md }}>
          {([
        ...
        {posts.length === 0 && (
          <View style={{ alignItems: "center", paddingVertical: theme.space.xxl }}>
            ...
            <Text style={{ color: theme.colors.textSecondary, fontSize: 14, marginTop: theme.space.md }}>
              這個分類還沒有貼文，來發第一篇吧！
            </Text>
          </View>
        )}
    ...
    <View style={{
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.sm }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          <View>
            <Text style={{ color: theme.colors.text, fontSize: 13, fontWeight: "600" }}>{post.authorAlias}</Text>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
              {post.authorDept && (
                <Text style={{ color: theme.colors.textSecondary, fontSize: 10 }}>{post.authorDept}</Text>
              )}
        ...
        <View style={{
          ...
          <Text style={{ color: catColor, fontSize: 10, fontWeight: "600" }}>{getCategoryLabel(post.category)}</Text>
        </View>
      </View>

      {/* Content */}
      <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "700", marginBottom: 4 }}>{post.title}</Text>
      <Text style={{ color: theme.colors.textSecondary, fontSize: 14, lineHeight: 20, marginBottom: theme.space.sm }}>
        {post.content}
      </Text>
      ...
      {post.category === "marketplace" && post.price != null && (
        <View style={{
          ...
          {post.condition && (
            <View style={{ backgroundColor: theme.colors.accentSoft, borderRadius: theme.radius.sm, paddingHorizontal: 6, paddingVertical: 2 }}>
              <Text style={{ color: theme.colors.accent, fontSize: 11 }}>
                {post.condition === "new" ? "全新" : post.condition === "like_new" ? "近全新" : post.condition === "good" ? "良好" : post.condition === "fair" ? "普通" : "堪用"}
              </Text>
            </View>
          )}
          {post.sold && (
            <View style={{ backgroundColor: "#EF444420", borderRadius: theme.radius.sm, paddingHorizontal: 6, paddingVertical: 2 }}>
              ...
              >
                {hasVoted && (
                  <View style={{
                    ...
                    width: `${pct}%` as any,
                    backgroundColor: opt.votedByMe ? theme.colors.accent + "20" : theme.colors.border + "40",
                    borderRadius: theme.radius.md,
                  }} />
                )}
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", zIndex: 1 }}>
                  ...
                  {hasVoted && (
                    <Text style={{ color: theme.colors.textSecondary, fontSize: 12 }}>{pct.toFixed(0)}%</Text>
                  )}
                </View>
              </Pressable>
            );
      ...
      {/* Tags */}
      {post.tags.length > 0 && (
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 4, marginBottom: theme.space.sm }}>
          {post.tags.map((tag) => (
            <Text key={tag} style={{ color: theme.colors.accent, fontSize: 11 }}>#{tag}</Text>
          ))}
        </View>
      )}

      {/* Action Bar */}
      <View style={{ flexDirection: "row", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: theme.space.sm }}>
        ...
        <Pressable onPress={onComment} style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
          <Ionicons name="chatbubble-outline" size={18} color={theme.colors.textSecondary} />
          <Text style={{ color: theme.colors.textSecondary, fontSize: 12 }}>{post.commentCount}</Text>
        </Pressable>
      ...
      >
        <View style={{ paddingTop: insets.top + 8, paddingHorizontal: theme.space.lg }}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.lg }}>
            ...
            <Text style={{ color: theme.colors.text, fontSize: 17, fontWeight: "700" }}>發佈貼文</Text>
            <Pressable
              onPress={handleSubmit}
              disabled={!title.trim() || !content.trim()}
              style={({ pressed }) => ({
                backgroundColor: title.trim() && content.trim() ? theme.colors.accent : theme.colors.disabledBg,
                borderRadius: theme.radius.md,
            ...
            >
              <Text style={{ color: title.trim() && content.trim() ? "#fff" : theme.colors.disabledText, fontSize: 14, fontWeight: "600" }}>
                發佈
              </Text>
            </Pressable>
          </View>
          ...
          {/* Category Picker */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginBottom: theme.space.md }}>
            {(["course_discussion", "confession", "vent", "marketplace", "food_review", "question", "poll", "lost_found"] as PostCategory[]).map((cat) => {
              ...
              return (
                ...
                >
                  <Text style={{ color: active ? "#fff" : theme.colors.textSecondary, fontSize: 12, fontWeight: active ? "700" : "500" }}>
            ...
            comments.map((c) => (
              <View key={c.id} style={{
                ...
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  ...
                  <Text style={{ color: theme.colors.muted, fontSize: 11 }}>{formatTimeAgo(c.createdAt)}</Text>
                </View>
                <Text style={{ color: theme.colors.textSecondary, fontSize: 14, marginTop: 4, lineHeight: 20 }}>
                  {c.content}
                </Text>
              </View>
            ))
