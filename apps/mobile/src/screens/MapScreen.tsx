...
const INTENT_MAP: Array<{ keywords: string[]; categories: CampusPoiCategory[] }> = [
  {
    keywords: ['吃', '飯', '餐', '食', '肚子餓', '午餐', '晚餐', '早餐', '便當'],
    categories: ['cafeteria', 'convenience'],
  },
  ...
  for (const intent of INTENT_MAP) {
    if (intent.keywords.some((kw) => q.includes(kw))) {
      ...
      if (userLat !== null && userLng !== null) {
        results.sort(
          (a, b) =>
            haverDist(userLat, userLng, a.lat, a.lng) - haverDist(userLat, userLng, b.lat, b.lng),
        );
      }
      return results;
    }
...
function getSmartSuggestions(
  ...
  const findNearest = (cats: CampusPoiCategory[]): CampusPoi | null => {
    ...
    return pois.reduce((nearest, p) =>
      haverDist(userLat, userLng, p.lat, p.lng) <
      haverDist(userLat, userLng, nearest.lat, nearest.lng)
        ? p
        : nearest,
    );
  };
      ...
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.4)' }}>
        ...
        >
          ...
          >
            ...
            <Text style={{ color: theme.colors.accent, fontWeight: '800', fontSize: 15 }}>
              開啟 AR 實景導航
            </Text>
          </Pressable>
          <FlatList
              ...
              >
                <View
                  style={{
                    ...
                    height: 30,
                    borderRadius: 15,
                    backgroundColor: `${dColor(item.direction)}12`,
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
                >
                  <Ionicons
                    name={dIcon(item.direction) as any}
                    size={15}
                    color={dColor(item.direction)}
                  />
                </View>
                <View style={{ flex: 1 }}>
...
export function MapScreen(props: Record<string, unknown>) {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeScenario !== null) {
      ...
      if (uLat !== null && uLng !== null)
        pois.sort(
          (a, b) => haverDist(uLat, uLng, a.lat, a.lng) - haverDist(uLat, uLng, b.lat, b.lng),
        );
      return pois;
    }
    // Search query
  ...
  const selectedPoi = useMemo(
    () => (selectedPoiId ? (getCampusPoi(selectedPoiId) ?? null) : null),
    [selectedPoiId],
  );
  ...
  const handleShowRoute = useCallback(
    (poi: CampusPoi) => {
      ...
      setRoutePreview({
        steps,
        totalDist: steps.reduce((s, st) => s + st.distance, 0),
        name: poi.name,
      });
    },
    [uLat, uLng],
  ...
  const handleSuggestion = useCallback(
    (sg: SmartSuggestion) => {
      if (sg.action === 'navigate_poi') {
        ...
        const idx = QUICK_SCENARIOS.findIndex((s) =>
          s.filter(CAMPUS_POIS).some((p) => p.category === cat),
        );
        if (idx >= 0) {
          ...
          {suggestions.map((sg, i) => (
            <Pressable
              ...
              style={({ pressed }) => ({
                ...
                paddingVertical: 10,
                borderRadius: 14,
                backgroundColor: `${sg.color}10`,
                borderWidth: 1,
                borderColor: `${sg.color}25`,
          ...
          {QUICK_SCENARIOS.map((sc, i) => {
            ...
            return (
              <Pressable
                key={i}
                onPress={() => {
                  setActiveScenario(active ? null : i);
                  setQ('');
                }}
                style={({ pressed }) => ({
                  flexDirection: 'row',
                  ...
                  borderWidth: 1,
                  borderColor: active ? `${sc.color}50` : theme.colors.border,
                  transform: [{ scale: pressed ? 0.96 : 1 }],
                })}
              >
                <Ionicons
                  name={sc.icon as any}
                  size={14}
                  color={active ? sc.color : theme.colors.muted}
                />
                <Text
                  style={{
                    color: active ? sc.color : theme.colors.textSecondary,
                    fontWeight: '600',
                    fontSize: 12,
                  }}
                >
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      <FlatList
        ...
        contentContainerStyle={{
          ...
          paddingTop: 8,
          gap: 8,
          paddingBottom: selectedPoi
            ? TAB_BAR_CONTENT_BOTTOM_PADDING + 100
            : TAB_BAR_CONTENT_BOTTOM_PADDING,
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => ({
                ...
                borderWidth: 1,
                borderColor: sel ? `${c}35` : theme.colors.border,
                borderLeftWidth: 4,
                borderLeftColor: c,
                paddingVertical: 12,
                paddingHorizontal: 14,
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    {poi.floor !== '地面' && poi.floor !== '戶外' && poi.floor !== '平面'
                      ? ` · ${poi.floor}`
                      : ''}
                    {poi.accessible ? ' · 無障礙' : ''}
                  </Text>
        ...
        >
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              >
                <Ionicons
                  name={CATEGORY_ICONS[selectedPoi.category] as any}
                  size={20}
                  color={CAT_COLORS[selectedPoi.category]}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Text
                ...
                <Text
                  style={{ color: theme.colors.muted, fontSize: 12, marginTop: 1 }}
                  numberOfLines={1}
                >
