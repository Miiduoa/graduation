...
describe("offline agent routing", () => {
  ...
  it("keeps action tools parameterized instead of plain Q&A", () => {
    ...
    expect(getToolById("report_repair")?.parameters.some((p) => p.name === "room" && p.required)).toBe(true);
    expect(getToolById("reserve_seat")?.parameters.some((p) => p.name === "time_slot" && p.required)).toBe(true);
  });
});
