...
export function NotificationsScreen(props: any) {
  ...
  const renderNotification = useCallback(
    ({ item: n }: ListRenderItemInfo<Notification>) => (
      <NotificationItem
        key={n.id}
        id={n.id}
        type={n.type}
  ...
  const ListHeader = useCallback(
    () => (
      <Card
        title="通知"
        subtitle={`共 ${notifs.notifications.length} 則（未讀 ${notifs.unreadCount}）`}
      >
      ...
      <Screen>
        ...
        >
          <Card title="通知" subtitle="尚未登入">
            <Text style={{ color: theme.colors.muted }}>請先到『我的』登入後才能查看通知。</Text>
          </Card>
        </ScrollView>
      </Screen>
    );
