...
export default function TeacherAttendancePage({ params, searchParams }: { params: { courseId: string }; searchParams?: { school?: string; schoolId?: string } }) {
  ...
  return (
    <SiteShell title="點名管理" schoolName={schoolName}>
      <main style={{ maxWidth: 960, margin: '0 auto', padding: 24 }}>
        ...
        {isReadOnlyView && (
          <div style={{
            ...
            {isTaView
              ...
              : <><span>🏛️ </span><strong>系主任視角</strong>：可唯讀瀏覽出席記錄，但<strong>無法開啟或結束 QR 點名</strong>（授課教師專用）。</>
            }
          </div>
        )}

        {/* 狀態列 */}
        <div style={{ padding: 24, border: '1px solid var(--border)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600 }}>目前點名狀態</div>
            <div style={{ color: 'var(--muted)', fontSize: 14 }}>{running ? '🟢 點名進行中（QR）' : '⚪ 尚未開啟'}</div>
          </div>
          {isReadOnlyView ? (
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 8 }}>
              {DEMO_STUDENTS.map((s) => {
                ...
                return (
                  <button key={s.uid} onClick={() => toggleLive(s.uid)} style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid var(--border)', background: st === 'present' ? 'rgba(22,163,74,0.08)' : st === 'late' ? 'rgba(217,119,6,0.08)' : 'rgba(220,38,38,0.08)', cursor: 'pointer', textAlign: 'left' }}>
          ...
          {sessions.map((s) => {
            ...
            return (
              <div key={s.id} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '14px 18px', background: 'var(--panel)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{s.startedAt}</div>
                    <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{s.mode.toUpperCase()} · 出席 {present}/{total}（{pct}%）</div>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => setExpandedId(isExp ? null : s.id)} style={{ ...linkBtn, border: '1px solid var(--border)', borderRadius: 6, padding: '6px 12px', fontWeight: 600 }}>
                      {isExp ? '▲ 收合' : '👥 查看名單'}
                    </button>
                    <button
                      onClick={() => {
                      ...
                      style={{ ...linkBtn, border: '1px solid var(--border)', borderRadius: 6, padding: '6px 12px' }}
                    >
                      📥 匯出 CSV
                    </button>
                    <a
                  ...
                  <div style={{ padding: '14px 18px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 8 }}>
                      {DEMO_STUDENTS.map((stu) => {
                        ...
                        return (
                          <div key={stu.uid} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: st === 'present' ? 'rgba(22,163,74,0.06)' : st === 'late' ? 'rgba(217,119,6,0.06)' : 'rgba(220,38,38,0.06)' }}>
                            <div style={{ fontWeight: 600, fontSize: 13 }}>{stu.displayName}</div>
                            <div style={{ fontSize: 11, color: 'var(--muted)' }}>{stu.studentId}</div>
                            <div style={{ fontSize: 12, color: STATUS_COLOR[st], marginTop: 2 }}>{STATUS_LABEL[st]}</div>
                          </div>
                        );
                      })}
        ...
        >
          ...
          <a
            href={`/ai-assistant${q ? q + '&' : '?'}q=${encodeURIComponent(`幫我分析「${course?.name ?? '資料結構'}」近期出勤記錄，找出缺席率超過 20% 的學生，並草擬一封出勤提醒通知`)}`}
            className="btn"
...
const linkBtn: CSSProperties = { background: 'transparent', color: 'var(--text)', border: 'none', cursor: 'pointer', fontSize: 13 };
