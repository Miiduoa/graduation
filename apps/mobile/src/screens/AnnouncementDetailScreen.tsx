...
function extractDatesFromText(text: string): ExtractedDate[] {
  ...
  const patterns = [
    ...
    {
      regex: /截止[日期時間：:]*[為是]?[：:]?\s*(\d{1,2})月(\d{1,2})日/g,
  ...
  const uniqueDates = dates.filter((d, i, arr) => {
...
export function AnnouncementDetailScreen(props: any) {
    ...
    try {
      ...
      if (!announcement) {
        ...
        try {
          ...
          const keywords = announcement.title
            .split(/[\s,，、]/)
  ...
  const handleAddToCalendar = (date: ExtractedDate) => {
    Alert.alert('加入行事曆', `要將「${date.text}」加入行事曆嗎？`, [
    ...
    <Screen>
      ...
      >
        <AnimatedCard title={item.title} subtitle="">
          ...
          <Text style={{ color: theme.colors.text, lineHeight: 24, fontSize: 15 }}>
            {item.body}
          </Text>
                  ...
                  {aiResult.extractedDates.map((d, i) => (
                    <Pressable
                      ...
                      style={{
                        ...
                        borderWidth: 1,
                        borderColor:
                          d.type === 'deadline' ? `${theme.colors.danger}30` : theme.colors.border,
                      }}
                    >
