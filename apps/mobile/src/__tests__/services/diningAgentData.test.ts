...
describe("dining agent data", () => {
  it("uses verified Providence dining options in order parameters", () => {
    ...
    const labels = orderTool?.parameters.find((param) => param.name === "cafeteria")?.options?.map((option) => option.label) ?? [];

    expect(labels).toEqual(expect.arrayContaining(["靜園餐廳", "宜園餐廳", "至善美食廣場一樓", "OK 便利商店"]));
  ...
  it("official PU catalog contains only current dining venue names", () => {
    const menuText = getPuDiningMenuItems("pu").map((menu) => `${menu.name} ${menu.cafeteria}`).join("\n");
