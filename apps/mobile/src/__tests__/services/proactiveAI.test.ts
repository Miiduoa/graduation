...
describe('proactive AI reports', () => {
  ...
  it('reports due and late assignments without inventing course data', () => {
    ...
    expect(
      reports.some(
        (report) => report.kind === 'assignment_due' && report.body.includes('資料庫期末專題'),
      ),
    ).toBe(true);
    expect(
      reports.some(
        (report) => report.kind === 'assignment_late' && report.body.includes('訪談紀錄整理'),
      ),
    ).toBe(true);
  ...
  it('includes important announcements in the daily proactive report', () => {
    ...
    expect(
      reports.some(
        (report) => report.kind === 'daily_brief' && report.body.includes('獎學金申請截止'),
      ),
    ).toBe(true);
    expect(
      reports.some((report) => report.kind === 'announcement' && report.title.includes('重要公告')),
    ).toBe(true);
  });
});
