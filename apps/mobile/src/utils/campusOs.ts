...
export function canRoleUseAction(
  role: CampusActorRole | undefined,
...
function asDate(value: unknown): Date | null {
  ...
  const seconds = (
    typeof maybeFirestore._seconds === 'number' ? maybeFirestore._seconds : maybeFirestore.seconds
  ) as number | undefined;
  const nanoseconds = (
    typeof maybeFirestore._nanoseconds === 'number'
      ? maybeFirestore._nanoseconds
      : maybeFirestore.nanoseconds
  ) as number | undefined;
...
export function getTodayCourses(courses: Course[], date: Date = new Date()) {
  ...
  return todayCourses.sort(
    (a, b) =>
      (startMinuteMap.get(a) ?? Number.MAX_SAFE_INTEGER) -
      (startMinuteMap.get(b) ?? Number.MAX_SAFE_INTEGER),
  );
}
