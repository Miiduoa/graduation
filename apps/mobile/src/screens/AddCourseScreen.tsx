...
function SSOCourseSync({ onCoursesImported, schoolId, semester }: SSOCourseSyncProps) {
  ...
  const [status, setStatus] = useState<
    'idle' | 'authenticating' | 'fetching' | 'success' | 'error'
  >('idle');
  ...
  const handleSSOLogin = async () => {
    ...
    try {
      ...
      const courseApiUrl =
        ssoConfig.courseApiUrl ||
        `${ssoConfig.authUrl.replace('/oauth/authorize', '/api/courses')}`;
      ...
      const courses: DataCourse[] = (data.courses || data.data || []).map(
...
export function AddCourseScreen(props: any) {
  ...
  const filteredSamples = SAMPLE_COURSES.filter(
    (c) => c.name.includes(searchQuery) || c.instructor.includes(searchQuery),
  );
        ...
        <ScrollView style={{ flex: 1, marginTop: 12 }} showsVerticalScrollIndicator={false}>
                  ...
                  {selectingTime ? (
                    ...
                    >
                      ...
                      <View style={{ flexDirection: 'row', gap: 12 }}>
                        <View style={{ flex: 1 }}>
                          <Text
                            style={{ color: theme.colors.muted, fontSize: 12, marginBottom: 6 }}
                          >
                            開始節次
                          </Text>
                          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            <View style={{ flexDirection: 'row', gap: 4 }}>
                              {PERIODS.map((p) => (
                                ...
                                >
                                  <Text
                                    style={{
                                      color: tempStartPeriod === p.num ? '#fff' : theme.colors.text,
                                      fontWeight: '600',
                                      fontSize: 13,
                                    }}
                                  >
          ...
          {selectedTab === 1 && (
            <View style={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
              ...
              <Text
                style={{ color: theme.colors.muted, fontSize: 12, marginLeft: 4, marginTop: 8 }}
              >
                範例課程
              </Text>
                ...
                <View style={{ gap: 12 }}>
                  ...
                  <SSOCourseSync
                    onCoursesImported={async (courses) => {
                      ...
                      if (courses.length > 0) {
                        Alert.alert('匯入完成', `已成功匯入 ${courses.length} 門課程到您的課表`, [
                          { text: '好', onPress: () => nav?.goBack?.() },
                        ]);
                      }
                    }}
