...
| 成績 what-if 試算（共用引擎） | [`gradePredictor.ts`](packages/shared/src/lms/gradePredictor.ts)：加權總成 **樂觀／悲觀／中性** 預估、**假設改分**、`requiredToReach` 反推；[`index.ts`](packages/shared/src/index.ts) re-export，`gradebookCompute` 與同源權重正規化語意 |
...
學習 Tab → **我的課程** → 課程分頁 → 任一課程卡上的八枚捷徑，全部掛在 **[`LearnStack.tsx`](apps/mobile/src/screens/LearnStack.tsx)**。**2026-05-15**：多數課務子畫面包 **`guardCourseView`**（[`RouteGuard`](apps/mobile/src/ui/RouteGuard.tsx) **`requires="courses.view"`**），避免職員／店家／無課務權限身分經通知 deep link **誤進**教材／討論／測驗等工作區；被拒時可 **`回到學習首頁`**，若有 **`courses.catalog`** 會額外提示 **前往課綱查詢**。**點名**仍走既有 **`GuardedAttendance`**（`courses.view` + `courses.attendance`）。2026-05-13 起主線做法是 **各 chip 依課程 id 直接對 TronClass 取數**（繞過會回空的抽象層／避免學生成績誤跳教師權限頁）；細部 endpoint、修法前後對照與重啟驗證步驟見 **[`docs/8_CHIPS_FINAL_AUDIT.md`](docs/8_CHIPS_FINAL_AUDIT.md)**。
...
| 面向              | 盤點結果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git tracked files | **`972`** 個（**複核**：`git ls-files \| wc -l`；基準日 **2026-05-15**，含 **`gradePredictor`**、TronClass demo、口試 checklist、[`UI_POLISH_2026_05_14`](docs/UI_POLISH_2026_05_14.md) 對照等）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Mobile UI         | **`121`** 個 `apps/mobile/**/*Screen.tsx`、`14` 個 `*Stack.tsx`（複核：`find apps/mobile/src/screens -name '*Screen.tsx' \| wc -l`，Stack 請用 `find … '*Stack.tsx'`；**2026-05-15** 複核）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**2026-05-15 追加（`gradePredictor`：加權總成 what-if 試算）**：TronClass／校方 LMS 通常只顯示已公佈成績，本 repo 在 `@campus/shared` 新增純函式模組 [`packages/shared/src/lms/gradePredictor.ts`](packages/shared/src/lms/gradePredictor.ts)（[`index.ts`](packages/shared/src/index.ts) re-export）：**`predictCurrent`** 區分已批改／未批改項目，輸出 `bestCase`／`worstCase`／`likelyCase`（以已批改平均分推估剩餘項）與 `letterGrade`；**`simulateWhatIf`** 以 `WhatIfOverride[]` 將指定項改為假設分數後重算並給 `delta`；**`requiredToReach`** 在凍結非未來項的前提下，計算 `futureItemIds` 上達到 `targetPercent` 所需的 **平均百分制**、`feasibility`（easy／doable／hard／impossible）與上限 `ceiling`；**`commonTargets`** 封裝 60／70／80／90 捷徑。權重自動正規化至加總 100、支援 `maxScore` 與 `excused`，與既有 **`gradebookCompute`** 策略一致；無 I／O，可由 Mobile **`CourseScoresScreen`**／Web／Agent 後續接上。
...
| 成績與學分         | `GradesScreen`、`CourseGradebookScreen`、`CourseScoresScreen`、`MyQuizScoresScreen`（`LearnStack`）、`CreditAuditScreen`、`CreditAuditInputScreen`                                                 | 成績查詢、教師成績簿、單課成績彙整、測驗分數總覽、畢業學分試算；**試算補強**：`@campus/shared` **`gradePredictor`** 可對同一套加權項做 what-if（尚未全面掛 UI，契約已就緒）                                                                       |
