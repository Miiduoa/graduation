...
export function CourseGradebookScreen(props: any) {
          ...
          {memberships.map((membership) => (
            ...
            >
              <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                <Pill
                  text={canManageCourse(membership.role) ? '教師視角' : '學生視角'}
                  kind="accent"
                />
  ...
  const classAverage =
    gradedRows.length > 0
      ? Math.round(
          (gradedRows.reduce((sum, row) => sum + (row.finalScore ?? 0), 0) / gradedRows.length) *
            10,
        ) / 10
      : null;
  ...
  return (
    <Screen noPadding>
      ...
      >
        <Card
          title={`${selectedCourseName} 成績簿`}
          subtitle={isTeacher ? '教師可查看整班成績與各作業分解' : '查看這門課的評分進度與期末總分'}
        >
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            <Pill text={`${gradebook.assignments.length} 項評分項目`} kind="accent" />
            <Pill
              text={`${visibleRows.length} 位${isTeacher ? '學生' : '我的成績'}`}
              kind="default"
            />
            <Pill
        ...
        <Card
          title="評分項目"
          subtitle={`最後發布：${formatDateTime(gradebook.finalScoresPublishedAt, '尚未發布')}`}
        >
          <View style={{ gap: 10 }}>
            {gradebook.assignments.length === 0 ? (
              ...
              gradebook.assignments.map((assignment) => (
                ...
                >
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                    <Pill
                      text={`截止 ${formatDateTime(assignment.dueAt, '未設定')}`}
                      kind="default"
                    />
                    {assignment.averageScore !== null ? (
                      <Pill text={`平均 ${assignment.averageScore}`} kind="default" />
                    ) : null}
                    <Pill
                      text={assignment.gradesPublished ? '已發布' : '未發布'}
                      kind={assignment.gradesPublished ? 'success' : 'warning'}
                    />
                  </View>
                </View>
              ))
        ...
        {visibleRows.map((row) => (
          ...
          >
            <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
              <Pill
                text={`總分 ${row.finalScore ?? '-'}`}
                kind={row.finalScore !== null ? 'accent' : 'muted'}
              />
              <Pill
                text={`評分進度 ${row.gradedAssignments}/${row.totalAssignments}`}
                kind="default"
              />
              <Pill
                text={
                  row.result === 'passed' ? '通過' : row.result === 'failed' ? '未通過' : '尚未完成'
                }
                kind={
                  row.result === 'passed'
                    ? 'success'
              ...
              {row.assignmentBreakdown.map((entry) => (
                ...
                >
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <Pill
                      text={`繳交 ${formatDateTime(entry.submittedAt, '未繳交')}`}
                      kind="default"
                    />
