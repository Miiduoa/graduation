...
const CAMPUS_INTERNAL_INTENTS = new Set([
  "announcements",
  "events",
  "menus",
  "pois",
...
function rankKnowledgeChunks(query, chunks, limit = 6) {
  ...
  return chunks
    .map((chunk) => ({
      ...
      score: scoreText(
        query,
        [chunk.title, chunk.summary, chunk.text, Array.isArray(chunk.tags) ? chunk.tags.join(" ") : ""].join("\n"),
      ),
    }))
...
function resolvePermissionScope(intent, hasAuth) {
  ...
  if (PERSONAL_INTENTS.has(intent)) {
    return intent === "credit_audit" || intent === "grades_analysis" ? "academic_private" : "user_private";
  }
...
function shouldUseServerWebSearch(rawText, intent) {
  ...
  const explicitExternal = /連網|搜尋|查網路|公開來源|google maps|wikipedia|維基|現任|即時.*(班次|天氣)|今天.*天氣|天氣|下雨|帶傘|路線|怎麼去|如何到|搭.*公車/i.test(text);
...
function normalizeAssistantActions(actions, permissionScope = "school_public") {
  ...
  return actions
    .map((action) => normalizeAssistantAction(action, permissionScope))
    .filter(Boolean);
}
...
async function fetchJson(url, { fetchImpl = globalThis.fetch, timeoutMs = WEB_TIMEOUT_MS, ...options } = {}) {
...
function routeAnswer(query) {
  ...
  const cleaned = query.replace(/^(請問|幫我|搜尋|查詢)/, "").replace(/[？?。！!]/g, "").trim();
  ...
  if (/靜宜|静宜|providence/i.test(origin) && /台中車站|臺中車站|台中火車站|臺中火車站/.test(destination)) {
    sources.unshift({
...
async function weatherAnswer(query, options = {}) {
  ...
  const data = await fetchJson(`https://api.open-meteo.com/v1/forecast?${params.toString()}`, options);
  ...
  return {
    content: [
      ...
      "",
      rain > 0 ? "目前有降雨資料，建議帶傘。" : "目前降雨量不高，但天氣仍可能變化，出門前可以再確認一次。",
      "",
      `查詢時間：${new Date().toLocaleString("zh-TW", { timeZone: "Asia/Taipei" })}`,
      "資料來源：Open-Meteo",
    ].join("\n"),
...
function buildAgentSystemPrompt({
  ...
  const knowledgeText = knowledgeChunks
    .slice(0, 6)
    .map((chunk, index) => `[K${index + 1}] ${chunk.title || chunk.sourceType || chunk.sourceId || "knowledge"}\n${truncate(chunk.summary || chunk.text, 900)}`)
    .join("\n\n");
...
async function callOpenAiCompatChat({
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (response.status === 429 || response.status >= 500) {
      lastError = { status: response.status, retryAfterMs: parseRetryAfter(response.headers, attempt) };
...
async function callAssistantModel({ messages, fetchImpl = globalThis.fetch, providerOrder = parseProviderOrder() }) {
  const errors = [];
