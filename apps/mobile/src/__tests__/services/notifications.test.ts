...
describe("notification preferences", () => {
  ...
  it("falls back to defaults when Firestore is offline and no cache exists", async () => {
    mockGetDoc.mockRejectedValueOnce(new Error("Failed to get document because the client is offline."));

    await expect(loadNotificationPreferences("u1")).resolves.toEqual(defaultNotificationPreferences);
  });
  ...
  it("saves preferences locally when Firestore write is offline", async () => {
    mockSetDoc.mockRejectedValueOnce(new Error("Failed to get document because the client is offline."));

    await expect(
      saveNotificationPreferences("u1", { ...defaultNotificationPreferences, events: false }),
    ).resolves.toBeUndefined();

    await expect(AsyncStorage.getItem("@notifications.preferences:u1")).resolves.toContain('"events":false');
  });
});
