...
function parseNextBestAction(row: Record<string, unknown>, id: string): NextBestAction {
  return {
    ...
    urgency: (row.urgency as NextBestAction["urgency"]) ?? normalizeUrgency(Number(row.priority ?? 5)),
    ...
    evidenceRefs: Array.isArray(row.evidenceRefs) ? (row.evidenceRefs as NextBestAction["evidenceRefs"]) : [],
...
function parseRiskSnapshot(row: Record<string, unknown>, id: string, userId: string): StudentRiskSnapshot {
  return {
    id,
...
function parsePulseAggregate(row: Record<string, unknown>, id: string, schoolId: string): PulseAggregate {
  ...
  return {
    ...
    currentLevel: Math.min(5, Math.max(1, Math.round(currentLevel))) as PulseAggregate["currentLevel"],
...
function parseAcademicSourceSnapshot(
  ...
  return {
    ...
    courses: Array.isArray(row.courses) ? (row.courses as AcademicSourceSnapshot["courses"]) : undefined,
    grades: Array.isArray(row.grades) ? (row.grades as AcademicSourceSnapshot["grades"]) : undefined,
    attendance: Array.isArray(row.attendance) ? (row.attendance as AcademicSourceSnapshot["attendance"]) : undefined,
    inboxTasks: Array.isArray(row.inboxTasks) ? (row.inboxTasks as AcademicSourceSnapshot["inboxTasks"]) : undefined,
    generatedAt: toDate(row.generatedAt),
    expiresAt: toDate(row.expiresAt),
  };
}

function parseRoleActionGraph(row: Record<string, unknown>, id: string, userId: string, schoolId: string): CampusRoleActionGraph {
  return {
    id,
    userId: typeof row.userId === "string" ? row.userId : userId,
    schoolId: typeof row.schoolId === "string" ? row.schoolId : schoolId,
    activeRoles: Array.isArray(row.activeRoles) ? (row.activeRoles as CampusRoleActionGraph["activeRoles"]) : ["student"],
    ...
    dataSources: Array.isArray(row.dataSources) ? (row.dataSources as CampusRoleActionGraph["dataSources"]) : [],
    generatedAt: toDate(row.generatedAt) ?? new Date(),
  };
}

function buildLocalAcademicSnapshot(userId: string, schoolId: string, inboxTasks: InboxTask[]): AcademicSourceSnapshot {
    ...
    sources: [
      {
        ...
        detail: inboxTasks.length > 0 ? "已由 TronClass 課務快取推導待辦。" : "尚未取得 TronClass 授權或課務快取。",
      },
      {
        key: "e_campus",
        label: "E 校園",
...
function buildLocalRoleActionGraph(userId: string, schoolId: string, actions: NextBestAction[]): CampusRoleActionGraph {
  const generatedAt = new Date();
  return {
...
function parseActionQueueItem(row: Record<string, unknown>, id: string, userId: string): ActionQueueItem {
  return {
    id,
    userId: String(row.userId ?? userId),
    schoolId: typeof row.schoolId === "string" ? row.schoolId : undefined,
    ...
    evidenceRefs: Array.isArray(row.evidenceRefs) ? (row.evidenceRefs as ActionQueueItem["evidenceRefs"]) : [],
...
async function listStoredNextBestActions(userId: string, schoolId?: string): Promise<NextBestAction[]> {
  ...
  return snaps
    .flatMap((snap) => snap?.docs.map((docSnap) => parseNextBestAction(docSnap.data(), docSnap.id)) ?? [])
    .sort((a, b) => a.priority - b.priority)
    .slice(0, 8);
}

export async function listNextBestActions(userId: string, schoolId?: string): Promise<NextBestAction[]> {
    ...
    return [
      {
        ...
        nextStep: "打開課程中樞",
        actionLabel: "查看課程",
        actionTarget: { tab: "課程", screen: "CourseHub", params: { groupId: firstCourse.groupId } },
...
export async function listRiskSnapshots(userId: string, schoolId?: string): Promise<StudentRiskSnapshot[]> {
  if (!isFirebaseMockMode()) {
    ...
    const snaps = await Promise.all(
      refs.map((ref) => getDocs(query(ref, orderBy("generatedAt", "desc"), limit(5))).catch(() => null))
    );
    const stored = snaps.flatMap((snap) =>
      snap?.docs.map((docSnap) => parseRiskSnapshot(docSnap.data(), docSnap.id, userId)) ?? []
    );
    if (stored.length > 0) return stored.sort((a, b) => (b.generatedAt?.getTime() ?? 0) - (a.generatedAt?.getTime() ?? 0));
  ...
  const criticalCount = actions.filter((action) => action.urgency === "critical" || action.urgency === "high").length;
  const score = Math.min(95, criticalCount * 22 + actions.filter((action) => !!action.dueAt).length * 7);
...
export async function listPulseAggregates(schoolId = "tw-pu"): Promise<PulseAggregate[]> {
  if (!isFirebaseMockMode()) {
    ...
    const snap = await getDocs(
      query(collection(db, "schools", schoolId, "pulseAggregates"), orderBy("updatedAt", "desc"), limit(20))
    ).catch(() => null);
    const stored = snap?.docs.map((docSnap) => parsePulseAggregate(docSnap.data(), docSnap.id, schoolId)) ?? [];
    if (stored.length > 0) return stored;
