...
export function MenuSubscriptionScreen(props: any) {
  ...
  const scheduleNotificationsForSubscription = useCallback(
    async (sub: Subscription): Promise<string[]> => {
  ...
  const handleToggleSubscription = async (subId: string) => {
    ...
    try {
      ...
      if (sub.enabled) {
        ...
        updatedNotificationIds = await scheduleNotificationsForSubscription({
          ...sub,
          enabled: true,
        });
      }
  ...
  const formatDays = (days: SubscriptionDay[]): string => {
    ...
    if (days.length === 2 && ['sat', 'sun'].every((d) => days.includes(d as SubscriptionDay))) {
      return '週末';
    }
        ...
        >
          {selectedTab === 0 && (
            <View style={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
              ...
              {subscriptions.length > 0 && (
                <Button text="新增訂閱" onPress={() => setSelectedTab(1)} />
              )}
            </View>
          )}
                ...
                <View style={{ gap: 12 }}>
                  ...
                  >
                    ...
                    <Text style={{ color: theme.colors.text, fontWeight: '600' }}>
                      {subscriptions.reduce((sum, s) => sum + (s.notificationIds?.length ?? 0), 0)}{' '}
                      個
                    </Text>
                  </View>
                  <View
