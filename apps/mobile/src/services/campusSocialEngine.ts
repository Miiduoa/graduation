...
const SENSITIVE_WORDS = ['自殺', '自殘', '跳樓', '毒品', '賭博', '色情', '霸凌', '威脅', '恐嚇'];
...
function generateMockPosts(): SocialPost[] {
  ...
  const posts: SocialPost[] = [
    ...
    },
    {
      ...
      content:
        'Ross 的統計學第七版，有螢光筆畫重點但不影響閱讀，原價 $650，賣 $250。面交限校園內。',
      images: [],
      tags: ['教科書', '統計學', '二手'],
    ...
    },
    {
      ...
      content:
        '在伯鐸樓三樓男廁洗手台撿到一個 AirPods Pro，白色充電盒。失主請私訊我確認顏色和序號。',
      images: [],
      tags: ['失物招領', 'AirPods'],
...
export async function createPost(data: {
  ...
  const post: SocialPost = {
    ...
    pollOptions: data.pollOptions
      ...
      : undefined,
    pollEndsAt: data.pollDurationHours ? now + data.pollDurationHours * 60 * 60 * 1000 : undefined,
    reported: false,
    hidden: false,
  };
 ...
 * 按讚/倒讚
 */
export async function toggleLike(postId: string, type: 'like' | 'dislike'): Promise<void> {
  const posts = await loadPosts();
