...
export function AssignmentDetailScreen(props: any) {
  ...
  const { items: groupMeta } = useAsyncList<Group>(async () => {
  ...
  const { items: myMemberRows } = useAsyncList<{ role?: string }>(async () => {
    if (isFirebaseMockMode()) return [];
    if (!groupId) return [];
    if (!auth.user) return [];
  ...
  const canManageCourse =
    group?.type === 'course' &&
    (myRole === 'owner' || myRole === 'instructor' || myRole === 'moderator');
  ...
  const { items: userProfiles } = useAsyncList<UserProfile>(async () => {
    ...
    return fetchSchoolDirectoryProfiles(school.id, [...new Set(uids)], db);
  }, [
    db,
    school.id,
    canManageCourse,
    submissions.map((s) => s.uid).join(','),
    myReviewTask?.submissionOwnerId,
  ]);
  ...
  const onSubmit = async () => {
    ...
    if (isLate && !assignment?.allowLate) {
      Alert.alert('截止時間已過', '此作業不允許遲交，是否仍要繳交？', [
  ...
  const onWithdraw = () => {
    ...
    Alert.alert('確認撤回', '撤回後作業將回到草稿狀態，您可以修改後重新繳交。確定要撤回嗎？', [
      { text: '取消', style: 'cancel' },
      {
        text: '確認撤回',
        style: 'destructive',
  ...
  const setGradesPublished = async (next: boolean) => {
    ...
    try {
      await setDoc(
        doc(db, 'groups', groupId, 'assignments', assignmentId),
        { gradesPublished: next },
        { merge: true },
      );
      reloadAssignment();
    } catch (e: any) {
  ...
  if (!groupId || !assignmentId)
    return <ErrorState title="作業" subtitle="缺少 groupId 或 assignmentId" />;
  if (assignmentLoading) return <LoadingState title="作業" subtitle="載入中..." rows={2} />;
  if (assignmentError)
    return <ErrorState title="作業" subtitle="讀取作業失敗" hint={assignmentError} />;
  ...
  if (!assignment) {
    return (
      <ErrorState
        ...
        hint={
          isFirebaseMockMode()
            ? '此功能需要 Firebase 連線。請到「課程」Tab 查看 TronClass 作業。'
            : '發生未知錯誤，請重試。'
        }
        actionText="返回"
        onAction={() => nav?.goBack?.()}
  ...
  const dueDate =
    assignment.dueAt?.toDate?.() ?? (assignment.dueAt ? new Date(assignment.dueAt) : null);
      ...
      >
        ...
        <AnimatedCard title={assignment.title} subtitle={group?.name}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            ...
            />
            <Pill
              text={canManageCourse ? '教師' : '學生'}
              kind={canManageCourse ? 'accent' : 'default'}
            />
            {mySubmission &&
        ...
        <AnimatedCard
          ...
          subtitle={
            mySubmission
              ? `最後更新：${mySubmission.updatedAt?.toDate?.()?.toLocaleString?.() ?? ''}`
              : '尚未繳交'
          }
          delay={50}
        >
            ...
            <Button
              text={
                submitting
                  ? '繳交中...'
                  : auth.user
                    ? mySubmission && !mySubmission.isDraft
                      ? '更新繳交'
                      : '送出作業'
                    : '請先登入'
              }
          ...
          {mySubmission && (
            <View
              style={{
                ...
                borderLeftWidth: 3,
                borderLeftColor:
                  canSeeMyGrade && typeof mySubmission.grade === 'number'
        ...
        {canManageCourse && (
          <AnimatedCard title="全部繳交" subtitle={`教師評分區 · 共 ${stats.total} 份`} delay={100}>
            {/* Stats Section */}
            <Pressable onPress={() => setShowStats(!showStats)}>
              ...
              >
                <Ionicons
                  name={showStats ? 'chevron-down' : 'chevron-forward'}
                  size={16}
            ...
            {showStats && (
              ...
              >
                <View style={{ flex: 1, minWidth: 80, alignItems: 'center' }}>
                  <ProgressRing
                    progress={stats.total > 0 ? stats.graded / stats.total : 0}
                    size={50}
                  />
            ...
            {stats.ungraded > 0 && (
              <View style={{ marginTop: 12 }}>
                <Button
                  text={`批量評分 (${stats.ungraded} 份)`}
                  kind="secondary"
                  onPress={onBatchGrade}
                />
              </View>
            )}
            ...
            {submissionsLoading ? (
              ...
              <ErrorState
                title="繳交"
                subtitle="讀取繳交失敗"
                hint={submissionsError}
                actionText="重試"
                onAction={reloadSubmissions}
              />
            ) : (
                      ...
                      >
                        ...
                        {s.links && s.links.length > 0 && (
                          <View style={{ marginBottom: 10 }}>
                            <Text
                              style={{ color: theme.colors.muted, fontSize: 11, marginBottom: 6 }}
                            >
                              附加連結：
                            </Text>
                            {s.links.map((l, i) => (
        ...
        {canManageCourse && submissions.length > 0 && (
          <AnimatedCard
            title="同儕互評系統"
            subtitle={
              peerReviewEnabled ? '已開啟互評，學生正在進行評審' : '開啟後系統將隨機分配評審任務'
            }
          >
            {!peerReviewEnabled ? (
              <View style={{ gap: 12 }}>
            ...
            {peerReviewSent ? (
              <View style={{ gap: 8 }}>
                ...
                {myAggregateScore !== null && (
                  ...
                  >
                    <Text style={{ color: theme.colors.muted, fontSize: 12 }}>
                      你收到的同儕互評結果（{peerReviewReceivedCount} 人評分）
                    </Text>
              ...
              <View style={{ gap: 12 }}>
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.text, fontWeight: '700', marginTop: 4 }}>
                    {profilesById[myReviewTask.submissionOwnerId]?.displayName ??
                      myReviewTask.submissionOwnerId.slice(0, 8)}
                  </Text>
