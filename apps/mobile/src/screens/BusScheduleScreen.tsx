...
const MOCK_ROUTES: BusRoute[] = [
  {
    ...
    description: '台中車站 → 靜宜大學 → 清水 (台灣大道)',
    frequency: '約 5-10 分鐘一班',
    ...
    scheduleTimes: {
      ...
      weekend: ['06:00', '06:15', '06:30', '06:45', '07:00', '07:15', '07:30', '07:45'],
    },
    stops: [
      { id: 's1', name: '台中車站(台灣大道)', arrivalTime: 0 },
      { id: 's2', name: '秋紅谷', arrivalTime: 15 },
      { id: 's3', name: '東海大學', arrivalTime: 25 },
      { id: 's4', name: '弘光科技大學', arrivalTime: 35 },
      { id: 's5', name: '靜宜大學', arrivalTime: 45, isCurrentLocation: true },
      { id: 's6', name: '沙鹿', arrivalTime: 50 },
    ],
  },
  {
    id: 'route2',
    ...
    scheduleTimes: {
      ...
      weekend: ['07:00', '07:20', '07:40', '08:00', '08:20', '08:40', '09:00'],
    },
    stops: [
      { id: 's7', name: '新民高中', arrivalTime: 0 },
      { id: 's8', name: '台中車站(台灣大道)', arrivalTime: 8 },
      { id: 's9', name: '靜宜大學', arrivalTime: 50, isCurrentLocation: true },
    ],
  },
  {
    id: 'route3',
    ...
    scheduleTimes: {
      ...
      weekend: ['07:00', '07:30', '08:00', '08:30', '09:00', '09:30'],
    },
    stops: [
      { id: 's10', name: '沙鹿火車站', arrivalTime: 0 },
      { id: 's11', name: '靜宜大學', arrivalTime: 8, isCurrentLocation: true },
      { id: 's12', name: '清水車站', arrivalTime: 20 },
    ],
  },
];
...
function formatOperatingHours(route: DataBusRoute): string {
  ...
  if (source && typeof source === 'object') {
    ...
    const times = [...weekday, ...weekend].filter(
...
function normalizeScheduleTimes(route: DataBusRoute): { weekday: string[]; weekend: string[] } {
  ...
  if (route.schedule && typeof route.schedule === 'object') {
    const weekday = Array.isArray(route.schedule.weekday)
      ? route.schedule.weekday.map(normalizeTimeValue)
      : [];
    const weekend = Array.isArray(route.schedule.weekend)
      ? route.schedule.weekend.map(normalizeTimeValue)
      : [];
    return {
      weekday: weekday.length > 0 ? weekday : weekend,
      weekend: weekend.length > 0 ? weekend : weekday,
...
export function BusScheduleScreen(props: any) {
      ...
      if (dataRoutes && dataRoutes.length > 0) {
        ...
        latestArrivals.forEach((arrivals, stopId) => {
          arrivals.forEach((arrival) => {
            const vehicleId =
              arrival.vehicleId ?? arrival.busId ?? `v-${arrival.routeId}-${stopId}`;
            if (!vehiclesFromArrivals.find((v) => v.id === vehicleId)) {
              vehiclesFromArrivals.push({
                ...
                eta:
                  typeof (arrival as any).estimatedMinutes === 'number'
                    ? (arrival as any).estimatedMinutes
                    : Math.max(
                        0,
                        Math.round(
                          (new Date(arrival.estimatedArrival ?? Date.now()).getTime() -
                            Date.now()) /
                            60000,
                        ),
                      ),
  ...
  const toggleFavoriteStop = (stopId: string) => {
    setFavoriteStops((prev) => {
      ...
      analytics.logEvent('bus_favorite_toggle', {
        stop_id: stopId,
        action: prev.includes(stopId) ? 'remove' : 'add',
      });
      return newFavorites;
    });
  };
  ...
  const handleScheduleReminder = useCallback(
    async (route: BusRoute, vehicle: BusVehicle) => {
      ...
      try {
        ...
        setReminders(nextReminders);
        await setStoredReminders(nextReminders);
        analytics.logEvent('bus_reminder_set', {
          route_id: route.id,
      ...
      >
        ...
        {currentRoute && viewMode === 'realtime' && (
          <>
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                            ...
                            >
                              <Ionicons
                                name={crowdInfo.icon as any}
                                size={12}
                                color={crowdInfo.color}
                              />
                              <Text
                                style={{ color: crowdInfo.color, fontSize: 11, fontWeight: '600' }}
                              >
                                {crowdInfo.text}
                              </Text>
                            </View>
                        ...
                        <View style={{ alignItems: 'flex-end' }}>
                          <Text
                            style={{ color: theme.colors.accent, fontWeight: '900', fontSize: 24 }}
                          >
                            {vehicle.eta}
                          </Text>
            ...
            <AnimatedCard
              title="站點列表"
              subtitle={`共 ${currentRoute.stops.length} 站`}
              delay={200}
            >
        ...
        {currentRoute && viewMode === 'schedule' && (
          <AnimatedCard
            title="發車時刻表"
            subtitle={`${currentRoute.name} - ${currentRoute.operatingHours}`}
          >
            <SegmentedControl
              options={[
            ...
            <View style={{ gap: 8 }}>
              ...
              >
                <Text
                  style={{ flex: 1, color: theme.colors.muted, fontWeight: '600', fontSize: 13 }}
                >
                  發車時間
                </Text>
                <Text
              ...
              {visibleScheduleTimes.map((time) => {
                ...
                const isNext =
                  scheduleMinute !== null &&
                  scheduleMinute >= nowMinutes &&
                  !visibleScheduleTimes.some((other) => {
                    const otherMinutes = parseTimeToMinutes(other);
                    return (
                      other !== time &&
                      otherMinutes !== null &&
                      otherMinutes >= nowMinutes &&
                      otherMinutes < scheduleMinute
                    );
                  });
                return (
                  <View
            ...
            {scheduleTimes.length > 10 && (
              ...
              >
                <Text style={{ color: theme.colors.accent, fontWeight: '600' }}>
                  {showFullSchedule
                    ? '收合時刻表'
                    : `查看完整時刻表（共 ${scheduleTimes.length} 班）`}
                </Text>
              </Pressable>
            )}
          </AnimatedCard>
