...
export async function getNextClassData(
  ...
  if (dayOfWeek === 0 || dayOfWeek === 6) {
    ...
    const nextStartTime =
      currentHour < 8 ? '08:10' : currentHour < 10 ? '10:20' : currentHour < 14 ? '14:10' : '16:20';
...
export async function getCafeteriaMenuData(
  ...
  const cacheKey = makeWidgetCacheKey('cafeteriaMenu', {
    schoolId,
    extraKey: cafeteriaId || 'default',
  });
...
export async function getLibraryData(userId: string, schoolId: string): Promise<LibraryWidgetData> {
...
export async function getAnnouncementData(schoolId: string): Promise<AnnouncementWidgetData> {
...
export async function getGradesData(userId: string, schoolId: string): Promise<GradesWidgetData> {
...
export async function getLostFoundData(schoolId: string): Promise<LostFoundWidgetData> {
