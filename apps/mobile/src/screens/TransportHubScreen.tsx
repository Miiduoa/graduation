...
function buildLeafletHtml(opts: {
  ...
  const { center, zoom, origin, destination, routeGeometry, routeColor = "#3b82f6",
    transitSegments, userLocation, isDark } = opts;
  ...
  // 大眾運輸各段
  const transitCode = (transitSegments ?? []).map((seg, i) => {
...
.origin-dot{width:14px;height:14px;border-radius:50%;background:#3b82f6;border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.3)}
  ...
  try{
    ...
    if(d.type==='updateRoute'){
      ...
      if(d.coords&&d.coords.length>1){
        ...
        L.polyline(d.coords,{color:d.color||'#3b82f6',weight:5,opacity:0.9,lineCap:'round',lineJoin:'round'}).addTo(map);
...
function openExternalMap(from: LatLng, to: LatLng, destName: string, mode: string = "walking") {
  ...
  Linking.openURL(scheme).catch(() => {
    ...
    Linking.openURL(`https://www.google.com/maps/dir/?api=1&origin=${from.lat},${from.lng}&destination=${to.lat},${to.lng}&travelmode=${mode === "cycling" ? "bicycling" : mode}`);
  });
}
...
const MODE_CONFIG: Record<string, { icon: string; color: string; label: string }> = {
  driving: { icon: "car-outline", color: "#8b5cf6", label: "開車" },
  cycling: { icon: "bicycle-outline", color: "#f59e0b", label: "騎車" },
  walking: { icon: "walk-outline", color: "#10b981", label: "步行" },
  transit: { icon: "bus-outline", color: "#3b82f6", label: "大眾運輸" },
};
...
export function TransportHubScreen(props: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pos?.latitude && pos?.longitude && isInTaiwan(pos.latitude, pos.longitude)) {
        ...
        console.log(`[Nav] GPS not in Taiwan (${pos?.latitude},${pos?.longitude}), using PU default`);
  ...
  // 選擇目的地 → 規劃路線
  const selectDestination = useCallback(async (result: SearchResult) => {
    ...
    try {
      console.log(`[Nav] Planning route: origin=(${origin.lat},${origin.lng}) → dest=(${dest.lat},${dest.lng})`);
  ...
  const startNavigation = useCallback(() => {
    ...
    navIntervalRef.current = setInterval(async () => {
      const currentPos = geo.latitude && geo.longitude
        ? { lat: geo.latitude, lng: geo.longitude }
        : origin;

      // 更新即時 ETA
      if (activeRoute) {
        const eta = calculateLiveETA(
          currentPos,
          destination,
          activeRoute,
          geo.speed,
  ...
  const routeColor = useMemo(() => {
    ...
    return MODE_CONFIG[activeRoute.mode]?.color ?? "#3b82f6";
  ...
  const transitSegments = useMemo(() => {
    ...
    return activeRoute.transitDetails
      ...
      .map(td => ({
        ...
        color: td.type === "bus" ? "#3b82f6" : td.type === "train" ? "#8b5cf6" : "#10b981",
        dash: td.type === "walk",
      }));
  }, [activeRoute]);
  ...
  }, [origin, destination, activeRoute?.id, routeColor, transitSegments, geo.latitude, geo.longitude]);
          ...
          <View style={{ height: MAP_HEIGHT, backgroundColor: theme.colors.surface2 }}>
              ...
              <View style={{
                ...
                <View>
                  ...
                  <Text style={{ color: "rgba(255,255,255,0.85)", fontSize: 11 }}>
                    {congestionLevel === "heavy" ? "路況壅塞" : congestionLevel === "moderate" ? "路況尚可" : "路況順暢"} · {formatDistance(activeRoute.totalDistance)}
                  </Text>
                </View>
                <Pressable onPress={stopNavigation}
                  style={{ backgroundColor: "rgba(255,255,255,0.25)", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 }}>
            ...
            {rerouteMsg && (
              <View style={{
                ...
                backgroundColor: "#3b82f6F0", borderRadius: theme.radius.lg,
                padding: 12, flexDirection: "row", alignItems: "center", gap: 10,
              }}>
                <Ionicons name="flash" size={20} color="#fff" />
                <View style={{ flex: 1 }}>
                  <Text style={{ color: "#fff", fontWeight: "700", fontSize: 13 }}>AI 智慧改道</Text>
                  <Text style={{ color: "rgba(255,255,255,0.85)", fontSize: 11 }}>{rerouteMsg}</Text>
                </View>
            ...
            {destination && activeRoute && !navMode && (
              <View style={{ position: "absolute", bottom: 10, right: 12, flexDirection: "row", gap: 8 }}>
                <Pressable
                  onPress={startNavigation}
                  style={({ pressed }) => ({
                    flexDirection: "row", alignItems: "center", gap: 6,
                    backgroundColor: "#10b981", paddingHorizontal: 14, paddingVertical: 8,
                    borderRadius: theme.radius.xl, opacity: pressed ? 0.8 : 1,
                ...
                <Pressable
                  ...
                  style={({ pressed }) => ({
                    flexDirection: "row", alignItems: "center", gap: 6,
                    backgroundColor: theme.colors.accent, paddingHorizontal: 14, paddingVertical: 8,
                    borderRadius: theme.radius.xl, opacity: pressed ? 0.8 : 1,
                    shadowColor: "#000", shadowOpacity: 0.2, shadowRadius: 6,
                    shadowOffset: { width: 0, height: 2 }, elevation: 4,
                  })}
                ...
                <View style={{
                  ...
                  {searching ? (
                    <View style={{ padding: 16, alignItems: "center" }}>
                      ...
                      <Text style={{ color: theme.colors.muted, marginTop: 6, fontSize: 12 }}>搜尋中...</Text>
                    </View>
                  ) : searchResults.length === 0 ? (
                    <View style={{ padding: 16, alignItems: "center" }}>
                      <Text style={{ color: theme.colors.muted, fontSize: 13 }}>找不到結果，請嘗試其他關鍵字</Text>
                    </View>
                  ) : (
            ...
            {planError && !planning && routes.length === 0 && destination && (
              <View style={{ padding: 20, alignItems: "center", gap: 10 }}>
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  <Button text="重新規劃" kind="primary" onPress={() => {
                    setPlanError(false);
                    selectDestination({ placeId: "", displayName: destName, shortName: destName, lat: destination.lat, lng: destination.lng, type: "", importance: 0 });
                  }} />
              ...
              <View style={{ padding: 12, gap: 8 }}>
                        ...
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                          ...
                          {route.congestionScore !== undefined && route.congestionScore > 0.2 && (
                            <View style={{
                              ...
                              backgroundColor: route.congestionScore > 0.5 ? "#f43f5e20" : "#f59e0b20",
                              paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4,
                            }}>
                              <View style={{
                                width: 6, height: 6, borderRadius: 3,
                                backgroundColor: route.congestionScore > 0.5 ? "#f43f5e" : "#f59e0b",
                              }} />
                              <Text style={{
                                color: route.congestionScore > 0.5 ? "#f43f5e" : "#f59e0b",
                          ...
                          <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                            {route.transitDetails.map((td, i) => (
                              <React.Fragment key={i}>
                                {i > 0 && <Ionicons name="chevron-forward" size={12} color={theme.colors.muted} />}
                                {td.type === "walk" ? (
                                  <View style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
                                    <Ionicons name="walk-outline" size={14} color={theme.colors.muted} />
                                  ...
                                  <View style={{
                                    ...
                                    backgroundColor: "#3b82f620", paddingHorizontal: 6, paddingVertical: 2,
                                    borderRadius: 4,
                                  }}>
                                    ...
                                    {td.estimateMinutes !== undefined && (
                                      <Text style={{ color: "#3b82f6", fontSize: 10 }}>
                ...
                {activeRoute && (
                  <View style={{ flexDirection: "row", gap: 8 }}>
                    ...
                    >
                      <Ionicons name={showSteps ? "chevron-up" : "list-outline"} size={16} color={theme.colors.accent} />
                    ...
                    {!navMode && (
                      <Pressable
                        ...
                        style={({ pressed }) => ({
                          flex: 1, flexDirection: "row", alignItems: "center",
                          justifyContent: "center", paddingVertical: 10, gap: 6,
                          backgroundColor: "#10b981", borderRadius: theme.radius.lg,
                          opacity: pressed ? 0.7 : 1,
                        })}
                      >
                        <Ionicons name="navigate" size={16} color="#fff" />
                        <Text style={{ color: "#fff", fontWeight: "700", fontSize: 13 }}>AI 智慧導航</Text>
                      </Pressable>
                    )}
                  </View>
                )}
                ...
                {showSteps && activeRoute && (
                  <View style={{
                    ...
                    {activeRoute.steps.map((step, i) => {
                      ...
                      const stepIcon = step.maneuver === "depart" ? "radio-button-on"
                        : step.maneuver === "arrive" ? "flag"
                        : step.maneuver === "notification" ? "bus"
                        : step.instruction.includes("左") ? "arrow-back"
                        : step.instruction.includes("右") ? "arrow-forward"
                        : "arrow-up";
                      ...
                      return (
                        ...
                        >
                          ...
                          <View style={{ alignItems: "center", width: 28 }}>
                            <View style={{
                              ...
                              <Ionicons
                                ...
                                size={14}
                                color={isFirst || isLast || step.maneuver === "notification" ? "#fff" : theme.colors.text}
                              />
                            </View>
                          </View>
                          ...
                          {/* 指示文字 */}
                          <View style={{ flex: 1 }}>
                            <Text style={{ color: theme.colors.text, fontSize: 13, fontWeight: "600", lineHeight: 19 }}>
                              {step.instruction}
                            </Text>
                            {step.distance > 0 && (
              ...
              <View style={{ padding: 12, gap: 12 }}>
                ...
                >
                  {geo.loading ? (
                    ...
                    <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: geo.latitude ? "#10b981" : "#f59e0b" }} />
                  )}
...
function QuickBusPanel() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try { setEstimates(await getBusEstimates(selectedRoute)); } catch {}
    }, 30000);
    return () => clearInterval(iv);
  }, [selectedRoute]);

  const currentStops = useMemo(() => stops.find(s => s.Direction === direction)?.Stops ?? [], [stops, direction]);
    ...
    <Card title="公車即時到站">
      <View style={{ flexDirection: "row", gap: 8, marginBottom: 10 }}>
        <View style={{
          ...
          backgroundColor: theme.colors.surface, paddingHorizontal: 10, minHeight: 40,
        }}>
          <Ionicons name="search-outline" size={16} color={theme.colors.muted} />
          <TextInput
            value={searchText} onChangeText={setSearchText}
            placeholder="路線號碼（如 300）" placeholderTextColor={theme.colors.muted}
      ...
      {!selectedRoute && results.length === 0 && (
        <View style={{ gap: 4 }}>
          <Text style={{ color: theme.colors.muted, fontSize: 11, marginBottom: 4 }}>靜宜周邊常用路線</Text>
          {PU_COMMON_BUS_ROUTES.slice(0, 6).map(r => (
            <Pressable key={r.id} onPress={() => selectRoute(r.id)}
              style={({ pressed }) => ({ flexDirection: "row", alignItems: "center", padding: 8, borderRadius: theme.radius.md, backgroundColor: theme.colors.surface2, opacity: pressed ? 0.7 : 1, marginBottom: 2 })}>
              <View style={{ width: 40, height: 24, borderRadius: 4, backgroundColor: "#3b82f6", alignItems: "center", justifyContent: "center", marginRight: 8 }}>
                <Text style={{ color: "#fff", fontWeight: "800", fontSize: 11 }}>{r.name}</Text>
              </View>
              <Text style={{ color: theme.colors.text, fontSize: 11, flex: 1 }} numberOfLines={1}>{r.desc}</Text>
            </Pressable>
          ))}
        </View>
      )}
      ...
      {results.length > 0 && !selectedRoute && (
        <View style={{ gap: 4 }}>
          {results.slice(0, 8).map(r => (
            <Pressable key={r.RouteUID} onPress={() => selectRoute(r.RouteID)}
              style={({ pressed }) => ({ flexDirection: "row", alignItems: "center", padding: 8, borderRadius: theme.radius.md, backgroundColor: theme.colors.surface2, opacity: pressed ? 0.7 : 1 })}>
              <View style={{ width: 40, height: 24, borderRadius: 4, backgroundColor: theme.colors.accent, alignItems: "center", justifyContent: "center", marginRight: 8 }}>
                <Text style={{ color: "#fff", fontWeight: "800", fontSize: 11 }}>{r.RouteName.Zh_tw}</Text>
              </View>
        ...
        <>
          {stops.length > 1 && (
            <View style={{ flexDirection: "row", gap: 6, marginBottom: 8 }}>
              {stops.map(d => (
                <Pressable key={d.Direction} onPress={() => setDirection(d.Direction)}
                  style={{ flex: 1, paddingVertical: 6, borderRadius: theme.radius.md, backgroundColor: direction === d.Direction ? theme.colors.accent : theme.colors.surface2, alignItems: "center" }}>
                  <Text style={{ color: direction === d.Direction ? "#fff" : theme.colors.text, fontWeight: "600", fontSize: 11 }}>
                    {d.Direction === 0 ? "去程" : "返程"}
                  </Text>
                </Pressable>
              ))}
          ...
          {loadingStops ? (
            ...
            <View style={{ gap: 1, maxHeight: 300 }}>
              {currentStops.map((stop, i) => {
                ...
                return (
                  <View key={stop.StopUID} style={{ flexDirection: "row", alignItems: "center", paddingVertical: 6, paddingHorizontal: 4, borderBottomWidth: i < currentStops.length - 1 ? 1 : 0, borderBottomColor: theme.colors.surface2 }}>
                    <View style={{ width: 18, height: 18, borderRadius: 9, backgroundColor: hasETA ? theme.colors.accent : theme.colors.surface2, alignItems: "center", justifyContent: "center", marginRight: 8 }}>
                      <Text style={{ color: hasETA ? "#fff" : theme.colors.muted, fontSize: 8, fontWeight: "700" }}>{stop.StopSequence}</Text>
                    </View>
                    <Text style={{ flex: 1, color: theme.colors.text, fontSize: 12 }}>{stop.StopName.Zh_tw}</Text>
                    <Text style={{ color: hasETA ? ((eta?.EstimateTime ?? 0) < 180 ? "#f43f5e" : theme.colors.accent) : theme.colors.muted, fontWeight: "700", fontSize: 12, minWidth: 50, textAlign: "right" }}>
                      {hasETA ? (eta!.EstimateTime! < 60 ? "即將到站" : `${Math.ceil(eta!.EstimateTime! / 60)} 分`) : eta?.StopStatus === 3 ? "末班已過" : "—"}
                    </Text>
                  </View>
                );
              })}
          ...
          <Pressable onPress={() => { setSelectedRoute(null); setStops([]); setEstimates([]); }}
            style={{ marginTop: 8, alignItems: "center" }}>
            <Text style={{ color: theme.colors.accent, fontSize: 12, fontWeight: "600" }}>← 返回路線列表</Text>
          </Pressable>
        </>
      )}
    </Card>
...
function QuickTrainPanel() {
  ...
  useEffect(() => {
    setLoading(true);
    getTrainSchedule(station).then(setData).catch(() => setData([])).finally(() => setLoading(false));
  }, [station]);

  const now = new Date();
  const nowStr = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
  const upcoming = useMemo(() => data.filter(t => {
  ...
  }).slice(0, 12), [data, station, nowStr]);

  return (
    <Card title="台鐵時刻表">
      <View style={{ flexDirection: "row", gap: 6, marginBottom: 10 }}>
        {PU_NEARBY_TRAIN_STATIONS.map(s => (
          <Pressable key={s.id} onPress={() => setStation(s.id)}
            style={{ flex: 1, paddingVertical: 6, borderRadius: theme.radius.md, backgroundColor: station === s.id ? theme.colors.accent : theme.colors.surface2, alignItems: "center" }}>
            <Text style={{ color: station === s.id ? "#fff" : theme.colors.text, fontWeight: "700", fontSize: 12 }}>{s.name}</Text>
            <Text style={{ color: station === s.id ? "rgba(255,255,255,0.7)" : theme.colors.muted, fontSize: 9 }}>{s.distance}</Text>
          </Pressable>
        ))}
      </View>
      {loading ? <ActivityIndicator color={theme.colors.accent} /> : upcoming.length === 0 ? (
        <Text style={{ color: theme.colors.muted, textAlign: "center", padding: 12 }}>今日無更多班次</Text>
      ) : (
        <View style={{ gap: 2 }}>
          {upcoming.map(t => {
            const s = t.StopTimes?.find(s => s.StationID === station);
            if (!s) return null;
            return (
              <View key={t.DailyTrainInfo.TrainNo} style={{ flexDirection: "row", alignItems: "center", paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: theme.colors.surface2 }}>
                <Text style={{ width: 44, color: theme.colors.text, fontSize: 12, fontWeight: "600" }}>{t.DailyTrainInfo.TrainNo}</Text>
                <Text style={{ width: 50, color: theme.colors.muted, fontSize: 10 }}>{t.DailyTrainInfo.TrainTypeName?.Zh_tw ?? ""}</Text>
                <Text style={{ width: 44, color: theme.colors.accent, fontSize: 13, fontWeight: "700" }}>{s.DepartureTime.substring(0, 5)}</Text>
                <Text style={{ flex: 1, color: theme.colors.muted, fontSize: 11, textAlign: "right" }}>{t.DailyTrainInfo.EndingStationName?.Zh_tw ?? ""}</Text>
              </View>
            );
          })}
        </View>
...
function QuickHSRPanel() {
  ...
  useEffect(() => {
    setLoading(true);
    getHSRSchedule().then(setData).catch(() => setData([])).finally(() => setLoading(false));
  }, []);
  ...
  const upcoming = useMemo(() => data.filter(t => {
    ...
    const as = a.StopTimes?.find(s => s.StationID === HSR_TAICHUNG_STATION_ID)?.DepartureTime ?? "";
    const bs = b.StopTimes?.find(s => s.StationID === HSR_TAICHUNG_STATION_ID)?.DepartureTime ?? "";
    return as.localeCompare(bs);
  }).slice(0, 12), [data, dir, nowStr]);

  return (
    <Card title="高鐵台中站">
      <View style={{ flexDirection: "row", gap: 6, marginBottom: 10 }}>
        {[{ v: null, l: "全部" }, { v: 0, l: "南下" }, { v: 1, l: "北上" }].map(o => (
          <Pressable key={String(o.v)} onPress={() => setDir(o.v)}
            style={{ flex: 1, paddingVertical: 6, borderRadius: theme.radius.md, backgroundColor: dir === o.v ? theme.colors.accent : theme.colors.surface2, alignItems: "center" }}>
            <Text style={{ color: dir === o.v ? "#fff" : theme.colors.text, fontWeight: "600", fontSize: 12 }}>{o.l}</Text>
          </Pressable>
        ))}
      </View>
      {loading ? <ActivityIndicator color={theme.colors.accent} /> : upcoming.length === 0 ? (
        ...
        <View style={{ gap: 2 }}>
          {upcoming.map(t => {
            ...
            return (
              <View key={t.DailyTrainInfo.TrainNo} style={{ flexDirection: "row", alignItems: "center", paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: theme.colors.surface2 }}>
                <Text style={{ width: 40, color: theme.colors.text, fontSize: 12, fontWeight: "600" }}>{t.DailyTrainInfo.TrainNo}</Text>
                <View style={{ width: 36, alignItems: "center" }}>
                  <Text style={{ color: isSouth ? "#3b82f6" : "#f59e0b", fontSize: 10, fontWeight: "700" }}>{isSouth ? "南下" : "北上"}</Text>
                </View>
                <Text style={{ width: 44, color: theme.colors.accent, fontSize: 13, fontWeight: "700" }}>{s.DepartureTime.substring(0, 5)}</Text>
                <Text style={{ flex: 1, color: theme.colors.muted, fontSize: 11, textAlign: "right" }}>{t.DailyTrainInfo.EndingStationName?.Zh_tw ?? ""}</Text>
              </View>
...
function QuickBikePanel() {
  ...
  useEffect(() => {
    setLoading(true);
    getNearbyBikesWithAvailability().then(setStations).catch(() => setStations([])).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <Card title="YouBike 站點" subtitle="靜宜 2km 內">
      <View style={{ flexDirection: "row", gap: 8, marginBottom: 10 }}>
        <View style={{ flex: 1, padding: 10, borderRadius: theme.radius.md, backgroundColor: theme.colors.accentSoft, alignItems: "center" }}>
          <Text style={{ color: theme.colors.accent, fontWeight: "900", fontSize: 20 }}>{total}</Text>
          <Text style={{ color: theme.colors.muted, fontSize: 10 }}>可借車輛</Text>
        </View>
        <View style={{ flex: 1, padding: 10, borderRadius: theme.radius.md, backgroundColor: theme.colors.surface2, alignItems: "center" }}>
          <Text style={{ color: theme.colors.text, fontWeight: "900", fontSize: 20 }}>{stations.length}</Text>
      ...
      {loading ? <ActivityIndicator color={theme.colors.accent} /> : (
        <View style={{ gap: 4 }}>
          {stations.slice(0, 10).map(st => (
            <View key={st.StationUID} style={{ flexDirection: "row", alignItems: "center", paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: theme.colors.surface2 }}>
              <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: st.availableRent === 0 ? "#f43f5e20" : theme.colors.accentSoft, alignItems: "center", justifyContent: "center", marginRight: 8 }}>
                <Text style={{ color: st.availableRent === 0 ? "#f43f5e" : theme.colors.accent, fontWeight: "900", fontSize: 13 }}>{st.availableRent}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontSize: 12, fontWeight: "600" }} numberOfLines={1}>{st.StationName.Zh_tw}</Text>
                <Text style={{ color: theme.colors.muted, fontSize: 10 }}>可借 {st.availableRent} · 可還 {st.availableReturn}{st.hasElectric ? ` · 電輔 ${st.electricBikes}` : ""}</Text>
              </View>
            </View>
          ))}
        </View>
