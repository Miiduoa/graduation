...
import {} from /* Platform */ 'react-native';
...
class StorageService {
    ...
    for (let retryCount = 0; retryCount <= UPLOAD_RETRY_CONFIG.maxRetries; retryCount++) {
      try {
        ...
        if (response?.status === 200) {
          ...
          const body = safeJsonParse<{ url?: string; error?: string }>(response.body, {
            error: '無法解析伺服器回應',
          });
        ...
        if ((isServerError || isTooManyRequests) && retryCount < UPLOAD_RETRY_CONFIG.maxRetries) {
          ...
          console.log(
            `[Storage] Upload failed with ${statusCode}, retrying in ${delay}ms (attempt ${retryCount + 1})`,
          );
        ...
        // 嘗試解析錯誤訊息
        if (response?.body) {
          const errorBody = safeJsonParse<{ error?: string; message?: string }>(response.body, {});
        ...
        if (isRetryableUploadError(e) && retryCount < UPLOAD_RETRY_CONFIG.maxRetries) {
          const delay = getUploadRetryDelay(retryCount);
          console.log(
            `[Storage] Network error, retrying in ${delay}ms (attempt ${retryCount + 1})`,
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
          continue;
        }
  ...
  async downloadFile(
    ...
    try {
      ...
      const downloadResumable = FileSystem.createDownloadResumable(url, localUri, {}, (data) => {
        if (onProgress && data.totalBytesExpectedToWrite > 0) {
          onProgress({
            loaded: data.totalBytesWritten,
            total: data.totalBytesExpectedToWrite,
            percentage: Math.round((data.totalBytesWritten / data.totalBytesExpectedToWrite) * 100),
          });
        }
      });
