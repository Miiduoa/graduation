  ...
  return (
    ...
    >
      ...
      <View>
        ...
        {widget.id === 'grades' ? (
          <Text style={{ color: theme.colors.muted, fontSize: 11, marginTop: 2 }}>GPA 3.75</Text>
        ) : null}
      </View>
    </View>
  );
        ...
        <View style={{ gap: 8 }}>
          {[
            ...
            <View key={`${widget.id}-${announcement.title}`} style={{ gap: 4 }}>
              ...
              >
                <Text
                  style={{ color: theme.colors.text, fontWeight: '600', fontSize: 13, flex: 1 }}
                  numberOfLines={1}
                >
                  {announcement.title}
                </Text>
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ color: theme.colors.muted, fontSize: 12 }}>{widget.description}</Text>
        </View>
        <WidgetStatusBadge status={widget.status} />
      </View>
...
export function WidgetPreviewScreen() {
  const widgetsEnabled = isFeatureEnabled('widgets');
  ...
  const previewWidgets = useMemo(() => WIDGETS.filter((widget) => widget.status === 'preview'), []);
  ...
  if (!widgetsEnabled) {
    return (
      <Screen>
        ...
        >
          <AnimatedCard title="小工具尚未上線" subtitle="正式版目前未開通此功能">
            <Text style={{ color: theme.colors.muted, lineHeight: 22 }}>
              只有完成原生 Widget 驗收與商店送審準備的學校，才會在正式版顯示小工具入口。
            </Text>
          </AnimatedCard>
        </View>
      </Screen>
                  ...
                  {(['small', 'medium', 'large'] as const).map((size) => (
                    <Pressable
                      ...
                      style={{
                        ...
                        borderRadius: theme.radius.md,
                        backgroundColor:
                          selectedSize === size ? theme.colors.accent : theme.colors.surface2,
                        alignItems: 'center',
                      }}
                    >
                      <Text
            ...
            <View style={{ gap: 12, paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING }}>
              ...
              <AnimatedCard title="整合狀態" delay={150}>
                <View style={{ gap: 12 }}>
                  ...
                  <Text style={{ color: theme.colors.muted, fontSize: 12, lineHeight: 18 }}>
                    小工具要真正出現在主畫面，仍需跟著 iOS / Android 原生 target
                    一起建置；本頁主要用來確認版型與資訊優先順序。
                  </Text>
                </View>
              </AnimatedCard>
            </View>
