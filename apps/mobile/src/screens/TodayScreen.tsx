...
function StreakBadge({ days }: { days: number }) {
  ...
  return (
    ...
    >
      ...
      <Text style={{ color: '#FFF', fontSize: 12, fontWeight: '800' }}>{days} 天</Text>
    </LinearGradient>
  );
}
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, paddingBottom: props.isLast ? 0 : 4 }}>
        ...
        <Text
          style={{ color: theme.colors.textSecondary, fontSize: 13, marginTop: 3, lineHeight: 18 }}
        >
...
function QuickActionChip(props: {
  ...
  return (
    ...
    >
      ...
      <Text
        style={{ color: theme.colors.text, fontSize: 11, fontWeight: '700', letterSpacing: 0.2 }}
      >
        {props.label}
      </Text>
    </Pressable>
...
export function TodayScreen(props: Record<string, unknown>) {
  ...
  const handleNextActionPress = () => {
    ...
    if (
      (nextAction.kind === 'assignment' || nextAction.kind === 'quiz') &&
      nextAction.assignmentId
    ) {
      nav?.navigate?.('收件匣', {
                ...
                {todayCourses.map((course, i) => {
                  ...
                  return (
                    <CourseTimelineItem
                      ...
                      onPress={() =>
                        navigateToCourseHome(nav, auth.profile?.role, { initialTab: 'schedule' })
                      }
                    />
                  );
                })}
