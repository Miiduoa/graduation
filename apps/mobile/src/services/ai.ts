...
async function waitForRateLimit(): Promise<void> {
  ...
  const effectiveInterval =
    rateLimitState.minRequestIntervalMs * (1 + rateLimitState.consecutiveRateLimits);
...
export type AIContext = {
  ...
  gradesSummary?: {
    gpa?: number;
    courses: Array<{ name: string; grade?: number; credits?: number }>;
  };
  weeklyReport?: {
    summary: string;
    stats: { onTimeRate: number; totalSubmissions: number; newAchievements: number };
  };
...
function getConfig() {
  ...
  const rawProvider = String(
    extra.aiProvider ?? process.env.EXPO_PUBLIC_AI_PROVIDER ?? 'offline',
  ).toLowerCase();
  const rawWebSearch = extra.aiWebSearchEnabled ?? process.env.EXPO_PUBLIC_AI_ENABLE_WEB_SEARCH;
  const rawAppEnv = String(
    extra.appEnv ?? process.env.APP_ENV ?? process.env.EXPO_PUBLIC_API_ENV ?? '',
  ).toLowerCase();
  const isReleaseLike =
...
function isStrictRealDataMode(): boolean {
  ...
  return (
    forceFlag === true || String(forceFlag).toLowerCase() === 'true' || apiEnv === 'production'
  );
}
...
function formatCourseSummary(course: CourseContextItem): string {
  ...
  const meetingText =
    meetings.length > 0 ? meetings.map(formatCourseMeeting).join('；') : '時間未提供';
...
function buildSystemPrompt(context: AIContext): string {
  ...
  if (context.pendingAssignments && context.pendingAssignments.length > 0) {
    ...
    context.pendingAssignments.slice(0, 8).forEach((a, i) => {
      ...
      parts.push(
        `${i + 1}. ${a.title}（${a.groupName}，${dueStr}${a.isLate ? '，⚠️ 已逾期' : ''}）`,
      );
    });
  ...
  if (context.gradesSummary) {
    ...
    if (context.gradesSummary.courses.length > 0) {
      context.gradesSummary.courses.slice(0, 8).forEach((c, i) => {
        parts.push(
          `${i + 1}. ${c.name}${c.grade != null ? `：${c.grade} 分` : '（尚未公布）'}${c.credits ? `（${c.credits}學分）` : ''}`,
        );
      });
    }
  }
  ...
  if (context.weeklyReport) {
    ...
    parts.push(
      `準時繳交率：${s.onTimeRate}%，本週繳交 ${s.totalSubmissions} 份，新解鎖成就 ${s.newAchievements} 個`,
    );
  }
  ...
  if (context.appPulseSummary && context.appPulseSummary.length > 0) {
    ...
    parts.push(
      '請把這段視為目前使用者狀態與 App 最新資料總覽；回答時優先處理 high/critical、逾期、即將截止、擁擠、未讀或進行中的事項。',
    );
  }
...
function extractSuggestions(content: string): string[] {
  ...
  if (normalized.includes('餐') || normalized.includes('吃') || normalized.includes('菜單'))
    parsed.push('其他選擇');
  if (normalized.includes('地點') || normalized.includes('位置') || normalized.includes('怎麼走'))
    parsed.push('開啟導航');
...
const INTENT_PATTERNS: {
  ...
  patterns: RegExp[];
  keywords: string[];
  subIntentMap?: Record<string, string[]>;
}[] = [
  {
    category: 'food',
    patterns: [
...
async function mockAIResponse(
  ...
  const skipMockDelay =
    process.env.NODE_ENV === 'test' || process.env.EXPO_PUBLIC_AI_TEST_FAST === '1';
  if (!skipMockDelay) {
  ...
  if (intent.category === 'general') {
    ...
    const diningFollowUp =
      /便宜|平價|划算|省錢|其他|還有|別的|換|素食|蔬菜|健康|清淡|不辣|第[一二三四五六七八九十\d]+|編號|那道|這道|哪一道/.test(
        lowerMsg,
      );
    const lastWasDining =
      /餐|飯|麵|菜單|餐廳|餐點|吃|濟時|伯鐸|靜園|主顧|白鬍子|Morning House|飲料|吐司|蛋餅|鐵板麵|水果杯/.test(
        lastAssistant,
      );
    if (lastWasDining && diningFollowUp) {
      intent = {
        category: 'food',
        confidence: 3,
        subIntent: /便宜|平價|划算|省錢/.test(lowerMsg) ? 'budget' : 'recommend',
      };
    }
  }
  ...
  const forceOnlineSearch =
    config.webSearchEnabled && shouldUseWebSearch(lastMessage, intent.category);
  ...
  const unsafeOrDishonestRequest = /偽造|竄改|作弊|偷看|帳號密碼|密碼|駭|破解|繞過|入侵|盜用/.test(
    lowerMsg,
  );
  if (unsafeOrDishonestRequest) {
    return {
      thinking,
      content:
        '這個要求我不能協助，也不會提供偽造、偷看帳號、破解或繞過系統的方法。\n\n如果你是遇到帳號、成績、系統操作或申訴問題，我可以幫你整理正式詢問信、申訴草稿，或告訴你該找哪個校內單位。',
      suggestions: ['幫我寫正式詢問信', '帳號登入有問題', '成績申訴怎麼寫'],
    };
  }

  const wantsDraft = /草稿|幫我寫|寫一封|寫信|寫訊息|訊息跟|公告|email|mail|改寫|潤飾/.test(
    lowerMsg,
  );
  if (wantsDraft) {
  ...
  if (forceOnlineSearch) {
    ...
    if (webAnswer) {
      ...
      return {
        thinking: [
          ...
          {
            step: '連網搜尋',
            detail: `已查詢 ${webAnswer.sources.length} 個公開來源`,
            status: 'done',
          },
          { step: '證據整理', detail: '已把搜尋結果整理成結論、依據與來源', status: 'done' },
          {
            step: '本機學習',
            detail: '已存入來源可追溯的本地 web-learning 知識庫',
            status: 'done',
          },
        ],
    ...
    const learnedItem = await findRelevantWebLearningItem(lastMessage, { allowStale: true }).catch(
      () => null,
    );
    if (learnedItem) {
      ...
      return {
        thinking: [
          ...
          {
            step: '本機知識庫',
            detail: '改用先前已保存且有來源的 web-learning 資料',
            status: 'done',
          },
        ],
    ...
    return {
      ...
      content:
        '我有嘗試連網搜尋，但這次沒有取得足夠可靠的公開來源，所以不會亂編答案。\n\n你可以換一個更具體的關鍵字，或指定要查的網站/機構名稱。',
  ...
  switch (intent.category) {
    ...
    case 'food': {
      ...
      return {
        ...
        suggestions:
          subIntent === 'order' ? ['整理點餐內容'] : ['便宜一點的', '其他選擇', '有素食嗎'],
    ...
    case 'health': {
      ...
      if (wantsBooking) {
        return {
          thinking,
          content: `聽起來你身體不太舒服，我先幫你把就醫下一步放在前面。\n\n下一步：開啟校園健康功能，或到至善樓 1F 衛保組整理掛號資訊。\n\n衛保組門診時間：週一～五 09:00-12:00、13:30-16:30。離線模式不能直接送出掛號，但你可以補上科別、日期和症狀，我會整理成可送出的掛號草稿。若有胸痛、呼吸困難、昏倒、劇烈疼痛或快速惡化，請直接打 119；我不能替代醫師診斷。`,
      ...
      if (wantsCounseling) {
        return {
          thinking,
          content: `聽起來你最近承受不少壓力，我先把求助下一步放在前面。\n\n下一步：可以開啟校園健康功能，或到至善樓 2F 諮商輔導中心預約初談。\n\n初次諮商通常需要先填寫初談表，校內諮商服務免費且保密。在等候前，先找一位可信任的人說明你現在的狀態；如果出現傷害自己的念頭，請立刻聯絡 1925 或 119。`,
      ...
      return {
        thinking,
        content: `聽起來你身體不太舒服，我先幫你把就醫與校內資源放在前面。\n\n下一步：如果症狀持續或加劇，請到至善樓 1F 衛保組或校外醫療院所就醫。\n\n先休息、補充水分，避免自己騎車或獨自外出。衛保組門診時間：週一～五 09:00-16:30。若有胸痛、呼吸困難、昏倒、劇烈疼痛或快速惡化，請直接打 119；我不能替代醫師診斷。`,
    ...
    case 'course': {
      ...
      if (/成績|分數|gpa|排名|幾分/.test(lowerMsg)) {
        ...
        if (grades && grades.courses.length > 0) {
          const list = grades.courses
            .slice(0, 5)
            .map(
              (c, i) =>
                `${i + 1}. ${c.name}：${c.grade ?? '尚未公布'}${c.credits ? `（${c.credits}學分）` : ''}`,
            )
            .join('\n');
          return {
            thinking,
      ...
      if (/作業|截止|deadline|繳交|期限/.test(lowerMsg)) {
        if (assignments.length === 0) {
          return {
            thinking,
            content: '目前沒有待繳作業，太棒了！好好放鬆一下。',
            suggestions: ['推薦午餐', '查活動'],
          };
        }
        const list = assignments
          ...
          .map(
            (a, i) =>
              `${i + 1}. ${a.title}（${a.groupName}）${a.dueAt ? ` — 截止：${a.dueAt}` : ''}${a.isLate ? ' ⚠️ 已逾期' : ''}`,
          )
          .join('\n');
        return {
          thinking,
      ...
      if (/課表|什麼課|有課|幾點上課|幾堂課/.test(lowerMsg)) {
        if (courseList.length === 0) {
          return {
            thinking,
            content: '目前沒有載入課程資料。你可以在設定中同步課表！',
            suggestions: ['查公告'],
          };
        }
        const asksSpecificDay =
          /今天|明天|後天|星期|週|禮拜|有課|幾點上課|什麼課/.test(lowerMsg) &&
          !/查.*課表|我的課表|本學期/.test(lowerMsg);
        if (!asksSpecificDay) {
          const list = courseList
            .slice(0, 8)
            .map((c, i) => `${i + 1}. ${formatCourseSummary(c)}`)
            .join('\n');
          return {
            thinking,
        ...
        if (dayRows.length === 0) {
          return {
            thinking,
            content: `${targetDayName}沒有課！本學期共 ${courseList.length} 門課。要我幫你安排其他事嗎？`,
            suggestions: ['推薦午餐', '預約圖書館座位'],
          };
        }
        const list = dayRows
          .map(
    ...
    case 'location': {
      ...
      if (targetKw && allPois.length > 0) {
        const matches = allPois.filter(
          (p: any) => p.name.includes(targetKw) || (p.category && p.category.includes(targetKw)),
        );
        if (matches.length > 0) {
    ...
    case 'print': {
      if (/餘額|額度/.test(lowerMsg)) {
        return {
          thinking,
          content:
            '⚠️ 影印卡餘額需要在校園列印系統登入後才能查詢，我目前無法即時讀取。\n\n你可以到圖書館 1F 儲值機查詢與加值（接受現金和學生證綁定）。',
    ...
    case 'mood': {
      if (/壓力|焦慮|憂|煩|累|低落|難過/.test(lowerMsg)) {
        return {
          thinking,
          content: `聽起來你最近壓力不小，我先把能立刻做的事整理給你。\n\n下一步：可以開啟校園健康功能預約諮商，或先找一位可信任的人陪你聊一下。\n\n現在先把手邊事情停 5 分鐘、喝點水、到有人在的地方。如果這種狀態持續，學校有免費且保密的心理諮商服務（至善樓 2F）。若出現傷害自己的念頭，請立刻聯絡 1925 或 119。`,
    ...
    default: {
      ...
      const isQuestion = /[？?]|嗎|什麼|怎麼|哪裡|誰|幾|多少|為什麼|可以嗎|能不能|有沒有/.test(
        lowerMsg,
      );
      ...
      if (isQuestion) {
        ...
        if (hasAssignments)
          contextHints.push(`你有 ${context.pendingAssignments!.length} 項待繳作業`);
...
export async function extractDates(
  text: string,
): Promise<Array<{ date: string; description: string }>> {
