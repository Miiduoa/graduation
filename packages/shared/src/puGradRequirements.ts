...
const universityCore: DetailedCreditCategory = {
  ...
  notes:
    '通識涵養課程四大向度合計至少16學分：永續與在地4、宗教與思維2、科技與服務2、跨域與設計4；另有4學分可分配於任一向度',
};
...
const collegeRequired: DetailedCreditCategory = {
  ...
  notes:
    '資訊學院共同必修課程，含計算機概論一/二、資料結構、網路通訊概論、作業系統、生涯規劃及職場倫理講座',
};
...
const deptElective_CSIE: DetailedCreditCategory = {
  ...
  subCategories: [
    ...
    },
    {
      ...
      notes:
        '含網路管理、物聯網應用、AI、行動軟體、智慧資料分析、行動商務、文創動漫設計、企業實習、資訊安全、寰宇學習等課群',
    },
  ],
...
export function findPuGradTemplate(
  ...
  return puDetailedTemplates.find(
    (t) => t.departmentId === departmentId && (!academicYear || t.academicYear === academicYear),
  );
}
