...
function parseQRPayload(
  url: string,
): { type: string; data: Record<string, any>; isValid: boolean } | null {
  try {
    ...
    const expectedSig = generateSecureHash(
      JSON.stringify({
        v: payload.v,
        t: payload.t,
        u: payload.u,
...
export function QRCodeScreen(props: any) {
  ...
  const useResolvedCameraPermissions =
    useCameraPermissions ?? (() => [null, async () => null] as const);

  const [mode, setMode] = useState<QRMode>('generate');
  const [qrType, setQrType] = useState<QRType>('checkin');
  const [generatedQR, setGeneratedQR] = useState<string>('');
  const [scanResult, setScanResult] = useState<string | null>(null);
  const [storedScans, setStoredScans] = useAsyncStorage<
    Array<{ data: string; time: string; type: string }>
  >('qr_recent_scans', {
    defaultValue: [],
  });
  ...
  const regenerateQR = useCallback(() => {
    const customData =
      qrType === 'custom' && customContent ? { content: customContent } : undefined;
  ...
  const handleStartScan = async () => {
    if (!hasCameraSupport) {
      Alert.alert('相機功能', '真實相機掃描需要安裝 expo-camera。目前使用模擬模式。', [
        { text: '取消', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            setRecentScans((prev) => [
              { data: mockData, time: new Date(), type: '群組邀請' },
              ...prev.slice(0, 9),
            ]);
          },
        },
      ...
      >
        ...
        {mode === 'generate' ? (
          <>
            ...
            <AnimatedCard
              title="你的 QR 碼"
              subtitle={QR_TYPES.find((t) => t.key === qrType)?.label}
              delay={100}
            >
              ...
              <AnimatedCard title="掃描 QR 碼" subtitle="對準 QR 碼即可掃描">
                <View style={{ alignItems: 'center', padding: 20 }}>
                  ...
                  >
                    ...
                    {hasCameraSupport && (
                      <Text style={{ color: theme.colors.success, fontSize: 11, marginTop: 8 }}>
                        ✓ 真實相機可用
                      </Text>
                    )}
                  </Pressable>
                ...
                <View
                  style={{ flexDirection: 'row', gap: 8, justifyContent: 'center', marginTop: 8 }}
                >
                  <Pressable
                  ...
                  >
                    <Ionicons
                      ...
                      size={18}
                      color={flashOn ? theme.colors.accent : theme.colors.muted}
                    />
                    <Text
                      style={{
                        color: flashOn ? theme.colors.accent : theme.colors.text,
                        fontWeight: '600',
                        fontSize: 13,
                      }}
                    >
              ...
              <AnimatedCard title="掃描結果" subtitle="最近一次掃描" delay={100}>
                ...
                <View style={{ marginTop: 12 }}>
                  <Button
                    text="處理此 QR 碼"
                    kind="primary"
                    onPress={() => handleProcessScanResult(scanResult)}
                  />
                </View>
              </AnimatedCard>
            )}
