    ...
    <Screen noPadding>
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                {flashDeals.map((deal) => {
                  ...
                  const expiresIn = Math.max(
                    0,
                    Math.round((new Date(deal.expiresAt).getTime() - Date.now()) / 60000),
                  );
                  return (
                    <Pressable
                      key={deal.id}
                      onPress={async () => {
                        ...
                        if (result) {
                          Alert.alert(
                            '搶到了！',
                            `${deal.menuItemName} 以 NT$${deal.discountPrice} 加入訂單`,
                          );
        ...
        {searchResults ? (
          ...
          <>
            ...
            {selectedCafeteria && (
              <View style={{ gap: 8 }}>
                <SectionTitle
                  text={`${getCafeterias().find((c) => c.id === selectedCafeteria)?.name ?? ''} 店家`}
                />
            ...
            >
              ...
              >
                {[3000, 4500, 6000, 8000].map((v) => (
                  <Pressable
                    key={v}
                    onPress={() => setBudgetInput(v.toString())}
                    style={{
      ...
      {showGroupCreate && (
        <Modal
          ...
          transparent
          animationType="slide"
          onRequestClose={() => setShowGroupCreate(false)}
        >
            ...
            >
                  ...
                  {getVendors()
                    ...
                    .map((v) => (
                      ...
                      >
                        <Text
                          style={{
                            color: groupVendorId === v.id ? '#fff' : theme.colors.text,
                            fontSize: 12,
                            fontWeight: '500',
                          }}
                        >
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    value={groupDeadlineHours}
                    onChangeText={setGroupDeadlineHours}
                    keyboardType="number-pad"
                    style={{
                      borderWidth: 1,
                      borderColor: theme.colors.border,
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <Pressable
                  onPress={() => setShowGroupCreate(false)}
                  style={{
                    flex: 1,
                    paddingVertical: 13,
                    borderRadius: 10,
...
function VendorDetailView(props: {
  ...
  const handleOrder = useCallback(async () => {
    ...
    try {
      ...
      const budgetMsg =
        budgetResult.spent > budgetResult.budgetLimit * 0.9
    ...
    <Screen noPadding>
      <ScrollView
        ...
        contentContainerStyle={{
          gap: 12,
          padding: 16,
          paddingBottom: TAB_BAR_CONTENT_BOTTOM_PADDING + (cart.length > 0 ? 60 : 0),
        }}
      >
        {/* 返回 */}
        <Pressable
          ...
          <View style={{ gap: 10 }}>
            {menuItems.length === 0 ? (
              <Card title="菜單尚未建立" subtitle="店家尚未上傳菜單">
                <Text style={{ color: theme.colors.muted, fontSize: 13 }}>可直接到店內點餐。</Text>
              </Card>
            ) : (
                  ...
                  {menuItems
                    ...
                    .map((item) => (
                      ...
                      >
                        ...
                        <Text
                          style={{ color: theme.colors.accent, fontWeight: '800', fontSize: 16 }}
                        >
                          ${item.price}
                        </Text>
          ...
          <View style={{ gap: 10 }}>
            ...
            >
              ...
              >
                <Text
                  style={{
                    color: reviewText.trim() ? '#fff' : theme.colors.muted,
                    fontWeight: '700',
                    fontSize: 14,
                  }}
                >
            ...
            {reviews.map((r) => (
              ...
              >
                ...
                {r.tags.length > 0 && (
                  <View style={{ flexDirection: 'row', gap: 4 }}>
                    {r.tags.map((t) => (
                      <Text key={t} style={{ color: theme.colors.accent, fontSize: 10 }}>
                        #{t}
                      </Text>
                    ))}
                  </View>
      ...
      {cart.length > 0 && (
        <Pressable
          ...
          style={{
            position: 'absolute',
            bottom: TAB_BAR_CONTENT_BOTTOM_PADDING + 8,
            left: 16,
            right: 16,
            flexDirection: 'row',
            justifyContent: 'space-between',
        ...
        >
          ...
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 17 }}>下單 ${cartTotal}</Text>
        </Pressable>
      )}
    </Screen>
  );
