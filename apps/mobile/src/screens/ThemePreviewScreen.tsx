  ...
  return (
    <Screen>
      ...
      >
        <AnimatedCard
          title="目前主題"
          subtitle={`${school.name} · ${theme.mode === 'dark' ? '深色' : '淺色'}模式`}
        >
            ...
            {mockSchools.map((s) => (
              ...
              >
                <View
                  style={[
                    styles.schoolColor,
                    { backgroundColor: s.themeColor || theme.colors.accent },
                  ]}
                >
...
function InfoRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <View style={styles.infoRow}>
      ...
      <Text style={{ color: theme.colors.text, fontSize: 13, fontFamily: 'monospace' }}>
        {value}
      </Text>
    </View>
  );
