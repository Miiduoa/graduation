 ...
 */
export function calculateRelativeAngle(bearing: number, deviceHeading: number): number {
...
export class ARNavigationSession {
   ...
   */
  private checkStepCompletion(currentLocation: Location, step: ARNavigationStep): boolean {
  ...
  calculateETA(
    ...
    for (let i = this.currentStepIndex; i < this.waypoints.length - 1; i++) {
      remainingDistance += calculateDistance(this.waypoints[i], this.waypoints[i + 1]);
    }
    ...
    const estimatedSeconds = remainingDistance / walkingSpeedMps + floorChanges * 30; // 30 seconds per floor change
  ...
  getState() {
    return {
      ...
      progress: this.steps.length > 0 ? this.currentStepIndex / (this.steps.length - 1) : 0,
 ...
 */
export function createMockNavigationSteps(destination: string): ARNavigationStep[] {
  return [
    {
      id: '1',
 ...
 */
export function updateARCapabilities(
...
export function getRouteProgress(
  ...
  const nextTarget =
    bestT >= 0.9 ? route[Math.min(bestSegmentIndex + 2, route.length - 1)] : segmentEnd;
 ...
 */
export function pathToNavigationSteps(graph: NavigationGraph, path: string[]): ARNavigationStep[] {
...
export function generateVoiceGuidance(
  ...
  const nearestThreshold = config.announceDistance.find(
    (d) => distanceToStep <= d + 2 && distanceToStep >= d - 2,
  );
