...
- **重算「專案快照」表格中的計數**（與本 README **基準日 2026-05-16（含 `campusBusRoutes`、`GoogleMapsLikeScreen`、`OnBusModeScreen`、Web **`demoRole`／`DemoRolePill`**、中控台、`demo_預覽`、pptx）** 之表一致時，下列指令應得到：**1064**／**138**／**15**／**75**／5／24／**26**／4／5／12；日後若數字漂移，請以指令輸出為準並更新表格，`git ls-files | wc -l` 請在 **已 commit** 後複核）：
...
| 面向              | 盤點結果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git tracked files | **`1064`** 個（**複核**：`git ls-files \| wc -l`；**2026-05-16 本批**含 **`campusBusRoutes`**、**[`GoogleMapsLikeScreen.tsx`](apps/mobile/src/screens/GoogleMapsLikeScreen.tsx)**（Leaflet WebView POC）、**[`OnBusModeScreen.tsx`](apps/mobile/src/screens/OnBusModeScreen.tsx)**、**[`AIAgentObservatoryScreen.tsx`](apps/mobile/src/screens/AIAgentObservatoryScreen.tsx)**、**`dynamicQuietHours`**／測試、**`aiStudyBuddyMatcher`** Profile 延伸欄位、**[`AttendanceLiveScreen`](apps/mobile/src/screens/AttendanceLiveScreen.tsx)** **`method`** 取值、`TodayCockpitScreen` **`getStudentLifeQuickFacts`**、`TeacherCockpitScreen` Office、Web **demoRole** 全流程、`aiOrchestrator` Jest、`demo_預覽`／`口試簡報.pptx`；並含接續 commit 之 **`aiSkillApplicator`** dynamic quiet、`clubs` **`canJoinClubs`** toast；並延續 **2026-05-15** LoginLanding／cockpit／PuWebView／TC demo 資產等）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Mobile UI         | **`138`** 個 `apps/mobile/**/*Screen.tsx`、`15` 個 `*Stack.tsx`（複核：`find apps/mobile/src/screens -name '*Screen.tsx' \| wc -l`，Stack 請用 `find … '*Stack.tsx'`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
