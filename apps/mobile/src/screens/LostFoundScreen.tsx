...
import {
  ...
  // 智慧推薦
  getSmartLFSuggestions,
  type LFSuggestion,
  // 模擬資料
  simulateLFStats,
  simulateRecentPosts,
  simulateMyReputation,
...
export function LostFoundScreen(props: any) {
  ...
  const loadData = useCallback(() => {
    ...
    const foundPosts = allPosts.filter(
      (p) => p.type === 'found' && (p.status === 'open' || p.status === 'matching'),
    );
  ...
  const renderHome = () => (
    <>
      ...
      <AnimatedCard
        title="本月統計"
        subtitle={`歸還率 ${Math.round(stats.returnRate * 100)}%`}
        delay={50}
      >
      ...
      <AnimatedCard title="最新動態" subtitle={`${posts.length} 筆`} delay={200}>
        <View style={{ gap: 10 }}>{posts.slice(0, 4).map((p) => renderPostCard(p))}</View>
        {posts.length > 4 && (
        ...
        <View style={{ gap: 6 }}>
          {[
            ...
            >
              ...
              <Text style={{ color: step.color, fontWeight: '700', fontSize: 13 }}>
                保管 {step.days} 天
              </Text>
      ...
      >
        <View style={{ flexDirection: 'row', gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text
              style={{ color: theme.colors.text, fontWeight: '700', fontSize: 14, marginTop: 5 }}
              numberOfLines={1}
            >
              {p.title}
            </Text>
      ...
      <>
            ...
            {ITEM_CATEGORIES.slice(0, 10).map((cat) => (
              <Pressable
                ...
                style={{
                  ...
                  borderRadius: 999,
                  backgroundColor:
                    selectedCategory === cat.id ? `${cat.color}20` : theme.colors.surface2,
                  borderWidth: 1,
                  borderColor: selectedCategory === cat.id ? cat.color : theme.colors.border,
                }}
              >
                <Ionicons
        ...
        >
          ...
          <Text
            style={{
              flex: 1,
              color: searchQuery ? theme.colors.text : theme.colors.muted,
              fontSize: 14,
            }}
        ...
        <AnimatedCard title={`${typeLabel}列表`} subtitle={`${filteredPosts.length} 筆`}>
          {filteredPosts.length === 0 ? (
            ...
            <View style={{ gap: 10 }}>{filteredPosts.map((p) => renderPostCard(p))}</View>
          )}
        </AnimatedCard>
  ...
  const renderReputation = () => {
    ...
    const progressToNext = nextLevel
      ? (myRep.totalPoints - currentLevel.minPoints) /
            ...
            {nextLevel && (
              <View style={{ width: '100%', gap: 4 }}>
                ...
                >
                  ...
                  <Text style={{ color: theme.colors.muted, fontSize: 10 }}>
                    Lv.{nextLevel.level} ({nextLevel.minPoints}分)
                  </Text>
                </View>
                <View
                  style={{
            ...
            {REPUTATION_LEVELS.map((lv) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={{ color: lv.color, fontWeight: '700', fontSize: 12 }}>
                    {lv.minPoints}分
                  </Text>
                </View>
              );
            ...
            {[
              ...
              >
                ...
                {pt.items !== null && (
                  ...
                  >
                    <Text style={{ color: pt.color, fontWeight: '700', fontSize: 12 }}>
                      {pt.items} 件
                    </Text>
                  </View>
                )}
