...
| Web 離線身分切換（`localStorage`） | **`DemoRole`**／**`DEMO_USERS`**：[`demoRole.ts`](apps/web/src/lib/demoRole.ts)、[`DemoRolePill.tsx`](apps/web/src/components/DemoRolePill.tsx)（[`SiteShell.tsx`](apps/web/src/components/SiteShell.tsx)）；擴充 [`demoData.ts`](apps/web/src/lib/demoData.ts) **`DemoUser`**（`department`／`affiliation`／多 uid）；歷史學期 GPA、畢業學分門檻、下學期候選課與 **`aiContext`** 對齊欄位等 |
...
- **重算「專案快照」表格中的計數**（與本 README **基準日 2026-05-16（含晚間 Web `/admin`、`/ai-assistant`、`/credit-planner`、Mobile **`BusV2`／TrustCard／vendorPredictor**、輔材 md、快照 **1084／142**）** 之表一致時，下列指令應得到：**1084**／**142**／**15**／**78**／5／24／**29**／4／5／12；日後若數字漂移，請以指令輸出為準並更新表格，`git ls-files | wc -l` 請在 **已 commit** 後複核）：
...
- [目前最重要的事實](#目前最重要的事實)
- [專案快照](#專案快照)
- [本期程式與 QA 對照摘要（2026-05-15–16，含晚間口試補強）](#readme-qa-20260516)
...
下列數字為 **2026-05-15 主線對齊、2026-05-16（含晚間口試／Web 示範路徑補強）複核** 對目前 repo 的實際盤點（見上方「複核指令」備註）。後續功能增減時，請以程式碼與當下指令輸出為準。

| 面向              | 盤點結果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git tracked files | **`1084`** 個（**複核**：`git ls-files \| wc -l`。**2026-05-16 晚段追加**：Web **`admin`／`ai-assistant`／`credit-planner`**、[`apps/web/src/lib/aiContext.ts`](apps/web/src/lib/aiContext.ts)、多頁對齊示範資料；Mobile **`BusV2Screen`**、**`MapStack` `BusV2`**、**[`BusStopDetailScreen.tsx`](apps/mobile/src/screens/BusStopDetailScreen.tsx)**、**[`aiTrustCard.ts`](apps/mobile/src/services/aiTrustCard.ts)**／**[`AITrustCardScreen.tsx`](apps/mobile/src/screens/AITrustCardScreen.tsx)**、**[`vendorPredictor.ts`](apps/mobile/src/services/vendorPredictor.ts)**、[`savedPlaces.ts`](apps/mobile/src/services/savedPlaces.ts)、[`voiceNav.ts`](apps/mobile/src/services/voiceNav.ts)、[`demoNotImplemented.ts`](apps/mobile/src/utils/demoNotImplemented.ts)、**[`TripPlannerScreen.tsx`](apps/mobile/src/screens/TripPlannerScreen.tsx)**、[`DepartmentDashboardScreen.tsx`](apps/mobile/src/screens/DepartmentDashboardScreen.tsx) 調整、**`safeNavigate`**＋對應測試、強化 **`aiStudyBuddyMatcher`**／**[`AIStudyBuddyScreen.tsx`](apps/mobile/src/screens/AIStudyBuddyScreen.tsx)**、`docs/AI_SERVICE_CONSOLIDATION_PLAN.md`、`DEMO_SCRIPT.md`、`DEMO_角色完整版.md`、`口試交付包_README.md`。**早段**仍含：**`campusBusRoutes`**、**[`GoogleMapsLikeScreen.tsx`](apps/mobile/src/screens/GoogleMapsLikeScreen.tsx)** POC、[`OnBusModeScreen.tsx`](apps/mobile/src/screens/OnBusModeScreen.tsx)、中控台、Web **`demoRole`** 全流程等，見上方 blockquote）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Mobile UI         | **`142`** 個 `apps/mobile/**/*Screen.tsx`、`15` 個 `*Stack.tsx`（複核：`find apps/mobile/src/screens -name '*Screen.tsx' \| wc -l`，Stack 請用 `find … '*Stack.tsx'`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Web routes        | **`29`** 個 `apps/web/**/page.tsx`、`4` 個 `apps/web/**/route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 測試檔            | Mobile **`78`**（`apps/mobile/src/__tests__/**/*.test.{ts,tsx}`；**不含** `pnpm ai:train:long` 專用之 `apps/mobile/scripts/ai-training-long.test.ts`；本晚段追加 **`safeNavigate`**、**`aiTrustCard`**、**`vendorPredictor`**、擴充 **`aiStudyBuddyMatcher`** 等）；Web **`5`**、Backend Functions **`24`**、Rules **`1`**（複核指令同前）                                                                                                                                                                                                                                               |
...
<h3 id="readme-qa-20260516">本期程式與 QA 對照摘要（2026-05-15–16，含晚間口試補強）</h3>
...
| `/admin`                                | 離線示範：系所／管理端公告審核中控台（`useDemoRole`／`DEMO_ANNOUNCEMENTS`） |
