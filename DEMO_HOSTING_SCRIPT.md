...
> 對象：畢業口試委員
> 平台：Web（apps/web，Next.js 16 + Turbopack）
> 主軸：**8 種角色 + 跨角色動作 → 真實資料變化**
> 時間：建議主軸 8–10 分鐘，再留 5 分鐘給委員自由操作
> 訊息頁版本：**3-tab 結構（💬 私訊 / 🔔 系統通知 / 👥 好友）**，有未讀通知時會自動切到「🔔 系統通知」tab
...
4. 按一下 **「♻️ 一鍵重置 demo 資料」**，再按 **「🌱 一鍵 seed 示範佇列」**。
   - 這會幫王小明同時送出 **6 條動作鏈**：
     請假 / 宿舍報修 / 訂餐 / 求助 / 繳交作業 / 申請加入攝影社
   - 同時 seed **友誼 + DM 對話**（讓 💬 私訊 tab 有內容）
   - 之後切到老師 / TA / admin / 系主任 / 社長，他們的 🔔 系統通知 tab 立刻就有東西可以演示
...
- 進 `/messages`。**頁面自動停在「🔔 系統通知」tab**（因為有未讀），不用手動切。
...
- 「左邊 💬 私訊 tab 也有 seed 過的師生對話，老師也能切過去看跟王小明的私下討論。」
...
操作：
- 切到 **社團幹部**（陳社長）。
- 進 `/messages` → 🔔 系統通知 tab 自動展開。
- 看到「【社員申請】王小明 申請加入 攝影社」訊息。
- 點開 → 「📨 社員申請」面板 → 按 **✅ 核准** → Toast。
