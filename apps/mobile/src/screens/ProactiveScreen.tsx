...
export function ProactiveScreen() {
  ...
  const handleAction = useCallback(
    (nudge: SmartNudge, action: SmartNudge['actions'][0]) => {
        ...
        {nudges.length > 0 && (
          ...
          >
            {[
              {
                label: '緊急',
                count: nudges.filter((n) => n.priority === 'critical').length,
                color: '#EF4444',
              },
              {
                label: '重要',
                count: nudges.filter((n) => n.priority === 'high').length,
                color: '#F97316',
              },
              {
                label: '建議',
                count: nudges.filter(
                  (n) => n.priority === 'medium' || n.priority === 'low' || n.priority === 'info',
                ).length,
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: theme.space.sm }}>
        ...
        <View style={{ flex: 1 }}>
          <Text
            style={{ color: theme.colors.text, fontSize: 15, fontWeight: '700', lineHeight: 20 }}
          >
            {nudge.title}
          </Text>
          <Text
