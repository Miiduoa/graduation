...
const ALL_ACHIEVEMENTS: Omit<Achievement, 'unlockedAt' | 'progress'>[] = [
  ...
  {
    id: 'assignment_machine',
    title: '作業狂人',
    description: '連續交出 10 份作業',
    icon: 'document-text',
    category: 'academic',
    xpReward: 100,
    rarity: 'rare',
  ...
  },
  {
    id: 'ai_curious',
    title: 'AI 好奇寶寶',
    description: '和 AI 助理對話 50 次',
    icon: 'chatbubbles',
    category: 'exploration',
    xpReward: 100,
    rarity: 'rare',
    condition: 'AI 對話 50 次',
    maxProgress: 50,
  },
  {
    id: 'night_owl',
    title: '夜貓子',
    description: '晚上 10 點後使用 App',
    icon: 'moon',
    category: 'exploration',
    xpReward: 20,
    rarity: 'common',
...
function checkAchievements(state: StoredState, action: XPAction): Achievement[] {
  ...
  if (achId) {
    ...
    if (
      ...
      !state.unlockedAchievements[achId]
    ) {
      newlyUnlocked.push({ ...ach, unlockedAt: Date.now() });
    }
  }
...
function generateLeaderboard(myXP: number, myName: string, myDept: string): LeaderboardEntry[] {
  ...
  return allEntries.sort((a, b) => b.xp - a.xp).map((e, i) => ({ ...e, rank: i + 1 }));
}
