...
- **重算「專案快照」表格中的計數**（與本 README **基準日 2026-05-18（含 LMS v2 第六批 push）** 對目前 repo **已 commit** 狀態一致時，`find … | wc -l` 結果應接近：**1302**／**165**／**15**／**81**／5／24／**66**／4／5／12。歷史節點：**第四批為止≈1138**；**第五批**（含 `supabase/`、lmsV2 前置、staged、[LMS 整合計畫](LMS_%E6%95%B4%E5%90%88%E8%A8%88%E7%95%AB.docx)）**≈1257**。**第六批**（`screens/lmsV2/`、`/lms-admin`、`migrations_combined.sql`、`docs/design` 等）**≈1302**。日後若數字漂移，請以指令輸出為準並更新 README；`git ls-files | wc -l` 務必於 **commit 後**複核）。
