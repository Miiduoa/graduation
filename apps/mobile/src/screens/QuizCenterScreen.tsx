      ...
      >
        ...
        >
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            ...
            <Pill
              text={`${publishedCount} 項已發布成績`}
              kind={publishedCount > 0 ? 'success' : 'muted'}
            />
            {routeGroupId && canEditCourse ? (
              <Pill text="教師可直接建立評量" kind="success" />
            ) : null}
          </View>
        </Card>
        ...
        {routeGroupId && canEditCourse ? (
          <Card
            title="建立評量"
            subtitle="建立後會同步寫入 quizzes 與 assignments，保留既有繳交流程"
          >
            <View style={{ gap: 10 }}>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <Pressable
                  ...
                  style={{
                    ...
                    backgroundColor:
                      quizType === 'quiz' ? theme.colors.accentSoft : theme.colors.surface2,
                    alignItems: 'center',
                  }}
                >
                  <Text
                    style={{
                      color: quizType === 'quiz' ? theme.colors.accent : theme.colors.text,
                      fontWeight: '700',
                    }}
                  >
                    測驗
                ...
                <Pressable
                  ...
                  style={{
                    ...
                    backgroundColor:
                      quizType === 'exam' ? theme.colors.warningSoft : theme.colors.surface2,
                    alignItems: 'center',
                  }}
                >
                  <Text
                    style={{
                      color: quizType === 'exam' ? theme.colors.warning : theme.colors.text,
                      fontWeight: '700',
                    }}
                  >
                    考試
              ...
              upcoming.map((item) => (
                ...
                >
                  ...
                  >
                    ...
                    <Pill
                      text={item.type === 'quiz' ? '測驗' : '考試'}
                      kind={item.type === 'quiz' ? 'accent' : 'warning'}
                    />
                  </View>
                  <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                    <Pill
                      text={`時間：${formatDateTime(toDate(item.dueAt), '未設定')}`}
                      kind="default"
                    />
                    {item.questionCount ? (
                      <Pill text={`${item.questionCount} 題`} kind="default" />
                    ) : null}
                    {item.durationMinutes ? (
                      <Pill text={`${item.durationMinutes} 分鐘`} kind="default" />
                    ) : null}
                    {item.source === 'quiz' ? (
                      <Pill text="正式 quizzes 模型" kind="success" />
                    ) : null}
                  </View>
                </Pressable>
              ))
        ...
        <Card title="評量能力" subtitle="正式資料模型已接上，後續可持續擴充">
          <View style={{ gap: 10 }}>
            ...
            <Text style={{ color: theme.colors.muted, lineHeight: 22 }}>
              這一版已先把正式 `quizzes`
              資料接進主流程，之後再向下擴充題庫與答題引擎，不需要再重做入口層。
            </Text>
          </View>
        </Card>
      </ScrollView>
