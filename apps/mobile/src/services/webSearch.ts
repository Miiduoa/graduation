...
function selectEvidence(
  ...
  sources: WebSearchSource[],
  maxCount = 4,
): Array<{ text: string; source: WebSearchSource; sourceNumber: number }> {
  const seen = new Set<string>();
  return sources
    .flatMap((source, sourceIndex) =>
      splitSentences(source.snippet)
        .slice(0, 4)
        .map((sentence) => ({
...
function inferQuestionMode(
  query: string,
): 'current_fact' | 'how_to' | 'compare' | 'definition' | 'forecast' | 'general' {
...
function buildReasonedSummary(
  query: string,
  sources: WebSearchSource[],
): { conclusion: string; rationale: string[]; confidence: WebGroundedAnswer['confidence'] } {
  ...
  if (mode === 'forecast') {
    return {
      conclusion:
        '結論：這類問題不能只靠搜尋結果斷言未來走勢。我可以整理已知資訊，但不會把股價、匯率或事件結果講成確定會發生。',
  ...
  const rationale =
    evidence.length > 0
      ...
      : sources
          .slice(0, 2)
          .map((source, index) => `來源 ${index + 1}：${truncate(source.snippet, 170)}`);

  const hasPrimaryLikeSource = sources.some(
    (source) =>
...
async function fetchWikipediaSummary(
  ...
  return {
    ...
    url:
      ...
      `https://${language}.wikipedia.org/wiki/${encodedTitle}`,
...
function normalizeWeatherLocation(
  query: string,
  ...
  const match = query.match(
    /(?:今天|現在|目前|明天|後天)?\s*([^，。？！?]{2,18}?)(?:天氣|氣溫|會下雨|下雨|帶傘)/,
  );
...
async function fetchWeatherAnswer(
  query: string,
  signal?: AbortSignal,
): Promise<WebGroundedAnswer | null> {
  if (!/(天氣|氣溫|下雨|帶傘|雨具|天候)/.test(query)) return null;
  const normalizedLocation = normalizeWeatherLocation(query) ?? {
    name: '台中市沙鹿區靜宜大學',
    latitude: 24.226,
    longitude: 120.563,
  };
...
function isRouteQuery(query: string): boolean {
  return /怎麼去|怎樣去|如何去|如何到|怎麼到|到.*怎麼走|搭什麼|搭哪|幾號公車|公車.*到|路線|導航|交通/.test(
    query,
  );
}
...
function fetchTransitRouteAnswer(query: string): WebGroundedAnswer | null {
  ...
  const fromPu = /靜宜|静宜|pu|providence/i.test(places.origin) || places.origin === '靜宜大學';
  const toTaichungStation = /台中車站|臺中車站|台中火車站|臺中火車站|台中.*車站|臺中.*車站/i.test(
...
export function shouldUseWebSearch(
  query: string,
  category: IntentCategoryLike = 'general',
): boolean {
  ...
  const explicitOnlineRequest =
    /連網|上網|網路|搜尋|查一下|查詢|幫我查|google|資料來源|來源|最新|即時|目前|現在|今天|昨天|明天|現任|新聞|公告最新|更新/.test(
      q,
    );
  ...
  const generalQuestion =
    /誰|哪|什麼|為什麼|如何|怎麼|多少|幾|是什麼|意思|定義|介紹|比較|差異|推薦|可以嗎|\?$|？$/.test(
      q,
    );

  return (
    generalQuestion ||
    /最新|目前|現在|今日|昨日|昨天|明天|即時|新聞|最近|現任|誰是|是誰|哪一年|幾年|幾月|幾號|價格|票價|匯率|股票|總統|市長|校長|ceo|版本|發布|政策|法規|天氣|氣溫|下雨|路線|導航|交通/.test(
      q,
    )
  );
}
...
function pickMainClaim(query: string, sources: WebSearchSource[]): string {
  ...
  if (/誰是|是誰|現任|總統|市長|校長|ceo/i.test(query)) {
    const current = sentences.find((sentence) =>
      /現任|现任|current|第\d+任|是|為|为/.test(sentence),
    );
...
export function buildWebGroundedAnswer(
  query: string,
  sources: WebSearchSource[],
  fetchedAt = new Date().toISOString(),
): WebGroundedAnswer | null {
  ...
  const sourceLines = usableSources.map((source, index) => {
    const updated = source.updatedAt
      ? `，更新：${new Date(source.updatedAt).toLocaleDateString('zh-TW')}`
      : '';
...
export async function answerWithOnlineSearch(
  query: string,
  signal?: AbortSignal,
): Promise<WebGroundedAnswer | null> {
