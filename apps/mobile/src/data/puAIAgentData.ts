...
export type ToolExecutionStatus = "pending" | "confirming" | "executing" | "success" | "failed" | "cancelled";
...
export const AGENT_TOOLS: AgentTool[] = [
  // ── 餐廳模組 ──
  {
    ...
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時",
    relatedTools: ["recommend_meal", "check_wait_time"],
    roleAccess: ["student", "faculty", "staff"],
  },
  {
    id: "recommend_meal", category: "cafeteria", name: "推薦餐點", icon: "sparkles-outline", color: "#10B981",
    description: "根據偏好、預算、營養需求推薦今日餐點",
    parameters: [
      { name: "budget", type: "number", label: "預算（元）", required: false, hint: "不填則不限" },
      { name: "preference", type: "multi_select", label: "偏好", required: false, options: [
        { value: "meat", label: "有肉" }, { value: "vegan", label: "素食" },
        { value: "spicy", label: "辣" }, { value: "healthy", label: "健康" },
        { value: "fast", label: "快速" }, { value: "cheap", label: "便宜" },
      ]},
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  },
  {
    id: "check_wait_time", category: "cafeteria", name: "查詢等候時間", icon: "time-outline", color: "#3B82F6",
  ...
  // ── 健康模組 ──
  {
    id: "book_health", category: "health", name: "預約掛號", icon: "calendar-outline", color: "#10B981",
    description: "預約衛保組門診或心理諮商",
    parameters: [
      { name: "department", type: "select", label: "科別", required: true, options: [
        { value: "general", label: "一般門診" }, { value: "mental", label: "心理諮商" },
        { value: "sports_injury", label: "運動傷害" }, { value: "dental", label: "牙科" },
        { value: "eye", label: "視力保健" }, { value: "nutrition", label: "營養諮詢" },
      ]},
      { name: "date", type: "date", label: "日期", required: false, hint: "不填則為最近可預約時段" },
  ...
  {
    id: "symptom_check", category: "health", name: "症狀自評", icon: "medical-outline", color: "#EF4444",
  ...
  {
    id: "record_mood", category: "health", name: "記錄心情", icon: "happy-outline", color: "#7C3AED",
    description: "記錄今天的情緒狀態",
    parameters: [
      { name: "level", type: "select", label: "心情", required: true, options: [
        { value: "1", label: "😞 很差" }, { value: "2", label: "😟 不太好" },
        { value: "3", label: "😐 普通" }, { value: "4", label: "🙂 不錯" },
        { value: "5", label: "😄 很好" },
      ]},
      { name: "factors", type: "multi_select", label: "影響因素", required: false, options: [
        { value: "academic", label: "課業" }, { value: "social", label: "人際" },
        { value: "sleep", label: "睡眠" }, { value: "health", label: "身體" },
      ]},
  ...
  // ── 圖書館模組 ──
  {
    id: "reserve_seat", category: "library", name: "預約座位", icon: "desktop-outline", color: "#8B5CF6",
    description: "預約圖書館自習座位或討論室",
    parameters: [
      { name: "type", type: "select", label: "類型", required: true, options: [
        { value: "individual", label: "個人自習座位" }, { value: "group_room", label: "團體討論室" },
        { value: "quiet_zone", label: "安靜閱覽區" },
      ]},
      { name: "date", type: "date", label: "日期", required: true },
      { name: "time_slot", type: "select", label: "時段", required: true, options: [
        { value: "morning", label: "上午 (08:00-12:00)" }, { value: "afternoon", label: "下午 (12:00-17:00)" },
        { value: "evening", label: "晚上 (17:00-21:30)" },
      ]},
      { name: "floor", type: "select", label: "樓層", required: false, options: [
        { value: "1F", label: "1F 大廳" }, { value: "2F", label: "2F 書庫" },
        { value: "3F", label: "3F 閱讀沙龍" }, { value: "B1", label: "B1 期刊區" },
      ]},
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時",
  ...
  {
    id: "search_book", category: "library", name: "查詢/借書", icon: "book-outline", color: "#0D9488",
    description: "搜尋圖書館藏書、查看可借狀態、預約借閱",
    parameters: [
      { name: "query", type: "string", label: "書名/作者/ISBN", required: true },
      { name: "action", type: "select", label: "操作", required: false, options: [
        { value: "search", label: "只查詢" }, { value: "reserve", label: "預約借閱" },
      ]},
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  ...
  // ── 宿舍模組 ──
  {
    id: "report_repair", category: "dorm", name: "報修", icon: "construct-outline", color: "#F59E0B",
    description: "提交宿舍設施維修申請",
    parameters: [
      { name: "category", type: "select", label: "類別", required: true, options: [
        { value: "plumbing", label: "水管/馬桶" }, { value: "electrical", label: "電力/燈具" },
        { value: "furniture", label: "家具/門鎖" }, { value: "ac", label: "冷氣/暖氣" },
        { value: "network", label: "網路" }, { value: "other", label: "其他" },
      ]},
      { name: "description", type: "string", label: "問題描述", required: true, hint: "越詳細越快處理" },
      { name: "room", type: "string", label: "房號", required: true },
      { name: "urgency", type: "select", label: "急迫度", required: false, options: [
        { value: "low", label: "不急" }, { value: "medium", label: "一般" }, { value: "high", label: "緊急" },
      ]},
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時提交，1-3 天處理",
    roleAccess: ["student"],
  },
  {
    id: "check_laundry", category: "dorm", name: "查洗衣機", icon: "water-outline", color: "#3B82F6",
    description: "查看洗衣機/烘衣機使用狀態和預估完成時間",
    parameters: [
      { name: "building", type: "select", label: "宿舍", required: false, options: [
        { value: "schultz", label: "希嘉學苑" }, { value: "bosco", label: "思高學苑" },
      ]},
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student"],
  ...
  {
    id: "check_package", category: "dorm", name: "查包裹", icon: "cube-outline", color: "#EC4899",
  ...
  // ── 失物招領模組 ──
  {
    id: "post_lost", category: "lost_found", name: "報失", icon: "alert-circle-outline", color: "#EF4444",
    description: "發布遺失物品公告，AI 自動比對拾獲物",
    parameters: [
      { name: "item", type: "string", label: "物品名稱", required: true },
      { name: "category", type: "select", label: "分類", required: true, options: [
        { value: "phone", label: "手機" }, { value: "wallet", label: "錢包" },
        { value: "keys", label: "鑰匙" }, { value: "earbuds", label: "耳機" },
        { value: "student_card", label: "學生證" }, { value: "umbrella", label: "雨傘" },
        { value: "other", label: "其他" },
      ]},
      { name: "location", type: "string", label: "遺失地點", required: true },
      { name: "features", type: "string", label: "特徵描述", required: true, hint: "顏色/品牌/貼紙/刻字等" },
      { name: "time", type: "string", label: "大約時間", required: false },
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時發布",
    relatedTools: ["search_found"],
    roleAccess: ["student", "faculty", "staff"],
  },
  {
    id: "search_found", category: "lost_found", name: "搜尋拾獲物", icon: "search-outline", color: "#10B981",
    description: "搜尋目前所有拾獲物品",
    parameters: [
      { name: "keyword", type: "string", label: "關鍵字", required: true },
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  },
  ...
  // ── 列印模組 ──
  {
    id: "print_file", category: "print", name: "列印", icon: "print-outline", color: "#6366F1",
    ...
    parameters: [
      ...
      { name: "color", type: "select", label: "色彩", required: false, options: [
        { value: "bw", label: "黑白" }, { value: "color", label: "彩色" },
      ]},
      { name: "sides", type: "select", label: "單雙面", required: false, options: [
        { value: "single", label: "單面" }, { value: "double", label: "雙面" },
      ]},
      { name: "printer", type: "select", label: "印表機", required: false, options: [
        { value: "lib_1f", label: "圖書館 1F" }, { value: "lib_b1", label: "圖書館 B1" },
        { value: "bodo_3f", label: "伯鐸樓 3F" }, { value: "admin_1f", label: "行政大樓 1F" },
      ]},
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "上傳 ~10 秒",
    roleAccess: ["student", "faculty", "staff"],
  },
  {
    id: "check_print_balance", category: "print", name: "查列印餘額", icon: "card-outline", color: "#374151",
    description: "查詢影印卡餘額",
    parameters: [],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  ...
  // ── 課程模組 ──
  {
    id: "request_leave", category: "course", name: "請假", icon: "document-text-outline", color: "#DC2626",
    description: "向授課教師提交請假申請",
    parameters: [
      { name: "course", type: "string", label: "課程名稱", required: true },
      { name: "date", type: "date", label: "日期", required: true },
      { name: "reason", type: "select", label: "假別", required: true, options: [
        { value: "sick", label: "病假" }, { value: "personal", label: "事假" },
        { value: "official", label: "公假" }, { value: "funeral", label: "喪假" },
      ]},
      { name: "detail", type: "string", label: "事由說明", required: true },
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時提交，待教師審核",
    roleAccess: ["student"],
  },
  {
    id: "check_grades", category: "course", name: "查成績", icon: "school-outline", color: "#3B82F6",
    description: "查詢最新成績或學期平均",
    parameters: [
      { name: "scope", type: "select", label: "範圍", required: false, options: [
        { value: "latest", label: "最新成績" }, { value: "semester", label: "本學期" },
        { value: "all", label: "歷年" },
      ]},
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
  ...
  {
    id: "check_assignments", category: "course", name: "查作業截止", icon: "clipboard-outline", color: "#F59E0B",
  ...
  // ── 交通模組 ──
  {
    id: "check_bus", category: "transport", name: "查公車", icon: "bus-outline", color: "#059669",
    description: "查詢校門口公車到站時間",
    parameters: [
      { name: "destination", type: "string", label: "目的地", required: false, hint: "例如：台中火車站" },
    ],
    requiresConfirmation: false, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  },
  ...
  // ── 行事曆模組 ──
  {
    id: "set_reminder", category: "calendar", name: "設定提醒", icon: "alarm-outline", color: "#8B5CF6",
    description: "設定時間到時推播提醒",
    parameters: [
      { name: "title", type: "string", label: "提醒內容", required: true },
      { name: "datetime", type: "string", label: "時間", required: true, hint: "例如：明天下午 2 點、3 小時後" },
      { name: "repeat", type: "select", label: "重複", required: false, options: [
        { value: "once", label: "一次" }, { value: "daily", label: "每天" }, { value: "weekly", label: "每週" },
      ]},
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  ...
  // ── 社群模組 ──
  {
    id: "send_message", category: "social", name: "發訊息", icon: "chatbubble-outline", color: "#EC4899",
    ...
    requiresConfirmation: true, isReversible: false, estimatedDuration: "即時",
    roleAccess: ["student", "faculty", "staff"],
  },

  // ── 同儕互動工具（學生 ↔ 學生）──
  {
    id: "peer_review", category: "social", name: "同儕互評", icon: "people-outline", color: "#8B5CF6",
    ...
    requiresConfirmation: true, isReversible: false, estimatedDuration: "1-2 分鐘",
    roleAccess: ["student", "faculty"],
    relatedTools: ["check_assignments"],
  },
  {
    id: "study_group_match", category: "social", name: "組隊配對", icon: "git-merge-outline", color: "#06B6D4",
    description: "AI 根據課表、興趣、能力自動配對組員或讀書夥伴",
    parameters: [
      { name: "purpose", type: "select", label: "目的", required: true, options: [
        { value: "study", label: "讀書會" }, { value: "project", label: "專題組員" },
        { value: "exam_prep", label: "考前衝刺" }, { value: "tutor", label: "課業輔導" },
      ]},
  ...
  {
    id: "share_notes", category: "social", name: "共享筆記", icon: "document-attach-outline", color: "#10B981",
  ...
  {
    id: "tutoring_request", category: "course", name: "課業求助", icon: "school-outline", color: "#7C3AED",
    description: "AI 媒合學長姐或高手同學進行課業輔導",
    parameters: [
      { name: "subject", type: "string", label: "科目/問題", required: true },
      { name: "urgency", type: "select", label: "急迫度", required: false, options: [
        { value: "low", label: "不急" }, { value: "medium", label: "這週內" }, { value: "high", label: "明天要交" },
      ]},
    ],
    requiresConfirmation: true, isReversible: true, estimatedDuration: "即時",
    roleAccess: ["student"],
  ...
  {
    id: "event_invite", category: "social", name: "活動邀約", icon: "calendar-outline", color: "#EC4899",
  ...
  {
    id: "carpool_match", category: "transport", name: "共乘配對", icon: "car-outline", color: "#059669",
    description: "AI 根據住址和時間配對共乘同學",
    parameters: [
      { name: "direction", type: "select", label: "方向", required: true, options: [
        { value: "to_school", label: "到學校" }, { value: "from_school", label: "回家" },
      ]},
  ...
  {
    id: "secondhand_trade", category: "social", name: "二手交易", icon: "swap-horizontal-outline", color: "#D97706",
    description: "AI 比對二手書/用品需求，自動配對買賣雙方",
    parameters: [
      { name: "action", type: "select", label: "我要", required: true, options: [
        { value: "sell", label: "賣" }, { value: "buy", label: "買" },
      ]},
      { name: "item", type: "string", label: "物品名稱", required: true },
  ...
  // ── 教師互動工具 ──
  {
    id: "assignment_publish", category: "course", name: "發布作業", icon: "create-outline", color: "#2563EB",
  ...
  {
    id: "peer_review_assign", category: "course", name: "分配互評", icon: "git-network-outline", color: "#7C3AED",
  ...
  {
    id: "attendance_alert", category: "course", name: "出席警示", icon: "alert-outline", color: "#DC2626",
  ...
  {
    id: "learning_insight", category: "course", name: "學習分析", icon: "analytics-outline", color: "#0891B2",
    description: "AI 分析全班學習數據，產生個別化回饋與建議",
    parameters: [
      { name: "course", type: "string", label: "課程", required: true },
    ],
    requiresConfirmation: false, isReversible: false, estimatedDuration: "10-30 秒",
    roleAccess: ["faculty"],
  },
];
...
export const TASK_CHAINS: TaskChain[] = [
  ...
  {
    id: "lost_item_flow", trigger: "我遺失|我掉了|找不到我的",
    name: "遺失物處理", description: "發布遺失文 → AI 比對 → 通知結果",
    icon: "alert-circle-outline", color: "#F59E0B",
    steps: [
      { order: 1, toolId: "post_lost", label: "發布遺失公告" },
      { order: 2, toolId: "search_found", label: "AI 自動比對拾獲物", dependsOn: 1 },
      { order: 3, toolId: "set_reminder", label: "設定追蹤提醒", autoParams: { title: "檢查遺失物配對結果" }, optional: true },
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      { order: 2, toolId: "set_reminder", label: "追蹤維修進度", autoParams: { title: "確認報修處理狀態", datetime: "2天後" } },
    ],
  },
  // ── 同儕互動任務鏈 ──
  {
    id: "group_study", trigger: "找人一起讀|讀書會|組讀書|找隊友|組隊",
    name: "組隊讀書", description: "配對夥伴 → 建群組 → 預約座位 → 設提醒",
    icon: "people-outline", color: "#06B6D4",
    steps: [
      { order: 1, toolId: "study_group_match", label: "AI 配對讀書夥伴" },
      { order: 2, toolId: "send_message", label: "���送邀約訊息", dependsOn: 1 },
      { order: 3, toolId: "reserve_seat", label: "預約討論室", autoParams: { type: "group_room" }, optional: true },
  ...
  {
    id: "help_request", trigger: "不會寫作業|看不懂課本|誰能教我|找人教我|課業求助",
    name: "課業求助", description: "描述問題 → 配對高手 → 發送求助",
    icon: "school-outline", color: "#7C3AED",
    steps: [
  ...
  {
    id: "sell_stuff", trigger: "想賣|要賣|二手|出清|賣書",
    name: "��手交易", description: "刊登物品 → AI 配對買家 → 通知",
    icon: "swap-horizontal-outline", color: "#D97706",
    steps: [
      { order: 1, toolId: "secondhand_trade", label: "刊登二手物品", autoParams: { action: "sell" } },
...
export function buildThinkingChain(
  ...
  const relevantData = dataChecks.filter(d => {
    ...
    if (intentDesc.includes("學業") || intentDesc.includes("課程")) return ["hasCourses", "hasAssignments", "hasGrades"].includes(d.key);
  ...
  const conclusion = hasEnoughData
    ? `基於${relevantData.filter(d => d.available).map(d => d.label).join("、") || "已知資訊"}回答`
    : noRealtimeData
    ? "提供參考資訊（非即時）"
    : "資料不足，建議查詢";
...
export function extractLearnableFacts(userMessage: string, prevMessages: string[]): LearnedFact[] {
  ...
  for (const [pattern, template] of dietaryPatterns) {
    ...
    if (match) {
      ...
      facts.push({ id: `fact-${Date.now()}-${facts.length}`, fact, source: "explicit", confidence: 0.95, learnedAt: now, category: "dietary" });
    }
  }
  ...
  for (const [pattern, template, cat] of personalPatterns) {
    ...
    if (match) {
      const fact = template.replace("$1", match[1] ?? "");
      facts.push({ id: `fact-${Date.now()}-${facts.length}`, fact, source: "explicit", confidence: 0.9, learnedAt: now, category: cat as LearnedFact["category"] });
    }
  }
...
export const PROACTIVE_TRIGGERS: ProactiveTrigger[] = [
  {
    id: "class_soon", type: "time_based", name: "上課提醒",
    condition: "上課前 15 分鐘", icon: "school-outline", color: "#3B82F6",
    message: "你的「{course}」{time} 在 {location} 上課，要出發了嗎？",
    priority: "high",
  },
  {
    id: "assignment_due", type: "time_based", name: "作業截止提醒",
    condition: "截止前 24 小時", icon: "alert-circle-outline", color: "#EF4444",
    message: "「{assignment}」明天截止！目前進度如何？需要我幫你安排時間嗎？",
    suggestedTool: "set_reminder", priority: "high",
  },
  {
    id: "package_arrived", type: "event_based", name: "包裹到件",
    condition: "新包裹到貨", icon: "cube-outline", color: "#EC4899",
    message: "你有一個新包裹到了！{carrier} 送達，放在 {location}。要我設定取件提醒嗎？",
    suggestedTool: "set_reminder", priority: "medium",
  },
  {
  ...
  {
    id: "laundry_done", type: "event_based", name: "洗衣完成",
    condition: "洗衣機完成", icon: "water-outline", color: "#10B981",
    message: "你的洗衣已完成！記得去取衣服，避免被其他人移出。",
    priority: "medium",
  },
  {
    id: "mood_check", type: "time_based", name: "每日心情",
    condition: "每天 21:00", icon: "happy-outline", color: "#7C3AED",
    message: "今天過得如何？花 10 秒記錄一下心情吧。",
    suggestedTool: "record_mood", priority: "low",
  },
  {
    id: "library_closing", type: "time_based", name: "圖書館閉館",
    condition: "閉館前 30 分鐘 + 你在圖書館", icon: "library-outline", color: "#8B5CF6",
    message: "圖書館 30 分鐘後閉館，記得收拾東西。需要我預約明天的座位嗎？",
    suggestedTool: "reserve_seat", priority: "medium",
  },
  {
    id: "rain_alert", type: "event_based", name: "下雨預報",
    condition: "未來 1 小時降雨機率 > 70%", icon: "rainy-outline", color: "#6366F1",
...
export const AGENT_ROLE_CONFIG: AgentRoleConfig[] = [
  ...
  {
    role: "staff", label: "職員", icon: "briefcase-outline", color: "#059669",
    toolCategories: ["cafeteria", "health", "library", "lost_found", "print", "calendar", "social"],
    proactiveCapabilities: ["lunch_time", "rain_alert"],
    description: "行政輔助代理：排程管理/訊息轉發/資源預約",
  },
  {
    role: "vendor", label: "商家", icon: "storefront-outline", color: "#F59E0B",
    toolCategories: ["cafeteria", "calendar", "social"],
    proactiveCapabilities: [],
    description: "營運輔助代理：訂單管理/庫存提醒/營業報表",
  },
  {
    role: "admin", label: "管理員", icon: "settings-outline", color: "#374151",
    toolCategories: ["cafeteria", "health", "library", "dorm", "lost_found", "print", "course", "transport", "calendar", "social", "system"],
...
export const AGENT_ROLE_INTERACTIONS: AgentRoleInteraction[] = [
  // ── 學生 ↔ 學生（同儕互動）──
  {
    ...
    actions: [
      ...
      { id: "carpool_match", label: "共乘配對", icon: "car-outline", description: "AI 根據住址/時間配對共乘同學" },
      { id: "secondhand_trade", label: "二手交易媒合", icon: "swap-horizontal-outline", description: "AI 比對二手書/用品需求，自動配對買賣" },
      { id: "mood_support", label: "同儕關懷", icon: "heart-outline", description: "AI 偵測到同學情緒低落時，匿名提示關懷行動" },
    ],
  },
  // ── 學生 → 教師 ──
  {
    ...
    actions: [
      { id: "auto_leave", label: "AI 代理請假", icon: "document-text-outline", description: "AI 自動填寫假單、附上證明、發送給教師" },
      { id: "question_forward", label: "課業問題轉發", icon: "chatbubble-outline", description: "AI 整理問題後以學生名義發送至教師" },
      { id: "office_hour_book", label: "預約 Office Hour", icon: "time-outline", description: "AI 查詢教師空閒時段、代為預約、發送提醒" },
      { id: "assignment_clarify", label: "作業疑問", icon: "help-circle-outline", description: "AI 整理學生的作業問題，結構化後發送給教師" },
    ],
  },
  // ── 學生 → 行政 ──
  {
    ...
    actions: [
      { id: "auto_repair", label: "AI 代理報修", icon: "construct-outline", description: "AI 收集問題描述/照片後自動提交維修單" },
      { id: "auto_booking", label: "AI 代理預約", icon: "calendar-outline", description: "AI 預約座位/設備/場地" },
      { id: "document_request", label: "文件申請", icon: "document-outline", description: "AI 代為申請在學證明/成績單/各類文件" },
      { id: "scholarship_apply", label: "獎學金申請", icon: "trophy-outline", description: "AI 比對資格、準備文件、代為送件" },
    ],
  },
  // ── 學生 → 商家 ──
  {
  ...
  // ── 教師 → 學生 ──
  {
    ...
    actions: [
      { id: "batch_notify", label: "批次通知學生", icon: "megaphone-outline", description: "AI 代為發送課程公告至全班" },
      { id: "grade_release", label: "成績發布通知", icon: "school-outline", description: "AI 整理成績後分別通知每位學生" },
      { id: "assignment_publish", label: "發布作業", icon: "create-outline", description: "AI 格式化作業要求、設定截止日、通知全班" },
      { id: "attendance_alert", label: "出席警示", icon: "alert-outline", description: "AI 偵測缺曠達標時自動提醒學生和導師" },
      { id: "peer_review_assign", label: "分配互評", icon: "git-network-outline", description: "AI 公平分配互評配對，確保每人評/被評次數均等" },
      { id: "learning_insight", label: "學習分析回饋", icon: "analytics-outline", description: "AI 分析全班學習數據，個別化回饋建議" },
    ],
  },
  // ── 教師 ↔ 教師 ──
  {
    ...
    actions: [
      { id: "co_teach_sync", label: "協同教學同步", icon: "sync-outline", description: "AI 同步共授課程進度、分配教學範圍" },
      { id: "resource_share", label: "教材共享", icon: "folder-open-outline", description: "AI 推薦相關教材、協助跨系課程合作" },
    ],
  },
  // ── 商家 → 學生 ──
  {
    from: "vendor", to: "student",
    actions: [
      { id: "order_ready", label: "餐點備妥通知", icon: "checkmark-circle-outline", description: "AI 推播取餐通知至學生" },
      { id: "promo_push", label: "優惠推送", icon: "pricetag-outline", description: "AI 依據學生偏好精準推送優惠" },
      { id: "menu_update", label: "菜單更新通知", icon: "newspaper-outline", description: "AI 推播新菜色給偏好匹配的學生" },
    ],
  },
  // ── 行政 → 學生 ──
  {
    ...
    actions: [
      { id: "repair_done", label: "維修完成通知", icon: "checkmark-done-outline", description: "AI 通知報修學生維修已完成" },
      { id: "document_ready", label: "文件備妥通知", icon: "document-outline", description: "AI 通知學生申請文件已可領取" },
    ],
  },
  // ── 管理員 → 所有人 ──
  {
    ...
    actions: [
      { id: "system_alert", label: "系統公告", icon: "notifications-outline", description: "AI 推播全校/特定群體通知" },
      { id: "survey_collect", label: "問卷收集", icon: "clipboard-outline", description: "AI 對話式收集問卷回答" },
      { id: "emergency_broadcast", label: "緊急廣播", icon: "warning-outline", description: "颱風停課/緊急事件即時推播所有使用者" },
    ],
  },
  {
    from: "admin", to: "faculty",
    actions: [
      { id: "policy_update", label: "政策通知", icon: "briefcase-outline", description: "AI 推播教務/行政政策更新給教師" },
      { id: "schedule_change", label: "排課異動", icon: "swap-vertical-outline", description: "AI 通知教師課程調動並協調替代方案" },
    ],
  },
];
...
export function simulateProactiveMessages(): { trigger: ProactiveTrigger; message: string }[] {
  ...
  messages.push({ trigger: assignTrigger, message: "「程式設計 HW5」明天 23:59 截止！需要我幫你安排時間完成嗎？" });
...
export function buildKnowledgeGraph(data: {
  courses: Array<{ id: string; name: string; teacher: string; dayOfWeek: number; credits: number; startPeriod?: number }>;
  assignments: Array<{ id: string; title: string; groupName: string; dueAt?: string; isLate: boolean }>;
  ...
  data.courses.forEach(c => {
    nodes.push({
      id: `course_${c.id}`, type: "course", label: c.name,
      data: { teacher: c.teacher, dayOfWeek: c.dayOfWeek, credits: c.credits, startPeriod: c.startPeriod },
      connections: data.assignments.filter(a => a.groupName.includes(c.name)).map(a => `assignment_${a.id}`),
      lastUpdated: now, accessCount: 0,
    });
  });
  ...
  data.assignments.forEach(a => {
    ...
    nodes.push({
      id: `assignment_${a.id}`, type: "assignment", label: a.title,
  ...
  data.menus.forEach(m => {
    nodes.push({
      id: `menu_${m.id}`, type: "menu", label: m.name,
  ...
  data.memory.learnedFacts.forEach(f => {
    const prefNode: KnowledgeNode = {
      id: `pref_${f.id}`, type: "preference", label: f.fact,
      data: { category: f.category, confidence: f.confidence, source: f.source },
      connections: [],
      lastUpdated: f.learnedAt, accessCount: 0,
    };
    ...
    if (f.category === "dietary") {
      const relatedMenus = data.menus.filter(m => {
        ...
        return factLower.split(/[\s,，、]/).some(word => word.length >= 2 && menuName.includes(word));
      });
...
export function queryKnowledgeGraph(
  ...
  top.forEach(({ node }) => { typeCount[node.type] = (typeCount[node.type] || 0) + 1; });
  const reasoning = Object.entries(typeCount)
    .map(([type, count]) => `找到 ${count} 個相關${type === "course" ? "課程" : type === "assignment" ? "作業" : type === "menu" ? "餐點" : type === "preference" ? "偏好" : "項目"}`)
    .join("，");
...
export interface ResponseTemplate {
  ...
  skeleton: string;
  /** 插槽名稱 → 資料類型映射 */
  slots: Record<string, "courses" | "assignments" | "menus" | "events" | "announcements" | "pois" | "memory" | "text">;
  /** 被成功使用次數 */
  useCount: number;
  /** 平均品質 */
  avgQuality: number;
...
const STOP_WORDS = new Set([
  ...
  "嗎", "呢", "吧", "啊", "喔", "欸", "那", "什麼", "怎麼", "可以",
  "請問", "請", "幫", "幫我", "想", "想要", "能", "能不能",
]);
...
function tokenize(text: string): string[] {
  ...
  // 先提取已知領域詞彙（長詞優先）
  const domainTerms = [
    "被當", "當掉", "不及格", "二一", "退學", "學分", "必修", "選修", "通識",
    "作業", "報告", "期中考", "期末考", "小考", "點名",
    "圖書館", "借書", "還書", "座位", "自習室",
    "宿舍", "寢室", "報修", "洗衣", "包裹", "門禁",
    "餐廳", "食堂", "午餐", "晚餐", "早餐", "素食", "便當",
    "公車", "校車", "停車場", "腳踏車",
    "列印", "影印", "印表機",
    "獎學金", "社團", "畢業", "實習", "成績", "排名",
    "公告", "活動", "講座", "報名", "比賽",
...
const RESPONSE_VARIATIONS: Record<string, string[]> = {
  ...
  assignment_intro: ["作業狀況如下：", "來看看待繳作業，", "目前作業進度：", "幫你整理了作業資訊："],
  dining_intro: ["今天的美食選擇：", "幫你看了菜單，", "推薦以下餐點：", "今天有這些選擇："],
  event_intro: ["近期活動整理：", "有這些活動可以參考：", "校園活動資訊："],
  positive_close: ["希望有幫到你！", "還有什麼想問的嗎？", "需要更多資訊可以再問我。", "有其他問題隨時說。"],
...
function handleFollowUp(
  ...
  if (/還有嗎|還有呢|其他的|其他呢|別的呢|更多/.test(q)) {
    if (topic === "dining") {
      const mentioned = ctx.shortTermMemory?.find(m => m.key === "last_recommended_meals")?.value ?? "";
      const mentionedNames = mentioned.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const remaining = data.menus.filter(m => !mentionedNames.some(name => m.name.includes(name)));
      if (remaining.length > 0) {
        const list = remaining.slice(0, 5).map((m, i) =>
    ...
    if (topic === "academic") {
      const mentionedCourses = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_courses")?.value ?? "";
      const mentionedNames = mentionedCourses.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const remaining = data.courses.filter(c => !mentionedNames.some(name => c.name.includes(name)));
      if (remaining.length > 0) {
        return `還有這些課程：\n${remaining.slice(0, 5).map((c, i) =>
    ...
    if (topic === "assignment") {
      const mentionedAssign = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_assignments")?.value ?? "";
      const mentionedNames = mentionedAssign.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const remaining = data.assignments.filter(a => !mentionedNames.some(name => a.title.includes(name)));
      if (remaining.length > 0) {
        return `還有這些作業：\n${remaining.slice(0, 5).map((a, i) =>
    ...
    if (topic === "event") {
      const mentionedEvents = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_events")?.value ?? "";
      const remaining = data.events.filter(e => !mentionedEvents.includes(e.title));
      if (remaining.length > 0) {
        return `還有這些活動：\n${remaining.slice(0, 5).map((e, i) => `${i + 1}. ${e.title}${e.location ? `（${e.location}）` : ""}`).join("\n")}`;
      }
      return "目前就這些活動了。";
    }
    if (topic === "announcement") {
      const mentionedAnn = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_announcements")?.value ?? "";
      const remaining = data.announcements.filter(a => !mentionedAnn.includes(a.title));
      if (remaining.length > 0) {
        return `還有這些公告：\n${remaining.slice(0, 5).map((a, i) => `${i + 1}. ${a.title}${a.source ? `（${a.source}）` : ""}`).join("\n")}`;
      }
  ...
  if (/多少錢|價[格錢]|幾塊|多少元/.test(q)) {
    if (topic === "dining") {
      const mentioned = ctx.shortTermMemory?.find(m => m.key === "last_recommended_meals")?.value ?? "";
      const mentionedNames = mentioned.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const found = data.menus.filter(m => mentionedNames.some(name => m.name.includes(name)) && m.price != null);
      if (found.length > 0) {
  ...
  if (/在哪|怎麼去|位置|怎麼走/.test(q)) {
    // 從多個記憶源搜尋地點
    const cafeteriaMem = ctx.shortTermMemory?.find(m => m.key === "mentioned_cafeterias")?.value ?? "";
    const locationSources = [cafeteriaMem];

    // 從課程記憶找上課地點
    if (topic === "academic") {
      const courseMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_courses")?.value ?? "";
      const firstName = courseMem.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstName) {
    ...
    // 從活動記憶找地點
    if (topic === "event") {
      const eventMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_events")?.value ?? "";
      const firstName = eventMem.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstName) {
    ...
    for (const src of locationSources) {
      if (src) {
        const matchedPois = data.pois.filter(p => src.includes(p.name) || p.name.includes(src.split(", ")[0]));
  ...
  if (/什麼時候|幾點|幾號|何時|到幾點|截止/.test(q)) {
    if (topic === "assignment") {
      const assignMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_assignments")?.value ?? "";
      const firstName = assignMem.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstName) {
        const assign = data.assignments.find(a => a.title.includes(firstName));
        if (assign?.dueAt) return `「${assign.title}」的截止時間是：${assign.dueAt}${assign.isLate ? "（⚠️ 已經逾期了！）" : ""}`;
      }
    }
    if (topic === "event") {
      const eventMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_events")?.value ?? "";
    ...
    if (topic === "academic") {
      ...
      if (firstName) {
        const course = data.courses.find(c => c.name.includes(firstName));
        if (course) return `「${course.name}」的上課時間：${DAY_NAMES_LOCAL[course.dayOfWeek]}第${course.startPeriod ?? "?"}節`;
      }
    }
  }
  ...
  if (/好吃嗎|推薦嗎|值得嗎|CP值/.test(q)) {
    const mentioned = ctx.shortTermMemory?.find(m => m.key === "last_recommended_meals")?.value ?? "";
    if (mentioned) {
      const firstItem = mentioned.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstItem) {
  ...
  if (/不想要|換一個|換別的|太[貴遠辣鹹甜難簡單無聊]|不喜歡|其他選擇|不好|不適合/.test(q)) {
    if (topic === "dining") {
      ...
      if (filtered.length > 0) {
        const list = filtered.slice(0, 5).map((m, i) =>
          `${i + 1}. ${m.name}${m.price != null ? ` — $${m.price}` : ""}${m.cafeteria ? `（${m.cafeteria}）` : ""}`
        ).join("\n");
        return `好的，換一些不同的：\n${list}`;
      }
  ...
  if (/詳細|詳情|更多資訊|告訴我更多|說明|具體/.test(q)) {
    if (topic === "academic") {
      const coursesMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_courses")?.value ?? "";
      const firstName = coursesMem.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstName) {
        const course = data.courses.find(c => c.name.includes(firstName));
        if (course) {
    ...
    if (topic === "dining") {
      const mealMem = ctx.shortTermMemory?.find(m => m.key === "last_recommended_meals")?.value ?? "";
      const firstName = mealMem.split(", ")[0]?.replace(/^\d+\.\s*/, "").trim();
      if (firstName) {
  ...
  if (/比較|哪個好|哪個便宜|哪個近|差別|差異|vs|VS/.test(q)) {
    if (topic === "dining") {
      const mentioned = ctx.shortTermMemory?.find(m => m.key === "last_recommended_meals")?.value ?? "";
      const names = mentioned.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const found = names.map(n => data.menus.find(m => m.name.includes(n))).filter(Boolean);
      if (found.length >= 2) {
        const comparison = found.slice(0, 3).map(m => m ? `• ${m.name}：${m.price != null ? `$${m.price}` : "價格不詳"}${m.cafeteria ? ` @${m.cafeteria}` : ""}` : "").join("\n");
        return `幫你比較一下：\n${comparison}`;
      }
    }
    if (topic === "academic") {
      const coursesMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_courses")?.value ?? "";
      const names = coursesMem.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const found = names.map(n => data.courses.find(c => c.name.includes(n))).filter(Boolean);
      if (found.length >= 2) {
        const comparison = found.slice(0, 3).map(c =>
          c ? `• ${c.name}：${c.teacher ?? "?"}老師，${c.credits ?? "?"}學分，${DAY_NAMES_LOCAL[c.dayOfWeek]}` : ""
        ).join("\n");
        return `幫你比較一下這幾門課：\n${comparison}`;
      }
    }
    if (topic === "event") {
      const eventsMem = ctx.shortTermMemory?.find(m => m.key === "last_mentioned_events")?.value ?? "";
      const names = eventsMem.split(", ").map(s => s.replace(/^\d+\.\s*/, "").trim()).filter(Boolean);
      const found = names.map(n => data.events.find(e => e.title.includes(n))).filter(Boolean);
      if (found.length >= 2) {
        const comparison = found.slice(0, 3).map(e =>
          e ? `• ${e.title}${e.location ? ` — ${e.location}` : ""}${e.startsAt ? ` — ${e.startsAt}` : ""}` : ""
        ).join("\n");
        return `幫你比較一下：\n${comparison}`;
      }
    }
...
export function generateLocalAnswer(
  ...
  if (tags.includes("greeting")) {
    ...
    if (hour >= 11 && hour <= 13 && data.menus.length > 0) {
      ...
      parts.push(`提醒一下，你有 ${data.assignments.filter(a => a.isLate).length} 份作業已逾期哦。`);
  ...
  // ── 作業查詢 ──
    ...
    if (data.assignments.length > 0) {
      ...
      if (lateOnes.length > 0) {
        ...
        lateOnes.forEach((a, i) => parts.push(`  ${i + 1}. ${a.title}（${a.groupName}${a.dueAt ? `，截止：${a.dueAt}` : ""}）`));
      }
      if (upcoming.length > 0) {
        parts.push(lateOnes.length > 0 ? "\n即將到期的作業：" : "待繳作業：");
        upcoming.slice(0, 8).forEach((a, i) => parts.push(`${i + 1}. ${a.title}（${a.groupName}${a.dueAt ? `，截止：${a.dueAt}` : ""}）`));
      }
      answer = parts.join("\n");
    } else {
  ...
  // ── 成績 / 被當風險 ──
    ...
    if (riskKeywords.test(q)) {
      ...
      if (lateAssignments.length > 0) {
        const affected = lateAssignments.map(a => a.groupName).filter((v, i, arr) => arr.indexOf(v) === i);
  ...
  // ── 餐飲查詢（上下文 + 槽位感知）──
    ...
    if (data.menus.length > 0) {
      ...
      if (prefSlot) {
        ...
        if (/不吃肉/.test(pref)) filtered = filtered.filter(m => !/排骨|雞|豬|牛|肉/.test(m.name));
        if (/辣/.test(pref) && !/不辣/.test(pref)) filtered = filtered.filter(m => /辣/.test(m.name));
        if (/不辣/.test(pref)) filtered = filtered.filter(m => !/辣/.test(m.name));
      }

      if (cafeteriaSlot) {
        const cfFiltered = filtered.filter(m => m.cafeteria && m.cafeteria.includes(cafeteriaSlot));
  ...
  // ── 活動查詢 ──
    ...
    if (data.events.length > 0) {
      const list = data.events.slice(0, 6).map((e, i) =>
  ...
  // ── 公告查詢 ──
    ...
    if (data.announcements.length > 0) {
      const list = data.announcements.slice(0, 6).map((a, i) =>
        `${i + 1}. ${a.title}${a.source ? `（${a.source}）` : ""}`
      ).join("\n");
      answer = `最新公告：\n${list}`;
    } else {
      answer = "目前沒有新的公告。";
  ...
  // ── 地點導航 ──
    ...
    const matchedPois = data.pois.filter(p => {
      ...
      return q.includes(pName) || q.includes(pCat) || pName.split("").some((c, i, arr) => i < arr.length - 1 && q.includes(arr[i] + arr[i + 1]));
    });
    if (matchedPois.length > 0) {
      const list = matchedPois.slice(0, 5).map((p, i) =>
        `${i + 1}. ${p.name}${p.category ? `（${p.category}）` : ""}`
      ).join("\n");
      answer = `找到以下地點：\n${list}\n\n你可以使用 APP 的導航功能前往。`;
    } else if (data.pois.length > 0) {
      answer = "我沒有找到確切的地點，以下是校園內的一些常用地點：\n" +
        data.pois.slice(0, 8).map((p, i) => `${i + 1}. ${p.name}（${p.category ?? ""}）`).join("\n");
    } else {
      answer = "目前沒有校園地點資料。";
    }
    confidence = 0.8;
  ...
  if (!answer) {
    const kgResult = queryKnowledgeGraph(
      buildKnowledgeGraph({
        courses: data.courses.map(c => ({ ...c, teacher: c.teacher ?? "", credits: c.credits ?? 0 })),
    ...
    if (kgResult.relevantNodes.length > 0) {
      ...
      kgResult.relevantNodes.forEach(n => {
        const typeName = n.type === "course" ? "課程" : n.type === "assignment" ? "作業" :
          n.type === "menu" ? "餐點" : n.type === "event" ? "活動" :
          n.type === "preference" ? "偏好" : "項目";
...
export function updatePairQuality(
  ...
  const avgQuality = updatedPairs.length > 0
    ? updatedPairs.reduce((s, p) => s + p.quality, 0) / updatedPairs.length : 3;
...
/** 計算本地引擎對某問題的信心分數 */
export function getLocalConfidence(
  question: string,
  tags: string[],
  db: LocalTrainingDB,
): number {
...
export function exportTrainingInsights(db: LocalTrainingDB): string {
  ...
  if (db.pairs.length >= 10) {
    ...
    db.pairs.slice(-50).forEach(p => p.tags.forEach(t => { tagFreq[t] = (tagFreq[t] || 0) + 1; }));
