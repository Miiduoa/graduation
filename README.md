...
下列條目不取代完整 diff，而是用「接手的人可以立刻對上檔案與責任」的方式描述**本批合併至主線**的工程重點（延續先前：本地代理寫入安全、工具層護欄、`aiAgentSafetyHardening` 等，仍適用）。**最近一次主線快照另含**：餐廳端／管理端、`recommendLunch`、AI 浮球／工具 registry、POI 詳情、`classifyIntent` 測試、Maestro 02–12 對齊、分批長訓腳本與展示用 cafeteria／訂餐截圖（見下列「餐飲／AI／E2E／訓練」列）。
