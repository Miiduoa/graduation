  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: theme.colors.bg, justifyContent: "center", alignItems: "center" }}>
        ...
        <Text style={{ color: theme.colors.textSecondary, marginTop: theme.space.md }}>載入行事曆...</Text>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      >
        ...
        <View style={{ paddingHorizontal: theme.space.lg, marginBottom: theme.space.md }}>
          <Text style={{ color: theme.colors.text, fontSize: 28, fontWeight: "800" }}>智慧行事曆</Text>
        ...
        {activeTab === "plan" && (
          <StudyPlanSection plans={studyPlans} onRegenerate={async () => {
...
function WeekSummaryCard({ weekView }: { weekView: WeekView }) {
  ...
  return (
    <View style={{
      ...
      {/* Day bars */}
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: theme.space.md }}>
        {days.map((day, i) => {
...
function DeadlineSection({ deadlines, onComplete }: { deadlines: Deadline[]; onComplete: (id: string) => void }) {
        ...
        return (
          <View key={d.id} style={{
            ...
            <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" }}>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.colors.text, fontSize: 15, fontWeight: "700" }}>{d.title}</Text>
                {d.courseName && (
                  <Text style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 2 }}>{d.courseName}</Text>
                )}
              </View>
              <Pressable
            ...
            <View style={{ flexDirection: "row", gap: theme.space.md, marginTop: theme.space.sm }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                <Ionicons name="time-outline" size={14} color={urgent ? d.color : theme.colors.textSecondary} />
                <Text style={{ color: urgent ? d.color : theme.colors.textSecondary, fontSize: 12, fontWeight: urgent ? "700" : "400" }}>
                  剩 {timeLabel}
                </Text>
              </View>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                <Ionicons name="hourglass-outline" size={14} color={theme.colors.textSecondary} />
                <Text style={{ color: theme.colors.textSecondary, fontSize: 12 }}>預估 {d.estimatedHours}h</Text>
              </View>
              {/* Urgency bar */}
...
function StudyPlanSection({ plans, onRegenerate }: { plans: StudyPlan[]; onRegenerate: () => void }) {
  ...
  return (
    <View style={{ marginHorizontal: theme.space.lg }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: theme.space.md }}>
        <Text style={{ color: theme.colors.text, fontSize: 16, fontWeight: "700" }}>AI 讀書計劃</Text>
        <Pressable onPress={onRegenerate} style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
        ...
        Array.from(grouped.entries())
          ...
          .map(([day, dayPlans]) => (
            <View key={day} style={{ marginBottom: theme.space.md }}>
              <Text style={{ color: theme.colors.accent, fontSize: 14, fontWeight: "700", marginBottom: theme.space.sm }}>
                {dayLabels[day]}
              </Text>
              {dayPlans.map((plan) => (
                <View key={plan.id} style={{
                  ...
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: "600" }}>{plan.courseName}</Text>
                    <Text style={{ color: theme.colors.textSecondary, fontSize: 12 }}>
                      {plan.startHour}:00 - {plan.endHour}:00
                    </Text>
                  </View>
                  <Text style={{ color: theme.colors.textSecondary, fontSize: 12, marginTop: 2 }}>{plan.description}</Text>
                  <Text style={{ color: theme.colors.muted, fontSize: 10, marginTop: 4, fontStyle: "italic" }}>
                    AI: {plan.aiReason}
                  </Text>
                </View>
              ))}
      ...
      {stats && (
        <View style={{
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-around", marginBottom: theme.space.lg }}>
            {[
              { label: "今日", value: `${stats.todaySessions}`, sub: `${stats.todayMinutes}分鐘`, color: theme.colors.accent },
              { label: "本週", value: `${stats.weekSessions}`, sub: `${stats.weekMinutes}分鐘`, color: "#3B82F6" },
          ...
          {stats.subjectBreakdown.length > 0 && (
            <View>
              <Text style={{ color: theme.colors.text, fontSize: 14, fontWeight: "600", marginBottom: theme.space.sm }}>
                科目分佈
              </Text>
              {stats.subjectBreakdown.slice(0, 5).map((s, i) => {
                ...
                return (
                  <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    <Text style={{ color: theme.colors.text, fontSize: 12, width: 80 }} numberOfLines={1}>{s.subject}</Text>
                    ...
                    <Text style={{ color: theme.colors.textSecondary, fontSize: 11, width: 40, textAlign: "right" }}>
                      {s.minutes}m
                    </Text>
                  </View>
                );
