...
function buildDataSource(overrides: Partial<DataSource> = {}): DataSource {
  const base = {
    ...
    listSeats: jest.fn(async () => [
      { id: "seat-1", zone: "quiet", seatNumber: "A-1", floor: "3F", hasOutlet: true, isQuietZone: true, status: "available" },
    ]),
    reserveSeat: jest.fn(async (seatId: string, userId: string, date: string, startTime: string, endTime: string) => ({
      id: "seat-res-1",
      seatId,
      userId,
      date,
...
describe("aiActionExecutor", () => {
  it("executes repair, lost-found, reminder, health booking, and seat reservation through DataSource", async () => {
    ...
    await expect(executeAgentToolAction({
      ...baseContext,
      dataSource: ds,
    ...
    await expect(executeAgentToolAction({
      ...
      dataSource: ds,
      tool: tool("post_lost"),
      params: { item: "學生證", category: "student_card", location: "圖書館", features: "藍色吊牌" },
  ...
  it("denies role-mismatched tools and drafts unresolved social actions", async () => {
    ...
    expect(ds.createActionQueueItem).toHaveBeenCalledWith(expect.objectContaining({
      action: "send_message",
      status: "pending_confirmation",
    }));
  });
