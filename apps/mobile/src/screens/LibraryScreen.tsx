...
import {
  ...
  smartSearchBooks,
  getSmartLibrarySuggestions,
  type SmartSuggestion,
  simulateFloorOccupancy,
  type FloorOccupancy,
  STUDY_TIMER_PRESETS,
  type StudyTimerPreset,
  READING_ACHIEVEMENTS,
  type ReadingAchievement,
  POPULAR_SEARCHES,
  STAFF_PICKS,
  getBookById,
...
export function LibraryScreen(props: Record<string, unknown>) {
  ...
  const handleBookARNav = useCallback(
              ...
              <Pressable
                onPress={() =>
                  Alert.alert(
                    '開放時間',
                    OPENING_HOURS.map(
                      (h) => `${h.label}：${h.isOpen ? `${h.open}–${h.close}` : '休館'}`,
                    ).join('\n'),
                  )
                }
              >
                    ...
                    {suggestions.map((s) => (
                      <Pressable
                        ...
                        onPress={() => {
                          ...
                          else if (s.action === 'ebook')
                            Alert.alert('HyRead 電子書', '前往 1F 無紙境電子書區或開啟 HyRead App');
                        }}
                        style={({ pressed }) => ({
                          width: 140,
                          padding: 14,
            ...
            <AnimatedCard title="樓層人潮" subtitle="即時座位狀況" delay={200}>
              <View style={{ gap: 6 }}>
                {occupancy.slice(0, 6).map((o) => {
                  ...
                  const barColor =
                    pct > 70 ? theme.colors.danger : pct > 40 ? '#F59E0B' : theme.colors.success;
                  return (
                    <Pressable
                      key={o.floor}
                    ...
                    <View key={a.id} style={{ alignItems: 'center', width: 72 }}>
                      ...
                      >
                        <Ionicons
                          name={a.icon as any}
                          size={22}
                          color={earned ? a.color : theme.colors.muted}
                        />
                      </View>
                      <Text
                        style={{
                          color: earned ? theme.colors.text : theme.colors.muted,
                          fontSize: 10,
                          marginTop: 4,
                          textAlign: 'center',
                        }}
                ...
                return (
                  <View style={{ gap: 8 }}>
                    ...
                    <ListItem
                      ...
                      title="續借次數"
                      subtitle={`${p.renewTimes} 次`}
                    />
                    <ListItem
                      icon="bookmark-outline"
                      title="預約冊數"
                      subtitle={`${p.reserveLimit} 冊`}
                    />
                    <ListItem
                      icon="alert-circle-outline"
                      title="逾期罰款"
                      subtitle={`每冊每日 $${p.overdueFinePerDay} 元`}
                    />
                  </View>
                );
              })()}
            ...
            >
              {[
                ...
                <View key={l.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                  <View
                    style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: l.color }}
                  />
          ...
          <>
            ...
            {STUDY_ROOMS.filter((r) => r.type === 'discussion').map((room, idx) => (
              <AnimatedCard
                key={room.id}
                title={room.name}
                subtitle={`容量 ${room.capacity} 人 · 上限 ${room.maxHoursPerSession} 小時`}
                delay={idx * 50}
              >
            ...
            {STUDY_ROOMS.filter((r) => r.type === 'research').map((room, idx) => (
              <AnimatedCard
                key={room.id}
                title={room.name}
                subtitle={`${room.floor} · 個人使用 · 上限 ${room.maxHoursPerSession} 小時`}
                delay={idx * 50}
              >
            ...
            {STUDY_ROOMS.filter((r) => r.type === 'av_room').map((room, idx) => (
              <AnimatedCard
                key={room.id}
                title={room.name}
                subtitle={`容量 ${room.capacity} 人 · 2F 數位學習中心`}
                delay={idx * 50}
              >
                <View style={{ flexDirection: 'row', gap: 12, marginBottom: 10 }}>
          ...
          <>
            {activeTimer && timerRunning ? (
              ...
              <AnimatedCard
                ...
                subtitle={`第 ${timerRound}/${activeTimer.rounds} 輪 · ${timerPhase === 'focus' ? '專注中' : '休息中'}`}
              >
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={{ color: theme.colors.accent, fontSize: 12, marginTop: 4 }}>
                            {preset.focusMinutes} 分鐘專注 · {preset.breakMinutes} 分鐘休息 ·{' '}
                            {preset.rounds} 輪
                          </Text>
                        </View>
                ...
                <AnimatedCard title="學習小技巧" delay={100}>
                  <View style={{ gap: 8 }}>
                    <ListItem
                      icon="water-outline"
                      title="補充水分"
                      subtitle="每 25 分鐘喝一口水，保持頭腦清醒"
                    />
                    <ListItem
                      icon="eye-outline"
                      title="20-20-20 護眼"
                      subtitle="每 20 分鐘看 20 英尺外的物體 20 秒"
                    />
                    <ListItem
                      icon="body-outline"
                      title="伸展運動"
                      subtitle="休息時站起來活動身體，提升血液循環"
                    />
                    <ListItem
                      icon="phone-portrait-outline"
                      title="遠離手機"
                      subtitle="專注時段將手機翻面或開啟勿擾模式"
                    />
                  </View>
                </AnimatedCard>
              </>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                >
                  ...
                  {selectedBook.pageCount && (
                    <InfoLine label="頁數" value={`${selectedBook.pageCount} 頁`} />
                  )}
                </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
          ...
          <View
            style={{
              ...
              borderRadius: 4,
              backgroundColor:
                book.availableCopies > 0 ? `${theme.colors.success}20` : `${theme.colors.danger}20`,
            }}
          >
            <Text
              style={{
